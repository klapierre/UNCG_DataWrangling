...
setwd("C:/Users/croni/Downloads/Data Wranging/UNCG_DataWrangling")
...
# Answer: It imported the same dataset two ways, once from data/CME011.cvs and once directly from the data portal URL, creating two dataframes: conSME_abundance and conSMEcoverAlt.  
...
# Answer: Sourcing data straight from the data portal makes sure you are always using the most up to date version of the data set, it also improves reproducibility. It might be worse if the portal isn't available, if the website is down or the link changes, the code could fail. 
