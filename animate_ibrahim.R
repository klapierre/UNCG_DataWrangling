...
ggplot(iris, aes(x = Species, y = Petal.Length)) + #creates graph with x as species and y as petal length
  geom_boxplot(aes(colour = Species)) + #turns into boxplot and colors the boxplots by species
  transition_states(Petal.Width, #cycling through values of petal width 
                    transition_length = 1, #spending the same amount of time going to the next cut as spent pausing there
                    state_length = 1) #same as above
...
#The differences are not as drastic as I thought they would be
...
ggplot(iris, aes(x = Species, y = Petal.Length)) +
  ...
  transition_states(Petal.Width,
                    transition_length = 1,
                    state_length = 1) +
  labs(x = "Species", y = "Petal Length (cm)", title = "Iris Petal Length of each species by Width")
...
ggplot(iris, aes(x = Species, y = Petal.Length)) +
  ...
  labs(x = "Species", 
       y = "Petal Length (cm)", 
       title = "Iris Petal Length of each species by Width") +
  enter_fade() + 
  exit_fade()
...
Iris_annimate <- ggplot(iris, aes(x = Species, y = Petal.Length)) +
  geom_boxplot(aes(color = Species)) + 
  transition_states(Petal.Width,
                    transition_length = 1,
                    state_length = 1) +
...
Iris_annimate <- animate(Iris_annimate, renderer = gifski_renderer())
...
anim_save('iris_annimate.gif', animation = last_animation(), path = NULL )
...
labs(title = 'Days: {frame_time}', x = 'Chick weight (grams)', y = 'Chick number') + #Creates labels and title
  transition_states(Time,  transition_length = 1, #sets timing of movement
                    state_length = 1) + 
  ease_aes('linear') #makes it smoother
...
anim_save("Chicks", animation = last_animation(), path = NULL)
...
chick_bar <-  ggplot(ChickWeight, aes(x=weight, y=Time)) + 
...
anim_save("chic2.gif", animation = last_animation(), path = NULL)
...
#You can see changes through out time, and maybe notice patterns in specific sets of time
...
#Line graphs that are super complex
...
line_graph <- ggplot(orange, aes(age, circumference, group = Tree, color = as.factor(Tree)))+ #makes graph, and sets axes 
scale_colour_brewer(palette = "BrBG") + #sets color palette
  geom_line() + #makes line graph
  labs(title = 'Orange Tree Circumference by Age', x = 'Age in Days', y = 'Circumference in mm') + #sets title and labels
  theme(legend.position = "top") + #creates a legend at top
  geom_point(aes(group = seq_along(age)),shape=3, size=5) #sets size and shape of points on graph
line_graph
...
# This is good if you wanted to point out how it changed through time

#Now for graph 2!We're going to do a bar graph!

#Let's start by making a simple bar graph
bar_graph <- ggplot(orange, aes(age, circumference, fill=as.factor(Tree))) + #create graph
  geom_bar(stat="identity", position=position_dodge()) + #make bar graph
...
bar_graph <- ggplot(orange, aes(age, circumference, fill=as.factor(Tree))) +
  ...
  labs(title = 'Orange Tree Circumference According to Age', x = 'Age (days)', y = 'Circumference (mm)') +
...
#Question: When would this type of graph and animation be an appropriate way to communicate your data?
#when you want to compare values of different variables
...
racing_bar_graph <- ggplot(orange, aes(x=age, y=circumference, fill=as.factor(Tree))) + 
  geom_bar(stat="identity",
           position=position_dodge()) +
  scale_fill_brewer(palette = "BrBG")+
  transition_states(Sample,
                    transition_length = 5, state_length = 1, wrap = FALSE) +
  ease_aes('linear')+
  view_follow(fixed_x = TRUE)+
  enter_fade()+
  exit_fade()

racing <- animate(racing_bar_graph, renderer = gifski_renderer())
anim_save('racing.gif', animation = last_animation(), path = NULL)
...
#It could make it hard to distinquish siblings from parents
...
titanicTidyAge <- rawTitanic %>% 
  ...
  mutate(AgeDecade = ifelse(AgeDecade>=60,"60+", AgeDecade))
...
Age_titanic <- titanicTidyAge
ggplot(Age_titanic, aes(x=Sex, fill = Sex)) +
  scale_fill_brewer(palette = "BrBG")+
  geom_bar() +
  transition_states(AgeDecade, state_length = 1, transition_length = 1) +
  labs(subtitle = "Age versus Gender ")
...
ggplot(titanicTidyCabin, aes(x=Deck, y=Age, fill=as.factor(Survived), shape=Sex)) +
  ...
  enter_fade() +
  exit_fade() +
  labs(subtitle = "Deck versus Age, Genders Survived")
