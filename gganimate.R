...
# Transition length is the amount of frames for the transition.
#state length is the pause length
...
transition_states(Petal.Width,
                  transition_length = 35,
                  state_length = 11)
# Same as the last task we were annotating the frames and pasue length.
...
#each state is dependent on the time variable.
...
# its important to be able to show the dataframe changes. 
...
# The data is using time as a manner to show the info.
...
# the accent to color the graphs and the bar_graph command to actually display it.
...
# The annimation shows individuals over time. So, yes it is apropriate. 
...
ggplot(orange, aes(circumference, group = Tree)) +  
  ...
  theme(axis.line=element_blank(),
        ...
        panel.grid.major.x = element_line( size=.1, color="blue" ),
        panel.grid.minor.x = element_line( size=.1, color="red" ),
        plot.margin = margin(1,1, 1, 2, "cm")) +
  geom_text(aes(y = 0, label = paste(Tree)), vjust = 0.3, hjust = 2) +
...
# people with different relationships are still counted the same which can skew the data. 
