...
ggplot(iris, aes(x = Species, y = Petal.Length)) + # setting the dataset, x axis, and y axis
  geom_boxplot(aes(colour = Species)) + # setting the color to color by species
  transition_states(Petal.Width, # telling ggplot what to use for the transitions (each frame is a different petal.width)
                    transition_length = 1, # the length of the transition
                    state_length = 1) # the length of the pause between frames
...
### increasing the transition length keeps each frame up for longer wherease increasing the state length makes the pauses longer
...
ggplot(iris, aes(x = Species, y = Petal.Length)) + 
  ...
  transition_states(Petal.Width,
                    ...
                    state_length = 2)+
  ylab("Petal Length")+
  ggtitle("Iris Petal Length Over Increasing Petal Widths")
...
Petal_length_width <- ggplot(iris, aes(x = Species, y = Petal.Length)) + 
  geom_boxplot(aes(colour = Species)) +
  transition_states(Petal.Width,
                    transition_length = 4,
                    state_length = 2)+
...
iris_saved <- animate(Petal_length_width, renderer = gifski_renderer())
...
anim_save('Petal_length_width.gif', animation = last_animation(), path = NULL)
...
library(readr)
rawTitanic <- read_csv("titanic.csv")
...
## you can't tell how many of the recorded value are siblings vs. spouses, or parents vs. children


# Question: What columns seem to be missing information?
## cabin number and age
...
titanicTidyCabin <- rawTitanic %>% 
  ...
  mutate(Deck = str_extract(Cabin, "[A-Z]+")) %>% 
  na.omit()
...
tidyAge <- titanicTidyAge %>% 
  filter(Survived==1)

ggplot(tidyAge, aes(y=Survived, x=Sex, fill = Sex)) +
  geom_bar(stat="identity") +
  transition_states(AgeDecade, state_length = 1, transition_length = 3) +
  ease_aes("sine-in-out") +
  labs(subtitle = "Passenger age: {closest_state}'s")+
  enter_fade()+
  exit_fade()+
  scale_fill_brewer(palette = "Accent")
...
ggplot(titanicTidyCabin, aes(x=Deck, y=Age, color=Survived, shape=Sex))+
  ...
  enter_fade() +
  exit_fade() +
  labs(subtitle = "Class {closest_state} passengers")
