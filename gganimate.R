...
ggplot(iris, aes(x = Species, y = Petal.Length)) + #Load in the data set and set our x & y variables
  geom_boxplot(aes(colour = Species)) + #Set up a boxplot, with the colors associating with each species in the dataset
  transition_states(Petal.Width, # Select to change the graph over time by this variable
                    transition_length = 1, #How long the transition lasts
                    state_length = 1) #How long it stays within a new state
...
iris_data <- ggplot(iris, aes(x = Species, y = Petal.Length)) + 
  geom_boxplot(aes(colour = Species)) +
  xlab("Species")+
  ylab("Petal Length over time")+
  transition_states(Petal.Width,
...
iris_save <- animate(iris_data, renderer = gifski_renderer())
anim_save('iris.gif', animation = last_animation(), path = NULL)
