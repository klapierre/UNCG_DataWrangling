...
countypop <- countypop
plot_usmap(data = countypop, values = "pop_2015", include = c("VA", "WV"), color = "black") + 
  scale_fill_continuous(low = "white", high = "red", name = "Population", label = scales::comma) + 
  labs(title = "Virginia and West Virginian", subtitle = "Population by County 2015") +
  theme(legend.position = "right")
...
age_pop_states <- inner_join(age_pop_tidy, states)
...
age_pop_again <- age_pop %>% 
  rename(region = Location) %>% 
  mutate(region = tolower(region)) %>% 
  select(region, Adults.55.64)

age_pop_again$Adults.55.64 <- as.numeric(age_pop_again$Adults.55.64)

age_pop_join <- inner_join(age_pop_again, states)

ggplot()+
  geom_polygon(data=usa, aes(x=long, y=lat, group=group), color = "black", fill=NA)+
  coord_fixed(1.3)+
  geom_polygon(data=states, aes(x=long, y=lat, group=group), fill=NA, color="white")+
  geom_polygon(data=age_pop_join, aes(x=long, y=lat, group=group, fill = Adults.55.64), color = "white")+
  geom_polygon(data=usa, aes(x=long, y=lat, group=group), color="black", fill = NA)+
  scale_fill_gradientn(trans = "log10", colors=rev(brewer.pal(9, "BrBG")))
...
ggplot() +
  geom_polygon(data = NY, aes(x=long, y=lat, group=group), fill = "lightblue", col = "black") +
  coord_fixed(1.5) +
  geom_point(data = starb_NY, aes(x = longitude, y = latitude), color = "red")
...
ggplot() +
  geom_polygon(data = CA, aes(x=long, y=lat, group=group), fill = "lightgreen", col = "black") +
  coord_fixed(1.5) +
  geom_point(data = starb_CA, aes(x = longitude, y = latitude), color = "blue")
#--------------------------------#
...
world_real <- map_data("world")

ggplot(world_real, aes(long, lat, group = group)) +
...
#the capitals of each country and their coordinate points
ggplot() +
  geom_polygon(data = world_real, aes(long, lat, group = group)) +
  geom_polygon(fill = "lightblue", color = "black") +
  coord_fixed(1.3) +
  geom_point(data = caps, aes(x= Longitude, y= Latitude), color = "#Fe019a" , size = .5, shape = 25)
...
ggplot() +
  geom_polygon(data = world_real, aes(long, lat, group = group), fill = "lightblue", color = "black") +
  #geom_polygon(fill = "white", color = "black") +
  coord_fixed(1.3) +
  geom_point(data = caps, aes(x= Longitude, y= Latitude), color = "#Fe019a" , size = .5, shape = 25) +
  geom_text(data = caps, aes(x = Longitude, y = Latitude, label = Capital.City), size = 1.2)
...
pines <- read.csv("observations-309667.csv.crdownload")

# What does this data frame look like? How many observations are there?
#just all of the data for iNaturalist
#there are 153439 observations
...
specific_caps <- caps %>% 
  filter(Country %in% c("United States of America", "Australia", "France", "Italy"))

ggplot() +
  geom_polygon(data = world_real, aes(long, lat, group = group), fill = "lightblue", color = "black") +
  coord_fixed(1.3) +
  geom_point(data = caps, aes(x= Longitude, y= Latitude), color = "orange" , size = .5, shape = 25) +
  geom_text(data = specific_caps, aes(x = Longitude, y = Latitude, label = Capital.City), size = 1.2, color = "red")
...
pine_dates <- pines %>% 
  ...
  filter(observed_on > 2023-01-01)

pine_dates_map <- pines %>% 
  filter(observed_on > 2023-01-01) %>% 
  filter(scientific_name %in% c("Pinus palustris"))
...
ggplot() +
  ...
  coord_fixed(1.3) +
  geom_text(data = specific_caps, aes(x = Longitude, y = Latitude, label = Capital.City), size = 1.2, color = "red")+
  geom_point(data = pine_dates_map, aes(x = longitude, y = latitude), color = "green", size = .2)

# Then plot all European pine and make those points yellow.
pine_dates_eur <- pines %>% 
  filter(observed_on > 2023-01-01) %>% 
  filter(scientific_name %in% c("Pinus sylvestris"))

ggplot() +
  geom_polygon(data = world_real, aes(long, lat, group = group), fill = "lightblue", color = "black") +
  coord_fixed(1.3) +
  geom_text(data = specific_caps, aes(x = Longitude, y = Latitude, label = Capital.City), size = 1.2, color = "red")+
  geom_point(data = pine_dates_eur, aes(x = longitude, y = latitude), color = "yellow", size = .2)

# This is a huge dataset so go ahead and plot another species your choice, just make it clear which one you choose

pine_dates_lolly <- pines %>% 
  filter(observed_on > 2023-01-01) %>% 
  filter(scientific_name %in% c("Pinus taeda"))

ggplot() +
  geom_polygon(data = world_real, aes(long, lat, group = group), fill = "lightblue", color = "black") +
  coord_fixed(1.3) +
  geom_text(data = specific_caps, aes(x = Longitude, y = Latitude, label = Capital.City), size = 1.2, color = "red")+
  geom_point(data = pine_dates_lolly, aes(x = longitude, y = latitude), color = "red", size = .2)
