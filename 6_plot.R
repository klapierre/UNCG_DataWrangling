...
# the x-axis of the graph was generated and labeled 0-500 with tick marks of 100.
# there is no data because we did not tell R how to plot the data in.
...
# the x axis remained the same but now we have a y axis which fits the data
...
# chonky graph. same graph but the bars are thicker and cover more groups
...
# most of the trouts are .5 - 1.5 years of age
...
# I got "Removed 88 rows containing non-finite values (`stat_bin()`)"
# This is most likely the result of there being NAs in the data table.
# It should be ok since the warning just tells me how many Nas were purged before the histogram was made
...
# the redband is being examined
# What are the aesthetics? What will the resulting graph be plotting?
# the aesthetics are scaleage vs length
# the resulting graph will be plotting scaleage and length
...
# there is a correlation between the fish age and its length being that at a certain age, the fish is the biggest it will ever be and young fish start off small
...
# IT'S BLUE
# it changed from every age having its own distinct color to being a simple gradient
...
?geom_smooth
# mgcv::gam() is used with formula = y ~ s(x, bs = "cs") with method = "REML"
...
# it is a bar graph with the weight as the y-axis and the scale age as the x-axis
# the fish sure got fat
# after some picking at the data, it looks like R took the sum of all the values in the scale age group and graphed that 
# (the sum of the values in scale age 1 = 83069) (the sum in #4 is 39234)
...
redbandSummary <- redband %>% 
  group_by(ScaleAge) %>% 
  summarise(across(.cols="Weight", .fns=list(mean=mean, sd=sd), na.rm=T)) %>%
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  geom_bar(stat='identity') + # so all of this makes the bar graph
  geom_errorbar(aes(ymin=Weight_mean-Weight_se, # this is for the error bars 
                    ymax=Weight_mean+Weight_se, # the min and max for the bars are along the y axis and are found using the given equations (mean - weight & mean + weight)
                    width=.2)) # tells us how long we want the tops of our error bars to look
...
# WOAH! many colorful lines of best fit
...
ggplot(redband, aes(x = Length, y = Weight)) + 
  geom_point() +
  theme(panel.grid.minor = element_blank()) # there are grid lines

ggplot(redband, aes(x = Length, y = Weight)) + 
  geom_point() +
  theme(panel.background = element_blank()) # the lines are gone and the background is completely blank
...
# it showed up in the UNCG_DataWrangling
# it was my graph!
...
ggsave("Redband_histogram_facet_01.png",
       plot = last_plot(), 
       width = 10, 
       height = 8, 
       units = "in", 
