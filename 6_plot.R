...
## the x axis, there is no data because we didn't tell R what data to use
...
## a histogram. the label for the x axis is the length column, and the y axis was given
## the title count as a default
...
## we made each bar in the histogram wider
...
## the majority of the Redband Trout population are 1 year old
...
## R removed 88 rows containing non-finite values. I think the NA values are confusing R. 
## I think it would be okay since the rows that were removed were NA values that wouldn't have
## contributed anyways. However, you would probably want to write in code to ignore the NA values
## to make sure there aren't any other data points being removed that you want to keep.
...
## the dataframe being used is the redband dataframe. The x axis will be ScaleAge and the y axis
## will be length. There wouldn't be a resulting graph since we didn't specify what kind we want.
...
## there is a positive correlation between scale age and length. The older the fish, the longer it is
## generally speaking
...
## the legend goes away since there are not multiple colors/variables that need to be distinguished
## from each other
...
## color means the color of the lines/outlines, fill means the inside of the boxplot
...
## running it without as.factor() means R included ScaleAge in the calculation of the box plot
## rather than making a box plot for each scale age
...
## it made a bar graph of weights over each scale age, however the scale is way too large.
## It might have summed the weights resulting in very large numbers
...
## you use stat="identity" to make sure that R uses your data as is, rather than counting the
## number of cases
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + # setting the x and y axis
  geom_bar(stat='identity') + # telling R to use our data as is
  geom_errorbar(aes(ymin=Weight_mean-Weight_se, # indicating the bottom of the error bar
                    ymax=Weight_mean+Weight_se, # indicating the top of the error bar
                    width=0.2)) # indicating the width of the error bar
...
## it determines the width of the top and bottom lines of the error bar 
