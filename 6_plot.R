...
## There is no data because there are no commands to add data or geometry. 
...
## a histogram graph was created using the data and the x and y axis are labeled simply and apropreitly.
...
## The binwidth changed from 30 to 50 making the raph much easier to read. Looks like less data is being thrown at me.
...
## I notice the quantity of fish around 1 years of age are very frequent while all other age groups are not.
...
## I think it means there is data being removed due to the value they are given ie. non-infinite values. I think the code should be altered before continueing. 
# ---------------------------------------------------------- #
...
## dataframe is redband and the aes are comparing the age and length of the fish.
...
## The fish get bigger as they get older.
...
## The difference is the distribution points no longer are color coded. Why is becuase removing the as.factor and ScaleAge removed the command. 
...
## The color function color codes the data and fill filled the box with the color we commanded it to.
...
ggplot(redband, aes(x=(ScaleAge), y=Weight)) +
  geom_boxplot(color = 'purple', fill= 'green')
## Well it removed the individual boxes so I imagine the function tells R to fill the individuals and not the entire are between all the boxes.
# ---------------------------------------------------------- #
...
## Its a bar graph that shows the distribution of weights over age. The results show the sum of all the wieghts rather then individuals at that weight.
...
ggplot(redbandSummeary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  geom_bar(stat='identity')
...
##  teh stat function was essential to runningn the code wihtout it the graph wouldn't be created.
...
## first line crates the graph and the axis, second dictates it to become a bar graph, third adds error bars and dictates what values and how to get the error bar.

# QUESTION: What does the statement width=0.2 do? If you're unsure, try removing
# it and seeing what happens.
## the width= .02 dictates the size of the error bar.
...
## correlation will be a dot plot with  trend line. Deviation will use a boxplot. Distribution will be could be either a bar of histogram.  
...
ggplot(redband, aes(x = Length, y = ScaleAge)) + 
  geom_point(size=7, color="darkblue", shape=1) +
  scale_x_log10() + 
  scale_y_log10() +
  ggsave("Redband_histogram_facet.png", dpi=450)
# QUESTION: Why do you think we focused on Redband Trout for most of this assignment?
## Redband had a lot of data and was easy to manipulate how and what data was being represented. 
