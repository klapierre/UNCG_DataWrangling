...
# no grapphing function was applied 
...
# a histogram of length (of fish) by count (the number of each specimen that fell into each length group) was built. Because a y-axis variable was not provided, R automatically provided one
...
# the x-axis was split into groups of 50, starting at 100 because we do not have any fish with a length of <50. Fish were sorted into lengths between 100-149, 150-199, 200-249, etc. 
...
# scale age is an exact number rather than a range of numbers, so we only have bars for each possible age, 0 - 7 years (as opposed to what we saw when computing with a defined bin width)
