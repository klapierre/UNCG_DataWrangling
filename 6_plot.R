...
#A plot containing an an X axis labeled length was generated. 
#We don't see any data on the graph because we don't have a y axis.
...
#A graph with a large spike around x 250 rising to around y 140. 
#The x and y axis are scaled differently and the capitilization of their
#labels are also inconsistent. 
...
#The amount of our rectangles changed as well as their width. 
...
#As fish increase in years, the population declines.
...
# "Removed 88 rows containing non-finite values (stat_bin)." There are non
# numerical values in our data. There is no need to remove NAs. 
...
#redband is being examined. Resulting graph will plot ScaleAge vs Length.
...
#As fish age increases, so does length. 
...
#The colors of the dots changes. Each number is scale age was being mapped with its own color.
#Now, we are mapping across as a color gradient. 
...
#Color colorizes the outline of the box and the whiskers as well as the outliers.
#Fill fills the empty space inside of the outline. 
...
#as.factor allowed us to create an individual box plot per age. Without this,
#we get a very strange affect (aggregated??)
# ---------------------------------------------------------- #
...
ggplot(redband, aes(x=Length, y=Weight)) + 
  geom_point(aes(color=as.factor(ScaleAge))) +  geom_smooth(formula = y ~ poly(x,2), color = "black", size = 2)
# ---------------------------------------------------------- #
...
#Y values seem far to large. Possibly the weight values are being summed per weight age.
...
redbandSummary <- group_by(redband, ScaleAge) %>% summarise(Weight_mean = mean(Weight, na.rm= T), Weight_sd = sd(Weight, na.rm = T)) %>% 
...
# stat='identity' will leave the data as it is. 
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + #Creates a new ggplot pulling data from redbandSummary, setting x to
  #ScaleAge as factor and y equal to Weight_mean.
  geom_bar(stat='identity') + #leaves the data as it was
  geom_errorbar(aes(ymin=Weight_mean-Weight_se, #establishes min for error bar
                    ymax=Weight_mean+Weight_se, #establishes max for error bar
                    width=0.2)) #sets width for error bar. 
...
#Sets the width of the arrow bar.
...
#The legend appear slighly different. 
