...
# a plot with only an x-axis was generated, there is no data included because we didnt 
#specify what to add in the plot and we have lots of columns and variables
...
#It put the data from the red band data set and also gave labels for each axis.
...
#The boxes have a width of 50 so that they encompass more data instead of putting the data in small boxes.
...
# The age 1 is most common and the rest are all way lower
...
#The warning says that 88 rows were rmoved containing non finite outside the scale range.This means that there were values
#that could not be shown because they didnt end in a range that is close to the values on the graph.
#It is best to alter teh code to get rid of the message because if its removing data from the graph
#its not allowing for us to see the whole entire dataset.
...
#The output shows a big y axis which doesnt make it look correct because the weights werent summarized
#by age showing the sums instead of seperating them.
...
# HINT:Don't forget to remove NAs and ungroup at the appropriate place.
  ...
  redbandSummary <- redband %>% filter(!is.na(Weight)) %>% group_by(ScaleAge) %>% 
...
#stat='identity' is used so that the ggplot used the values we give them instead of 
#calculating it on its own.
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  #Creates a summarized plot with the x axis as the age and the y axis as the weight
  geom_bar(stat='identity') +
  #The bars are drawn using the values we give
...
#Width 0.2 changes the width at the top and bottom of the error bars. Without it they become wider.
...
#It was saved to the current working directory that i have for R studio which allows it to 
#show up in my files.
...
# the scatterplot was an example of correlation, the bar graph showed deviations
# and both the histogram and the box plot showed distributions.
...
ggplot(SpokaneFish, aes(x = Length, y = Weight, color = Species)) +
  geom_point(shape = 17) + scale_x_log10() + xlab("Fish Length (mm)") + 
  ylab("Fish Weight (g)") + facet_wrap(~Species)

ggsave("SpokaneFish_length_weight_by_species.png", width = 9, height = 7, dpi = 450)

# QUESTION: Why do you think we focused on Redband Trout for most of this assignment?

#To get us ready for our own individual projects and because if we were to start off 
#learning by using more complex data that has multiple species it would be harder to understand.
#the information.
