...
##It generated a blank plot with just the x-axis labeled for Length.
##The code only defines length on the x-axis,so no actual graph is created yet.
...
##It made a histogram of the fish lengths. The x-axis is labeled Length and the 
#y-axis is count. The x-axis shows the lengths and the y-axis shows 
#how many fish are in each group.
...
##The bars got wider and there are fewer of them. The data is grouped into bigger
#sections so it looks less detailed.
...
##Most of the fish are younger because the bars are higher at lower ages.
#There are not many older fish.
...
#This means some fish didn’t have a weight so they weren’t included in the graph.
#I think it’s okay to continue, but you could remove the missing values if you 
#wanted to get rid of the warning.
...
##The dataframe being used is redband. The aesthetics are ScaleAge on the x-axis
#and Length on the y-axis. It will plot fish age versus fish length.
...
##As the fish get older, their length generally increases. So there is a positive 
#relationship between age and length.
...
##When using as.factor(), the colors are grouped into categories for each age.
#Without it, the colors form a gradient because ScaleAge is treated as 
#a continuous variable. This happens because ggplot handles factors as categories
#and numeric values as continuous.
...
ggplot(redband, aes(x=Length, y=Weight)) + 
  geom_point(aes(size=ScaleAge, color=ScaleAge))
...
##Color controls the outline of the boxplot, and fill controls the inside color

# QUESTION: Why did we have to specify as.factor() for ScaleAge in the initial
# aes() statement? 
##We use as.factor() because ScaleAge represents categories,not a continuous 
#variable. This makes ggplot treat each age as its own group instead of a gradient.
...
##It is using a loess smoothing function.Thats default for smaller datasets like this.
...
##Another statistical transformation we used earlier was geom_histogram().
...
##The graph shows bars that are way too tall on the y-axis. It doesn’t seem right
#because it is plotting the raw weight values instead of a summary. 
#I think ggplot is just using the actual values without calculating a mean, 
#so the bars don’t represent what we want.
...
##It tells ggplot to use the actual values from the dataframe instead of calculating counts.
...
##The first line sets the dataframe and axes.
##The bar graph line creates bars using the mean weight values.
##The error bar line adds error bars using the standard error above and below the mean.

# QUESTION: What does the statement width=0.2 do? If you're unsure, try removing
# it and seeing what happens.

##It controls how wide the error bar caps are.If you remove it, the caps become wider.
...
##The colors now apply to both the points and the lines instead of just the points. 
#Before, only the points were colored by ScaleAge, but now the smoothing lines 
#are also colored by ScaleAge because the aesthetic was moved to the main ggplot statement.
...
##panel.grid.minor refers to the small grid lines in the background, 
#while panel.background refers to the background of the plot itself.
#element_blank() removes whatever part of the plot it is applied to.
...
##element_text() is used to change the appearance of text, like the size, color, or angle.
...
##The file was saved in my working directory. The graph that was saved was the last plot I created,
#which was the faceted histogram of length by scale age.
...
##The dot plot with a trend line is an example of correlation because it shows
#the relationship between two variables. The bar graph shows deviations from a baseline 
#because it compares mean values with error bars. The histogram and boxplot are 
#examples of distributions because they show how the data is spread.
...
ggsave("SpokaneFish_weight_vs_length.png", dpi = 450, width = 9, height = 7)

# QUESTION: Why do you think we focused on Redband Trout for most of this assignment?

#We focused on Redband Trout to simplify the dataset and make patterns easier to 
#see without variation from other species.It also helps focus on one population.
