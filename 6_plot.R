...
I see a blank graph with the x-axis labeled length separated by 100s. I think that there is no data in the graph yet because we have not put the data in the graph.
...
There is data that has been plotted.There is also an y-axis that is labled as count an it is separated by 50s. Also the x-axis still just has 100s without units.
...
The changes that were made is now the y-axis is separated by 100s. This changes the way that the graph looks and now it looks bigger like the bindith changes how wide the graph bars appear.
...
The majority of the fish are about 1 years old and then after that their numbers drop off and none of the fish seem to make it past age 8.
...
This means that there was probably some data missing.I think its is ok to continue because we are just looking at weight right now.
# ---------------------------------------------------------- #
...
The aesthetics in this graph are the x-axis represents the fish’s scale age and the y-axis represents the fish’s length. The graph can help show the relationship between the age and size.
...
The older the fish is the bigger the fish is 
...
The color changes to a really pretty blue. i think without as.factor() tells R to treat ScaleAge as categories and not numerical values.
...
For the boxplots the color control controls the color of the outlines and lines, and the fill control controls the color of the inside of the box.

# QUESTION: Why did we have to specify as.factor() for ScaleAge in the initial
# aes() statement? 
We needed to use as.factor() for SacleAge because R would treat each age as its own category. Without it R treats ScaleAge as a continuous numerical value. Making the graph make a large boxplot instead of liitle small ones.
...
The graph uses a smoothing function for a statistical transformation.Due to the fact that the dataset has over 1,000 observations the default model is porbably a generalized model which creates a smooth curved live that fits that dataset better.
...
The graph outputs show large bea heights because we are plotting each individual weight value and putting them together fro each age.This does not seem correct to me because the graph is showing total combined weight for all fish instead of the average weight. I think this might have happened because the data was not summarized before the graph was created.
...
the stat="identity" tells geom_bar to use the y-values from the dataframe for the heights of teh bars. also in this graph it uses Weight_mean to make a bar for heights for easch ScaleAge group.
...
geom_errorbar(aes(ymin = Weight_mean - Weight_se,
                  ymax = Weight_mean + Weight_se,
                  width = 0.2)) 
adds error to each bar,ymin sets the lower limit,ymax sets the upper limit,width controls the width of small error bars.

# QUESTION: What does the statement width=0.2 do? If you're unsure, try removing
# it and seeing what happens.
The statements tells R how wide the lines should be for the error bars.
...
The difference in this graph is that naow there are dots and lines in this graph.
...
ggplot(redband, aes(x = as.factor(ScaleAge), y = Length)) + 
  ...
  scale_y_log10() +
  xlab("Redband trout ScaleAge (years)") + 
...
element_text() refers to formatting teh text on the graph which is why the worf weight looks so big on the graph.
...
The file showed up in the git tab.Its the last graph created.
...
ggsave("Redband_histogram_facet.png", width=10, height=8, dpi=600, unit="in")
?ggsave
...
I think the dot plot with a trend line shows a correlation, the boxplot shows deviatations from a bench mark or a baseline. Also the histogram shows some distrubution.
...
ggplot(SpokaneFish, aes(x = Length, y = Weight, color=Species)) + 
  ...
  xlab("Length (mm)") + 
  ylab("Weight (g)")
ggsave("SpokaneFish_Weight_vs_Length.png",
       width = 9, height = 7, dpi = 450, units = "in")
# QUESTION: Why do you think we focused on Redband Trout for most of this assignment?
I believe we focused more on Redband trout for this assignemnt because it has the most amount of data we could use.
