...
## An x-axis was created with ticks every 100 units. There is no data, because all we did was create an axis, not a shape to mix the data with.
...
## On top of the data being included in a bar graph format, the y-axis was automatically created with it. I notice that the x-axis is the "Length", while the y-axis is the "count" or amount.
...
## Increasing our "binwidth" has made a more simplified version of our data, by grouping similar data next to each other to merge into one bar graph. So instead of many individual bars, we have eight bars.
...
## The vast majority of Redband Trout are a year old.
...
## "Removed 88 rows containing non-finite outside the scale range (`stat_bin()`)." I think this means that there were 88 observations excluded from the plot because R could not find a way to use them, probably because they were NA values. I think it's okay for the warning to stay, as removing the warning will still give us the same result in our plot. But, I also understand that it would be a better practice to not have to see any warnings.
...
## We are examining the Redband Trout, where the aesthetics are the ScaleAge in the x-axis and Length in the y-axis. The graph will be plotting the age of the trout by the length of the trout.
...
## "geom_smooth" is using the function "stat_smooth" for our statistical transformation fit.
...
## "stat_smooth" was used within "geom_smooth" to create statistical transformations.
...
## The output is a bar graph, where the x-axis is age and y-axis is weight. The weight measurement seems extremely high. It seems like it counted numbers of observations rather than proportions.
...
## "stat = 'identity'" tells ggplot use y-values instead of row numbers.
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
##This makes a plot using the "redbandSummary" data frame, creating aesthetics in the process. This takes "ScaleAge" and turns it into a variable that can be categorized as the x-axis, while also creating "Weight_mean" as the y-axis.
  
  geom_bar(stat='identity') +
## This tells ggplot to use y-values within "redbandSummary" instead of just listing row numbers.
  
  geom_errorbar(aes(ymin=Weight_mean-Weight_se,
## This created error bars on each bar in the plot. This allows the minimum of the error bars to be taken from data in the "weight_mean" and "Weight_se" and subtracting them in that order.

                    ymax=Weight_mean+Weight_se,
## This allows the maximum of the error bars to be taken from data in the "weight_mean" and "Weight_se" and adding them together them in that order.

                    width=0.2))
## This controls the width of the horizontal portions of the error bars.
...
## "width = 0.2" controls the width of the horizontal portions of the error bars.
