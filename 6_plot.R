...
#The x-axis was labeled "length" and given number labels scaled to the data. We
#didn't get any actual data because we only asked R to establish the x-axis itself.
...
#The code generated a histogram based on the length column in our dataframe. The
#label for the x-axis stayed the same as we directly set it this way. The y-axis
#label was set to "count", presumably a count of the number of length
#observations within each number range. 
...
#The number of bars on the histogram decreased, meaning the range of numbers 
#included in each bar increased. 
...
#1-year-old fish were by far the most common. Frequency decreased the older the
#fish got.
...
#I got the warning message "Removed 88 rows containing non-finite outside the 
#scale range (`stat_bin()`).". I think this is fine as I interpret this message
#as saying that the NA values were removed.
...
#The dataframe being examined is the same redband dataframe from earlier. The
#aesthetics are the x and y axes. The resulting graph will be plotting the 
#relationship between scale age and length.
...
#I would conclude that they are directly positively correlated.
...
#The colors themselves were changed from being distinctive to being a gradient.
#This is because, without the "as.factor", the data is considered quantitative
...
#Color, for boxplots, means the color of the outlines of the boxes and the lines
#extending from them. Fill means the color of the inside of the boxes.
...
#Because, as I said before, we want the ScaleAge variable to be considered 
#categorical in this case for increased readability.
...
#The regression lines from geom_smooth are also colored.
...
ggplot(redband, aes(x = as.factor(ScaleAge), y = Length)) + 
  geom_boxplot(fill="purple", color="orange") +
  scale_y_log10() +
