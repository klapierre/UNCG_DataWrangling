...
redband <- read.csv(file = "C:/Users/nsalt/Documents/DataWrangling/UNCG_DataWrangling/LowerSpokaneFish.csv") %>%
...
# The type of graph to create was not specified, such that there's no data
# shown at this point.
...
# A histogram of the lengths was created the lengths are also exact, the 
# x-axis label was pulled from the column we're charting while the y-axis 
# label was autogenerated, assumed to be a count.
...
# It grouped the data into counts per range rather than exact values.
...
# The fish population is heavily skewed towards the young, with the bulk of the
# fish being aged about a year, tapering off heavily after the 1st year.
...
# It seems that it likely removed the NAs from the data set. The fact that the 
# NAs were auto-removed seems alright then, so no alterations would be necessary?
...
# The y-axis is presumably a sum of all weights at that age, rather than a mean
# weight for that age.
...
# It tells R to chart the values to the y-axis.
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + #Tells R what data to use
  geom_bar(stat='identity') + #Tells R what graphic to use and to chart the values on the y-axis
  geom_errorbar(aes(ymin=Weight_mean-Weight_se, #Tells R to add an error bar and set the minimum as the mean - the st. error
                    ymax=Weight_mean+Weight_se, #Tells R to set the top of the error bar at the mean + st. error
                    width=0.2)) #Makes the error bars have a width of 0.2
...
# Sets the dimensions of the error bars.
...
# Here a trend line is created for each age group, rather than across all age 
# groups.
