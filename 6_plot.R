...
#an x-axis labeled length. We have not specified what type of plot we wanted
...
#A histogram. x axis is the same and the y axis now has a label of 'count' automatically
...
#the bars became wider. Each bar now has a width of 50 instead of 30?
...
#most of the population is no more than a few years old
...
#removed 88 rows containing non-finite values. I think its ok, just NA values that cant be plotted.
...
#redband is the dataframe being examined and the aesthetics are ScaleAge on the x axis and Length on the y axis.
#nothing is plotted yet since no geom was specified
...
# ranges of length for different aged fish, showing an upwards trend in length with age, with diminishing increases in later years  
...
#as.factors statement allowed each value of ScaleAge to be its own discrete color rather than without,
#the colors of different ScaleAge blend together on a scale.
...
#color refers to the data points and outline of the boxplot. Fill means the fill color inside each box.
...
#because otherwise the boxplot would just contain a single box for all the data, regardless of age
#as factor makes sure each ScaleAge is a new boxplot
...
ggplot(redband, aes(x = Length, y = Weight)) + 
  geom_point(aes(color=as.factor(ScaleAge))) +
  geom_smooth(formula = y ~ poly(x,2), size=2, color= 'black')
...
#panel.grid.minor are the minor x and y axis lines and element_blank removes the element
ggplot(redband, aes(x = Length, y = Weight)) + 
  geom_point() +
  theme(panel.background = element_blank())
#panel.background is the background that isn't part of grid
...
#properties of a text element,  in this case the y-axis title
...
#in the UNCG_DataWrangling folder set as our working directory. It saved the last plot created (hisogram above)
...
# of a correlation(A)? Which showed deviations from a benchmark or baseline(B)? And which
# was an example of a distribution(C)?
#A. dot plot with trend line. B.boxplot. C.Histogram
...
# weight as a factor vs length, putting length on a log10 scale, points as filled 
...
ggplot(read.csv('LowerSpokaneFish.csv'), aes(x = Weight, y = Length)) + 
  scale_y_log10() +
  geom_point(shape = 17, aes(color=Species)) +
  xlab ('Weight (g)') +
  ylab ('Length (cm)')+
  facet_wrap(~Species)
ggsave("LowerSpokaneFish_ScatterbySpecies.png", width = 9, height = 7, units = "in", dpi= 450)
  
# QUESTION: Why do you think we focused on Redband Trout for most of this assignment?
# A large majority of the data was solely from the redband species. We would not have had much data if we focused on another species.
