...
#Within the plots tab on the right there is a graph. I believe there is no data because we have not synced the file with the graph.
...
#There are now bars on the graph and the y-axis is labeled 'count' and the x-axis is labeled 'length'.
...
#The bars on the graph seemed to get wider.
#When the band width was set to 30 the bars on the graph became thinner.
...
#Out of the population that has been graphed, a lot of the fish are equal to or younger than 2.5 years old.
...
#The warning message says that 88 rows of data were removed and removing the data that is not numerical would get rid of the warning message.
...
#The graph is black instead of color and removing 'as.factor' allowed there to be no color assignment.
...
#Color represents the color of the line in the box and points on the graph.
#Fill represents the color within the box excluding the line inside.
...
#Based on the codes above, as.factor is used for non numerical values.
...
#The generalized additive model is being used
...
#The very first part was generating a histogram.
...
#The graph is showing data based on the correlation between weight and as.factor(ScaleAge). It seems a little off because the code has 'stat=identity' which changed the graph. Something going on with the y-axis.
...
#stat=identity allows the weight_mean values to be used for the bars on the graph.
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  #Creates a graph that has a value for the x and y axis.
  geom_bar(stat='identity') +
  #Means that we are using only the numerical values.
  geom_errorbar(aes(ymin=Weight_mean-Weight_se,
                    ymax=Weight_mean+Weight_se,
                    width=0.2))
#Identifies a minimum and maximum for each bar.
...
#Removing it changes the height in the error bars for each bar on the graph.
...
#The graph looks like a scatterplot now and has more colors incoorporated.
...
#The first code applies a grid behind the data and the second removes the grid background. I think the 'element_blank' function gets rid of whatever theme and makes it blank.
...
#The element_text refers to the text that identifies either of the axis.
