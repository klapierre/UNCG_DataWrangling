...
#The x axis was generated by the code above for the length column. This code
#only makes the aesthetic part of the histogram like the axis, it does not code
#for the actual plotting of data from the length column on the histogram.
...
#The code above generated a histogram with the length of redband on the x axis
#and the count or number of redband of that length on the y axis. This code 
#actually plotted the length data.
...
#This made the histogram bars by length 50 rather than 30 so there are less bars 
...
#The largest amount of the population is age 1 and number decreases as 
#age increases except for age 0.
...
#This means there are NA in the dataset for weight, which I confirmed by looking at
#at this column in the dataset. It is probably not ok to proceed and the NA
#should probably be filtered from the dataset and then the histogram plotted again.
...
#The dataframe being examined is redband. The aesthetics is x= ScaleAge and y=Length
#The resulting graph will be length of each age of redband.
...
#The color of the plotted points differs. This is because as.factor makes the ScaleAge
#categorical with increments of 1 for the age so different colors and when as.factor is not
#in the code ScaleAge is continuous so it goes from light blue with youngest redband
#to dark blue with oldest redband.
...
#color is the outline of the shape of the box while fill is the color inside of 
#the box
...
ggplot(redband, aes(x = (ScaleAge), y = Weight)) + 
  geom_boxplot(color = 'purple', fill = 'green')
#It specified how many boxes would be made or per each age. Without this it is just one 
#box for all ages.
# ---------------------------------------------------------- #
...
#mgcv::gam() is used because it is more than 1000 rows
...
ggplot(redband, aes(x = Length, y = Weight)) + 
  geom_point(aes(color = as.factor(ScaleAge))) + 
  geom_smooth(method = "lm", formula= y ~ poly(x, 2), color= "black", linewidth = 2)
#R gave a warning to change size to linewidth and I did so. Im not sure if this is correct.
...
#The graph is a bar graph that plots weight for each age. However these bars 
#have very large weights with the tallest bar showing a weight of over 80,000. 
#This is likely incorrect as it summed up all of the weights for each age. This 
#is because of the 'identity' part of the code which makes R use the y value listed
#in the column.
...
#Tells R to use to actual values that are in the column rather than adding up 
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + #Tells R
  #to use the redbandSummary dataframe and ech bar is by categroies of scale age
  geom_bar(stat='identity') + #Tells R to use value within the Weight_mean columm
  geom_errorbar(aes(ymin=Weight_mean-Weight_se, #lower bar computed by value in 
                    #Weight_mean column minus value in Weight_se column
                    ymax=Weight_mean+Weight_se, #upper bar computed by adding these
                    #rather than subtracting
                    width=0.2)) #shows width of cap
...
#specifies width of cap on errorbars
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  geom_bar(stat='identity', color= 'lightgreen', fill='darkgreen') +
...
ggplot(redband, aes(x = ScaleAge, y = Length)) + 
  geom_boxplot(fill="darkgreen", color="orange") +
  scale_y_log10() +
