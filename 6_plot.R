...
## A blank plot framework was generated by running the code above
## There is no data because ggplot works in layers
...
## A histogram of fish length for the filtered redband dataset was generated
## The x-axis label is already set to length and the y-axis is already set to
## count
...
## The width of each bin changed to 50 units of length
...
## The figure genereated shows the most Redband Trout are in the mid-length range
## with fwer very small or very large fish. 
...
ggplot(redband, aes(x = Weight)) + 
  ...
  labs(x = "Weight",
       y = "Count",
       title = "Distribution of Redband Trout Weight")
...
## Warning message: Removed 88 rows containing non-finite outside the scale range
## ('stat_bin()').## This means ggplot already chose 30 bins to divide your 
## Weight data into because I didn't tell it how wide the bins should be.
## Yes, it's okay to proceed and the plot will still works correctly. 
# ---------------------------------------------------------- #
#### PART 1.3 GEOMETRIC OBJECT: POINTS                    ####
# ---------------------------------------------------------- #

# QUESTION: In the code below, what is the dataframe being examined?
## The dataframe is redband 
# What are the aesthetics? What will the resulting graph be plotting?
## The aesthetics are scaleAge is mapped as the x-axis and length is mapped as 
## the y-axis.
## It will result in showing nothing 
...
## The scatterplot shows a positive relationship between age and length. As fish
## age increases, their length generally increases as well, indicating that older
## fish tend to be larger than younger fish. 
...
ggplot(redband, aes(x=Length, y = Weight)) +
  ...
  labs(x = "Length (mm)", 
       y = "Weight (g)",
       title = "Length vs. Weight in Redband Trout")
...
## When it was wrapped it was treaed as a categorical variable, so each age 
## appeared as a separate group on the x-axis. 
## When it was removed it was treated as a continuous numeric variable, resulting
## in a continuous x-axis and allowing the relationship between age and length 
## to be shown
...
## Color controls the outline, like the borders of the box. 
## Fill controls the inside color of the box itself. 

# QUESTION: Why did we have to specify as.factor() for ScaleAge in the initial
# aes() statement?
## We used as.factor() because boxplots require a categorial varible to create
## groups
# HINT: Try running the code without that statement, what happens?
## gglpot treats ScaleAge as a continuous numeric variable
...
## For small datasets they use LOESS and for larger datasets use GAM
# HINT: What is the default model type for a dataframe of our size?
## Default model type is LOESS
...
# assignment. ## Grammar of Graphics 
...
## The output comes out as a bar graph; it does seem suspicious to me becasue it
## so high. Maybe what happend was the raw weigh values got stacked. 
...
redbandSummary <- redband %>%
  ...
  summarize(
    ...
    mutate( 
      Weight_se = 1.96 * (Weight_sd / sqrt(n))
      ) %>%
      ungroup()
...
## stat='identity' tells geom_bar() to use the actual y values provided instead
## of counting
...
# Start a ggplot using the dataframe ' redbandSummary' 
# aes() sets the " aesthetics": 
# x-axis is the ScaleAge variable, convert to a factor 
# y-axis is the Weight_mean variable, determine bar heights
# Add a bar graphy layer 
# stat = 'identity' tells ggplot to use the y-vales from the data (Weight_mean)
## instead of counting rows 
# Add error bars on top of the bars
# ymin shows the bottom of the error bar 
# ymax shows the top of the error bar
# width shows the width of the horizontal cap at the end of the error bars

# QUESTION: What does the statement width=0.2 do? If you're unsure, try removing
# it and seeing what happens.
# controls the horizontal width of the error bar caps; if you remove them then
# ggplot uses a default width, so the caps will be wider and may overlap with 
# other data. 
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  geom_bar(stat='identity', fill = "light green", color = "dark green") +
  geom_errorbar(aes(ymin=Weight_mean-Weight_se,
                    ymax=Weight_mean+Weight_se,
                    color = "dark orange",
                    width=0.2))
...
# it went to a scatterplot diagram with linear lines 
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Length )) +
  geom_boxplot(fill = "sky blue", color = "purple") +
  scale_y_log10() +
  labs(x = "Scale Age (years)", y = "Length (mm)")
...
# panel.grid.minor controls the minor grid lines in the plot; element_blank()
## removes them completely
# panel.background controls the rectangle behind the plot; element_blank() makes
## it black (no color or fill) 
...
## controls the size, color, front face, angle, and more
...
# it shows up in my files as "Redband_histogram_facet.png" 
# it contains all the histogram panels, with the x-axis fixed and the y-axis 
# varying by panel 
...
# Dot plot with a trend line is an example of correlation
# Bar graph showed deviations from a benchmark or baseline 
# Histogram is an example of distribution
...
library(tidyverse)
LowerSpokaneFish <- read_csv("LowerSpokaneFish.csv")
fish_plot <- ggplot(LowerSpokaneFish, aes( x= Length, y = Weight,
                                           color = Species)) +
  geom_point(shape = 17, size = 3, fill = "white") +
  scale_y_log10() + 
  facet_wrap(~ Species, scales = "free") +
  labs(x = "Length (mm)", y = "Weight (g)", 
       title = "Weight vs Length for Spokane Fish by Species") + 
  theme_minimal() +
  theme(legend.position = "none")

fish_plot
ggsave("SpokaneFish_Weight_vs_Length_by_Species.png",
       plot = fish_plot,
       width = 9,
       height = 7,
       dpi = 450)
# QUESTION: Why do you think we focused on Redband Trout for most of this assignment?
# Maybe to simplify the analysis, consistency in biological traits, or ecological
# relevance 
