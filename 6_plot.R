...
## It only shows the x axis without any data. There's no data due to not having
## a variable to compare to the x-variable.
...
## It generated a histogram of the frequency of each length. The x-axis
## is labeled as length while the y-axis is labeled as count.
...
## It made the width of each section 50 units instead of 30, resulting
## in wider/broader bars with less specificity.
...
## Most of them don't live longer than 6 years.
...
## It likely means some values were NA and should be fine to leave as is.
...
## The dataframe is redband, the aesthetics are ScaleAge plotted against Length,
## and the graph will be plotted with ScaleAge as the x-axis and the Length as the y-axis.
...
## There's a positive correlation between fish age and length.
...
## It made the colors a blue gradient instead of a unique color for each age.
## This is likely due to the blue gradient being a default.
...
## Color is the color of the outline of all the lines while fill is the color of the boxes.

# QUESTION: Why did we have to specify as.factor() for ScaleAge in the initial
# aes() statement? 
## Without it, it only makes one box rather than multiple.
...
## The function used is a scatter plot with a trend line included.
...
## Histograms were used to generate a graph of the distribution for each value of
## a given variable.
...
ggplot(redband, aes(x=Length, y=Weight))+
  geom_point(aes(colour = as.factor(ScaleAge)))+
...
## It plots the data as is rather than performing other calculations first.
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  ## Creates the plot with the x-axis being the ScaleAge and y-axis being Weight_mean.
  geom_bar(stat='identity') +
  ## Makes the plot a bar graph without the default calculations.
  geom_errorbar(aes(ymin=Weight_mean-Weight_se,
                    ## Sets the minimum for the error bars.
                    ymax=Weight_mean+Weight_se,
                    width=0.2))
## Sets the maximum for the error bars.
...
## Sets the width for the error bars.
...
ggplot(redband, aes(x = ScaleAge, y = Length))+
  geom_boxplot(fill = "pink", color = "yellow")+
  scale_y_log10()+
...
## The bar and dot plots showed correlation, the boxplot showed deviations, and 
## the histogram showed a distribution.
...
ggplot(SpokaneFish, aes(x = Weight, y = Length))+ 
  geom_point(shape = 17, aes(colour = as.factor(Species)))+ 
  facet_wrap(~Species)+
  scale_y_log10()+
  xlab("Weight (g)")+
  ylab("Length (mm)")
ggsave("Lower Spokane Fish Full Graph.png", width = 9, height = 7, units = "in", dpi = 450)
# QUESTION: Why do you think we focused on Redband Trout for most of this assignment?
## To avoid putting too many variables on one graph.
