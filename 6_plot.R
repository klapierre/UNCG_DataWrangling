...
#ANSWER: The x axis was generated. There is no data because we did not specify the y axis yet. 
...
#ANSWER: A histogram was generated that is showing us the frequencies of the different lengths of fish. The x axis is the length and the y axis is the "count" or how many fish are that length. 
...
#ANSWER: The bins or the bars got wider and there are less of them because they are spanning across 50 length units on the x axis. 
...
ggplot(redband, aes(x = ScaleAge)) + 
  geom_histogram(binwidth=1, fill = "lightpink4")
...
#ANSWER: The majority of the fish that were sampled were only 1 year old, then there is a sharp decline in sampled fish that are older than 1. 
...
#ANSWER: The warning message: Removed 88 rows containing non-finite outside the scale range (`stat_bin()`). It means that there are 88 data points that are NAs. If you are expecting NAs to be present then it is probably okay. But if you are not expecting NAs then you should go back and check.
...
#ANSWER: the dataframe is redband. The aesthetics are the x and y axis. We are plotting ScaleAge of the fish on the x axis and fish length on the y axis. 
...
#ANSWER: As fish age increases, so does fish length. Until it starts to plateau at age 4.
...
#ANSWER: The plot with the as.factor() changed the color of the the data point based on how old the scale was (each year was a different color). This new plot where we removed as.factor the scale age is a gradient. Because as.factor counts it as a categorical variable and without it, the scale age is considered a continuous variable.  
...
#ANSWER: Color changes the outline of the boxplots. While "fill" fills the boxplots with color. 
...
ggplot(redband, y = Weight) + 
  geom_boxplot(color = 'purple', fill = 'green')

#ANSWER: without it in the initial aes statement, you get an error. the boxplot object requires the the categoreical variable to be defined on the x axis (which is set up in the initial aes)
...
?geom_smooth
#ANSWER: a GAM function is being used? Confused because it only mentions df fewer than 1000 observations and we have slightly more than that. 
