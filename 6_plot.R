...
#Answer: What was generated is a blank graph, with an X-axis labeled length and the axis is marked by 100s. There are no units and no data points. I believe there is no data becuase we have not pulled the data into the graph yet.
...
#Answer: Now there is data plotted, the x-axis is still length by 100s with no units, and now there is a y-axis labled as count measured by 50s, becuase it is just count there are no units but I would assume that it is count of fish.
...
#Answer: What changed is that now the y-axis is by 100s instead of 50's so the graph looks a bit more zoomed in, and fills the graph better. it seems like bindith effects how wide the bars are.
...
#Answer: A majority of the fish are around 1 year in age, and past 1 the number in the population per year decreases a lot, with none living past 7. I also notice that there are very very few that are under 1 year of age. I am not really sure why that is
...
#Answer: The warning message states "Removed 88 rows containing non-finite outside the scale range ('stat_bin()'). Okay so I think that this means that there might be some data missing and so that was not included.I think that with weight it is okay to proceed, but if we were talking about survival you might want to include that some were not found.
...
#Answer: So this is a bar graph, with the age as 0-7 like all of the other graphs that we have created, but the weight is by 20,000 which is definitly odd. I wonder if the unit is really small.
...
#Answer: Stat tells the code how to determine the height of the bars. When using identity it is telling the code to use the numbers you put in Y to determine the heightwhereas if you use count it would count how many of each value to determine the height of the bar. 
?geom_bar
...
#ggplot makes a graph using the redbandSummary data set, with the x axis as Scale age that is use as factors, and the the y axis is the Weight_mean
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
#the graph will be a bar graph with the bar height determined by the weight_mean column
  geom_bar(stat='identity') +
#The error bars with have the ymin as the weight_mean value minus the Weight_se value, and the ymax as the Weight_mean plus the Weight_se values, and then the width of the lines that make the error bars is 0.2
...
#Answer: Completed, see above


# QUESTION: What does the statement width=0.2 do? If you're unsure, try removing
# it and seeing what happens.
#Answer: The statement width=0.2 tells the code how thick the lines that make the error bars should be.
...
ggplot(redband, aes(x = as.factor(ScaleAge), y = Length)) + 
  geom_boxplot(color="deeppink",fill="olivedrab4") +
  scale_y_log10() +
...
#Answer: It appears that panel.grid.minor applies a thin grid background to the graph, whereas panel.background has no lines. Then the element blank(), I think is the default for the theme factor that is attatched to it, or is just nothing that is applied.
...
#Answer: element_text refers to the titles in the graph, in the code above it was assigned to the y axis making it HUGE text.
...
#Answer: It showed up in the files, and it was the last graph that I had made, the one of lengths faceted by scale age.
...
#Answer: An example of a deviation would be the boxplot, and an example of a distribution woould be the histogram, and correlation would be the dotplot.
...
#Answer: Because there is significantly more data for the Redband Trout than any of the other fish.
