...
# included in the output?
    # It created an x-axis and named it 'Length', there's no data because the command was not told to plot data.
...
# about the labels for the x- and y-axes?
    # a histogram was generated plotting the number of entries for each length bracket, defaulted to a bin width of 30. The x-axis has the name given in the code, while the y-axis has a generic "count" label.
...
# QUESTION: What changed about our output when we asked for binwidth=50?
    ...
    # the width of each bin was increased, so there are now fewer bins, and the 'count' value for most bins is higherince more entries fit into them.
...
# about the Redband Trout population based on the figure you generated?
    ...
    # the majority of fish are scale age 1, or at least a plurality of fish are scale age 1
...
# as.factor() statement and the graph where you removed as.factor()? Why?
    ...
    # in the graph that had the as.factor statement, the points were assigned distinct hues of color based on each age, and age is treated as a discrete value. In the second graph, age is treated as continuous and the points are colored along a gradient to reflect this.
...
# QUESTION: What does color mean for boxplots? What does fill mean for boxplots?
    ...
    # color means the boarder color, and fill means thhe color inside the shape's outline
...
ggplot(redband, aes(x = ScaleAge, y = Weight)) + 
  geom_boxplot(color = 'purple', fill = 'green')
    ...
    # without specifying scale age as a factor, it becomes a continuous value and creates one really wide box
...
# HINT: What is the default model type for a dataframe of our size?
    ...
    # because we have fewer than 1000 points, geom_smooth() is using stats::loess() for the smoothing function
...
# right to you? What do you think happened to result in this graph?
    ...
    # the x-axis is named strangl. The values for weight also go insanely high, I would guess its summing the observations to give total weight.
...
# geom_bar() help or google to find the answer.
    ...
    # stat="identity" tells the bars to use the identity of the y-axis value for bar height rather than default to count for bar height
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  # names the data used and sets up the x- and y-axis
  geom_bar(stat='identity') +
  # creates bars, using Weight_mean for the height of the bars
  geom_errorbar(aes(ymin=Weight_mean-Weight_se,
                    # sets the point for the bottom of the error bar to be the value for Weight_mean - the value for Weight_se
                    ymax=Weight_mean+Weight_se,
                    # sets the point for the top of the error bar to be the value for Weight_mean + the value for Weight_se
                    width=0.2))
...
# it and seeing what happens.
    ...
    # the width=0.2 statement changes the width of the top and bottom cross bars on the error bars, making them much smaller than default. I guess its .2 of the width of the bar in the bar chart, as setting width=1 makes the cross bars on thhe error bars equal in width to the graph bars.
...
# QUESTION: What is different about this graph from before?
    ...
    # color is applied to both the points and the lines using the same function
...
ggplot(redband, aes(x = as.factor(ScaleAge), y = Length)) +
  geom_boxplot(fill = "orchid3", color = "darkolivegreen4") +
  scale_y_log10() +
