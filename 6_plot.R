...
# when running the code above, we generated a blank graph. There is no data included in the 
# plot because we did not add a geometry layer. 
...
# A histogram was generated by running the code above. I noticed that the x-axis is labeled length 
# and the y-axis is labeled count. 
...
# The data frame that is being examined is redband. The aesthetics are x-axis is ScaleAge
# and y-axis is Length. The resulting graph will plot ScaleAge on the x-axis and Length for
# the y-axis. There is no data because we are missing the geometry layer. 
...
# When a fish gets older, its length also increases. 
...
ggplot(redband, aes(x = as.factor(ScaleAge), y = Length)) + 
  geom_boxplot(fill = "pink", color = "orange") + 
  scale_y_log10() + 
...
# Based on the output, panel.grid.minor refers to the minor grid lines in the graph, 
# element blank removes them. Panel. background refers to the background of the graph, 
# element blank removed the background fill. 
...
# element_text refers to the appearance of the text. The code above specifies the y-axis text. 
...
# The file showed up in the git tab and in the current working directory. 
# The graph is "Redband_histogram_facet", the one we created at the end of part 1.10. 
...
# The figure that was an example of correlation was the dot plot with a trend line. 
# The figure that showed deviations from a benchmark or baseline was the bar graph with errors. 
# The figure that was an example of distribution was the histogram and box plot. 
...
plot_species <- ggplot(spokaneFish, aes(x = Length, y = Weight)) +
  geom_point(aes(color = Species), shape = 17, size = 3) +
  scale_x_log10() +
  facet_wrap(~ Species) +
  labs(x = "Length (mm)", y = "Weight (g)", color = "Species") +
  theme_minimal()
