...
There is an aesthetic shown with no data. There has not yet been any data entered, so there is nothing to display
...
A histogram was generated. The x-axis has length of fish, and the y-axis has number of fish found of that length.
...
The width of the histogram bars increased, and te data became much more generalized.
...
# dataframe.
ggplot(redband, aes(x=ScaleAge))+geom_histogram(binwidth = 1)
...
The most abundant portion of the population are those fish with scale ages of 1-2 years
...
ggplot(redband, aes(x=Weight))+geom_histogram(binwidth = 20)
...
Removed 88 rows containing non-finite values. There are likely NA values in the dataframe. Alterations should be made to get rid of the warning.
...
The redband dataframe is being examined. Scale age and length will eb plotted on the x and y, but there is no data to plot.
...
As fish get older, they lengthen until about 2-3 years of age.

# TASK: Write your own code to visualize the relationship between Redband 
# length and weight.
ggplot(redband, aes(x=Length, y=Weight)) + geom_col()
...
The as.factor statement produced a rainbow scheme that was easily noticable. The second function makes everything a shade gradient of blue.
...
Color in this boxplot determines color of boxplot borders and outliers. Fill determines color of the inside of the boxplots.

# QUESTION: Why did we have to specify as.factor() for ScaleAge in the initial
# aes() statement?
as.factor ensures that the analysis is by age, not across all individuals
...
A dot plot with a trendline would be a good example of a correlation. A boxplot is a good example of deviations from a baseline. A histogram and a bar graph can show distributions.
...
LSF<-read.csv("LowerSpokaneFish.csv")

ggplot(LSF, aes(x= Weight, y=Length)) + 
...
They are likely a population that is well studied and documented
