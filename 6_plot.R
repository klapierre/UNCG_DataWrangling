...
##ANSWER: A plot with lengths as the x axis. There is no data because we haven't specified how we want the data displayed yet.  
...
##ANSWER: A histogram for our fata was generated in the plot; the x axis remained the same while values and the "count" label were automatically included for the y axis. 
...
##ANSWER: The histogram was reorganized so that each bar had a wider range of numbers, leading to less bars overall for the same set of data. 
...
##ANSWER: I notice that the age of the population is skewed right, so the average age is on the younger side. 
...
##ANSWER: The warning message was "Removed 88 rows containing non-finite outside the scale range (`stat_bin()`)." It means that 88 rows were not included in the plot due to their non-numerical values. I think it is okay to have this warning so long as the dataset is large enough to have meaningful results without those rows. 
...
##ANSWER: There is more color in the as.factor statement while the statement without as.factor is a blue gradient. This is because in the plot with the as.factor statement, each scale age value gets its own color, whereas without the as.factor statement scale age is interpreted as a continuous (non-discrete) value. 
...
##ANSWER: Color refers to the outline color of the boxplots and their lines and points. Fill refers to the color within the boxes themselves. 
...
##ANSWER: Without the as.factor statement, the ScaleAge data is continuous, so there is one large box for all of the ScaleAge data. The as.factor statement is needed to create a boxplot for each age point.  
...
##ANSWER: It is a quadratic model.
...
##ANSWER: The output is a bar graph of ScaleAge and Weight. The y axis values are very high, this is likely because the boxes are a sum of the weights of all of the fish of one age group rather than the average weight. 
...
redbandSummary <- redband %>% group_by(ScaleAge) %>% 
  summarize(Weight_mean = mean(Weight, na.rm = TRUE), Weight_sd = sd(Weight, na.rm = TRUE)) %>% 
              mutate(Weight_se = 1.96 * Weight_sd) %>% ungroup()
...
##ANSWER: The "stat = 'identity'" line tells R to use Weight_mean values for the y axis rather than plotting frequency.
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  # Uses the ggplot function to make a plot from the redbandSummary dataframe with
  # the x axis plotting ScaleAge as a discrete number and the y axis plotting mean weight. 
  geom_bar(stat='identity') +
  # Tells R to use mean weight as y axis values. 
  geom_errorbar(aes(ymin=Weight_mean-Weight_se,
                    # Tells R to make error bars with the minimum number as
                    # mean weight - standard deviation of weights.
                    ymax=Weight_mean+Weight_se,
                    # Tells R to make error bars with the maximum number as
                    # mean weight + standard deviation of weights.
                    width=0.2))
                    # Specifies the width of the error bars. 
...
##ANSWER: It specifies the width of the error bars, without the width function the error bars are as wide as the bars themselves. 
...
ggplot(redband, aes(x = as.factor(ScaleAge), y = Length)) + 
  geom_boxplot(fill = 'skyblue', color = 'olivedrab') + scale_y_log10() + 
...
#ANSWER: I think that panel.grid.minor refers to the grid lines while panel.background refers to the area between the grid lines (the "boxes"). element_blank() makes the aesthetic aspect completely blank. 
...
##ANSWER: element_text() specifies changes to the text element of an aesthetic portion of the plot. 
