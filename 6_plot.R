...
# A blank plot with Length along the x-axis. No points were generated as 
# we didn't tell R where to pull data from
...
# Length is obvs. still the X and the Y axis is the count for each
# length value.
...
# This sort of "smooths" out the histogram
...
# Most fish seem to be 1 year when caught.
...
# There are some NA values in the weight column. It seems that R
# is ripping them out before it creates the graph though?
# You could create a subset that removes all NA values and remake the graph,
# but it would juts look the same?
...
# Fish get longer as they get older.
...
# We went from distinct factors (the individual ages) being listed as distinct colors to
# the age being shown as a gradient of a single color
...
# Color seems to be the outline and fill is what fills it. 
...
# It takes it as the mean for all scale ages and weights. Gotta tell R where
...
ggplot(redband, aes(x=Length, y=Weight)) + 
  geom_point(aes(color = as.factor(ScaleAge))) +
  geom_smooth(formula = y ~ poly(x,2), col = "black", size = 2)
...
# It's looking pretty funky. R is trying to coerce weight into some
# specific threshold? 
...
redbandSummary <- redband %>% group_by(ScaleAge) %>% 
  summarize(across(.cols=c('Weight'), .fns = list(mean=mean, sd = sd), na.rm = T)) %>% 
...
# With stat = identity you tell R to skip aggregating the values yourself and just the values we give it.

# The above code gave us nice bars.  Now we need to add error bars! We will do this
# by adding in a second geometric object that specifies error bars. Try it by
# running the following code:

ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  geom_bar(stat='identity') + #basically same as above 
  geom_errorbar(aes(ymin=Weight_mean-Weight_se,
                    ymax=Weight_mean+Weight_se,
                    width=0.5)) # add errorbars ymin and ymax are defined here.
...
# Makes those flat lines at the top and bottom of error bars larger 
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  geom_bar(stat='identity', col = "darkgreen", fill = "lightgreen") +
  geom_errorbar(aes(ymin=Weight_mean-Weight_se,
                    ymax=Weight_mean+Weight_se,
                    width=0.4), col = "darkorange")
...
# The aes is applied to everything after so the points and the geom_smooth inheirit the
# color schemes
...
ggplot(redband, aes(x = as.factor(ScaleAge), y = Length)) + 
  geom_boxplot(size=1, col = "magenta", fill = "darkgreen", shape=1) +
  scale_y_log10() +
...
# The background on which our graph is drawn. Element_blank draws on a pre-set?
...
# Altered the Y element text size. (very large)
...
#In the WD. It was the last plot we made.
...
ggsave("Redband_histogram_facet.png",plot = last_plot(), dpi = 600, width = 10, height = 8, units = "in")
...
# A histogram can show a distribution
# A correlation can be shown with a scatter plot and trend line
# A boxplot could show deviation. (Thanks to those error bars)
...
ggsave("ScaleAgeVsLengthBySpecies.png",plot = last_plot(), dpi = 450, width = 9, height = 7, units = "in")
