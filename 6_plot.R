...
#A blank graph was generated with the x-axis labeled 'length'. Also, a range of 100-400 along the x-axis. No data is included in the output because I did not specify what kind of object graph, observations, and values to use.
...
#A histogram was generated showing multiple bars different across the graph based on 'redband'data. The x-axis label is 'Length' with a range of 100-400. The y-axis label is 'count' with a range of 0-150.
...
#The width of the bars look wider and fewer compared to the previous plot.
...
#Based on the figure I generated, the Redband Trout population is mostly a year old. Less of the trout population is above four years old.
...
#I recieved the following warning message: "Removed 88 rows containing non-finite outside the scale range ('stat_bin()')" .I think this message means 88 rows were removed because the values did not fall within range or contain 'NA'.I think I should alter the code to remove any NA's listed because it can affect the interpretation of the graph.
ggplot(redband,aes(x=Weight))+geom_histogram(na.rm=TRUE)
# ---------------------------------------------------------- #
...
ggplot(redband, aes(x=ScaleAge, y=Length))
#The 'redband' dataframe is being examined. The aesthetics are an x-axis label 'ScaleAge' and y-axis labeled 'Length'. The plot will not include any output because I did not include which geometric object I will be using. Both axis should be labeled with their proper ranges.
...
#A point plot was generated with 'ScaleAge' on the x-axis and 'Length' on the y-axis. I conclude that there is a common length for each scale age group. ScaleAge of 3 for example, most trout lengths fall in the range between 340-400 with few outliers.
...
#The graph with the as.factor statement has eight different colors (mainly the prime rainbow colors). The graph without as.factor has different shades of blue as the color scheme. The as.factor converts the 'ScaleAge' column as a factor, therefore the inputs are factors.
...
#For boxplots, color is for the lines and points of a box, and a fill is the color within each box.
...
ggplot(redband, aes(x =ScaleAge, y = Weight)) + 
  geom_boxplot(color = 'purple', fill = 'green')
#The as.factor function converts 'ScaleAge' into a factor, therefore it's inputs are treated as categories and have a 'point' in the 'box'. Without this function, there's just one box since ScaleAge has no categories.
# ---------------------------------------------------------- #
...
?geom_smooth
#The geom_smooth or Smoothed conditional means function is being used. The stat='smooth' aids the eye in seeing patterns in the presence of overplotting.For observations greater than 1000, the default method used is the 'Linear Model'. 'geom_smooth' fits a straight line (linear regression) to the data. 
...
#A bar graph for weight based on ScaleAge. The x-range is 0 to 7. The y-range is 0 to over 80,000. The scale of the y-axis does not seem right because there are inputs less than 1000 in the 'Weight' column. I think the values for 'weight' were added for each ScaleAge. 
...
redbandSummary<-redband %>% 
  group_by(ScaleAge) %>% 
  summarise(Weight_mean=mean(Weight,na.rm=TRUE),Weight_sd=sd(Weight,na.rm=TRUE),n=n()) %>% 
  mutate(Weight_se=Weight_sd/sqrt(n)) %>% 
  ungroup()
...
#stat='identity' tells ggplot to use the actual values from the dataset and do no calculations. 
...
#The first line takes the 'redbandSummary' dataframe and plots ScaleAge vs Weight_mean. The next line tells ggplot to use the actual values from the dataset and do no calculations. The third line codes for the error bars. The top and bottom lines of the error bars will have a width of 0.2. Weight_mean+Weight_se calculates the y-max of the error bar. Weight_mean-Weight_se calculates the y-min of the error bar.  
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  ...
  geom_errorbar(aes(ymin=Weight_mean-Weight_se,
                    ymax=Weight_mean+Weight_se,color="darkorange",
                    width=0.4))
...
#In this graph, each ScaleAge value has a 'smooth' line along with a specific color.
...
#A dot plot with a trend line is an example of correlation. A histogram is an example of distribution. Boxplots and bar graphs with error bars are examples of deviations from a benchmark or baseline.
...
SpokaneFishSummary<-ggplot(SpokaneFish, aes(x=Length, y=Weight,color=Species))+
  geom_point(shape=17)+
  scale_x_log10()+
  facet_wrap(~Species)+
  labs(x="Fish Length (mm,log10 scale)",
       y="Fish Weight (g)",
       color="Species")+
  theme_bw()
ggsave("SpokaneFish_weight_length_by_species.png",
       plot=SpokaneFishSummary,
       width=9,
       height=7,
       units="in",
       dpi=450)
# QUESTION: Why do you think we focused on Redband Trout for most of this assignment?
#I think we focused on Redband Trout for most of this assignment because there was a lot of data and connections that could pull from the dataframe, for this single species.
