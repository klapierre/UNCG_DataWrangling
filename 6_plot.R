...
# This is a skeleton for the histogram we wish to build with the basics of our dataset,
# But, it is only dependent upon aesthetic purposes and does not use any of the actual data.
...
# The code above still has Length as the x-axis, but because we specified that we wished to create a histogram
# The y-axis was labeled as "count", for the amount counted within each length.
...
# This increased the band width on the histogram.
...
# The Redband trout in this dataset are mostly between years 1-3, with very few older or younger than 1-3 years.
...
# 88 rows were removed which contained non-finite values. This means that R is confused by the NA values,
# While it may be safe to continue, we would likely want to write code which ignores any NA's within the dataframe.
...
# This graph is trying to compare ScaleAge and Length, however there would be no generated graph, 
# Because a graph type was not specified. 
...
#According to the graph generated, typically we see the older fishes are longer in length.
...
# The as.factor() statement treats ScaleAge as a factor to base the graph colors off of.
# Without this factor, the graph merely fills in all the spaces as black. 
...
# Color indicates the outline, while fill indicates what color the boxes will be filled with.
...
# Instead of using ScaleAge as a factor for multiple variables, it is considered as a single variable to be used along the x-axis
...
# This has output a bar graph. The scale on the y-axis is way out of proportion to the actual data.
# It almost seems as if the weight measurements were multiplied by the number of observations? Not sure!
...
redbandSummary <- redband %>%
  group_by(ScaleAge) %>% 
  summarize(across(.cols="Weight", .fns=list(mean=mean, sd=sd), na.rm=T)) %>% 
...
# In a regular geom_bar function, stat = 'count', which essentially inputs a standard x axis value,
# but the "identity" specifically tells the function to sum the y values by the x variables.
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + #This sets a variable to the x and y axises
  geom_bar(stat='identity') + # This sets the y values
  geom_errorbar(aes(ymin=Weight_mean-Weight_se, # The minimum of the error bar is the mean - standard error
                    ymax=Weight_mean+Weight_se, # The maximum of the error bar is the mean + standard error
                    width=0.2)) # The bars have a width of 0.2
...
# It indicates how wide the error bar caps are
...
ggplot(redband, aes(x = as.factor(ScaleAge), y = Length)) +
  geom_boxplot(fill = "skyblue", color = "purple") +
  scale_y_log10() +
