...
##Blank plot with only fish length as x axis but no data points or bars have been added yet.
...
##Trout data bars are shown. X axis shows fish lengths and the y axis shows the count.
...
##More younger fish and less older fish.
...
##Warning message says rows were removed. This means that rows with invalid or NA were ignored.
#Ok to go ahead - no impact on overall analysis.
...
##Instead of colors being grouped by age shows up as a gradient.
...
##Color is for boxplot outline, Fill is for inside color of the boxes.
...
##Specified so each age group is a separate group on the x axis instead of continuous. 
...
##Smoothing curve - fits curved line to data.
...
##Raw weight values are being shown. This is not right because the data is not summarized.
#I think all individual observations are being stacked causing large bars.
...
##Tells ggplot to use actual values in the dataset instead of calculating summaries automatically.
...
# running the following code:
 ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + ##sets up plot with x and y axis
  geom_bar(stat='identity') + ##create bars using mean weight data
  geom_errorbar(aes(ymin=Weight_mean-Weight_se, ##error bars added
...
##Saved in working directory. Saved most recent plot made.
