...
#it generated a graph that has the x of Length going from 100-400
#there is no data becaues we didn't indicate what kind of graph we wanted
...
#we got a histogram of the data, and now there is a y axis of "count'
...
#I think it just essentially smoothed out our data
#it indicated the size of the bins - the bars
...
#there seems to be a large proportion of fish that are 1 year old with very few that live past 4
...
#my warning was "removed 88 rows containing non-finite values (stat_bin)
#I think that maybe there were some rows that did not have numerical values
#I mean, you should probably fix it, but it doesn't seem detrimental - maybe go through and see if there are variables missing
...
#the dataframe is redband, the aes() is having ScaleAge on the x and the Length on the y 
#we will see the data of ScaleAge later
...
#the fish continue to growth until they are around four years old when it starts to plateau
...
#range of blues instead of rainbow and a different legend
#it had been originally coloring as a factor of ScaleAge
...
#color means the outlines and fill means the insides
...
#otherwise it scales it by something else, and we wanted to see the color for the age
...
#the output is a bar graph
#the scale of the y does not seem correct
#I am unsure what happened with the y, maybe it used a different scale or it just automatically plotted something, or maybe used the number of fish rather than their weights together
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + #uses ggplot and redbandSummary data, and then specifies x is set as a facotr of ScaleAge with the y being the weight_mean column
  geom_bar(stat='identity') + #states we want a bar graph and we want to leave the data as it is
  geom_errorbar(aes(ymin=Weight_mean-Weight_se, #adds error bars with min at mean-se
                    ymax=Weight_mean+Weight_se, #error bars with max as mean+se
                    width=0.2)) #the width of the error bars is .2
...
#sets the width of the error bars
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
  geom_bar(color = 'dark green', fill = 'light green', stat='identity') +
...
#there's suddenly a lot more lines - for each of the ages
