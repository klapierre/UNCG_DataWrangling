...
# It just gave the X-axis coordinate line and few numbers on the line with Length
# as the X-axis label. It did not include any output because we have not instructed
# R about the format we want the output to be in.
...
# A histogram. The x-axis was what we had specified in the code, but the y-axis
# automatically appeared as count.
...
# We saw larger proportion of data was put together in one unit compared to the 
# previous code. Previously we were visualizing data within 30-unit range but 
# now we are visualizing it within 50-unit range.
...
# Majority of the fish in the Redband Trout population were around 1 year.There
# were very few that were 5 years old or higher.
...
# QUESTION: In the code below, what is the dataframe being examined? reband

# What are the aesthetics? What will the resulting graph be plotting?
# aesthetics: X-axis will have ScaleAge while y-axis will have Length.
# Since we have not specified what kind of output we want, the R will not
# provide any output.
...
# Generally the length of fish increases with age.
