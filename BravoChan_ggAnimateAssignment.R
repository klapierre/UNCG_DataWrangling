...
ggplot(iris, aes(x = Species, y = Petal.Length)) + #command to plot a graph using the iris dataset, by species, using the distribution of petal length
  geom_boxplot(aes(color = Species))+ #command to use different colors based on species
  labs(x = "Species", y = "Petal Length") #labels for the x and y axis
...
ggplot(iris, aes(x = Species, y = Petal.Length)) + #command to plot a graph using the iris dataset, by species, using the distribution of petal length
  geom_boxplot(aes(colour = Species)) + #command to use different colors based on species
  transition_states(Petal.Width, #commands marking the beginning of animation rendering using the variable for petal width
                    transition_length = 1, #duration of transitions between snapshots
                    state_length = 1) #duration of the pause in each snapshot
...
#Labels were previously added. Now on to title.

ggplot(iris, aes(x = Species, y = Petal.Length)) + 
  geom_boxplot(aes(colour = Species)) +
  labs(x = "Species", y = "Petal Length",
       title="Petal Length Variation") +
  transition_states(Petal.Width,
                    transition_length = 3,
                    state_length = 3)
...
AnimGraph <- ggplot(iris, aes(x = Species, y = Petal.Length)) + 
  geom_boxplot(aes(colour = Species)) +
  enter_appear(early=TRUE) +
  transition_states(Petal.Width,
                    transition_length = 3,
...
AnimGraphVarSave <- animate(AnimGraph, renderer=gifski_renderer())
...
AnimGraphSave <- anim_save('petals.gif', animation=last_animation(), path=NULL)
...
ggplot(ChickWeight, aes(weight, Chick)) + #command to render a scatterplot of each chick and its weight progression under specific diets
  geom_point(aes(color=Diet)) + #specify different colors for each type of diet
labs(title = 'Days: {frame_time}', x = 'Chick weight (grams)', y = 'Chick number') + #delineates the labels for each variable 
  transition_states(Time,  transition_length = 1, #determines the duration of each transition
                    state_length = 1) + 
  ease_aes('linear') #determines how fast the variables will move and what path they will make
...
AnimChickGraphSave <- anim_save('chicks.gif', animation=last_animation(), path=NULL)
...
ChicksBar <- ggplot(ChickWeight, aes(x=weight, y=Time, fill=Diet))+ 
...
AnimChickBarGraphSave <- anim_save('chicksBar.gif', animation=last_animation(), path=NULL)
...
#So you can see trends in your data. Anomalies would be a great indicator of variables incorrectly grouped, or data frames incorrectly joined.
...
#It would be interesting to visualize trendlines overtime. Also, violin graphs when dealing with big datasets that has large variance over time.
...
#To communicate changes over time. 


#Now for graph 2!We're going to do a bar graph!

#Let's start by making a simple bar graph
bar_graph <- ggplot(orange, aes(age, circumference, fill=as.factor(Tree))) + #command to plot a bargraph, using age for its x-axis, circumference for the y-axis, colored by identity
  geom_bar(stat="identity", position=position_dodge()) + #avoid overlap of objects that are side-by-side
  scale_fill_brewer(palette = "Accent") #uses a specific palette to fill in the graph
bar_graph #displays the graph

#TASK: Annotate each line on what it does for the graph
#Done!
#TASK: Add titles! Change the width! Flip the axis! Cause chaos!!!
bar_graph <- ggplot(orange, aes(circumference, age, fill=as.factor(Tree))) +
  geom_bar(stat="identity", position=position_dodge()) + 
  scale_fill_brewer(palette = "Accent") + 
  labs(x = "Circumference (mm)", y ="Age (Days)",title="Circumference Growth on Orange Trees") 
bar_graph
...
#Question: When would this type of graph and animation be an appropriate way to communicate your data?
#This is an interactive way to show the progressive change of a group of individuals/observations
...
racebar_graph <- ggplot(orange, aes(circumference, age, fill=as.factor(Tree))) +
  geom_bar(stat="identity", color="black", position=position_dodge()) + 
  scale_fill_brewer(palette = "Dark2") + 
  labs(x = "Circumference (mm)", y ="Age (Days)",title="Circumference Growth on Orange Trees:{closest_state}") +
  transition_states(Sample,transition_length=1,state_length=1,wrap=FALSE) +
  view_follow(fixed_x = TRUE)  +
  ease_aes('linear')+
  enter_fade()+
  exit_fade()

racing_oranges <- animate(racebar_graph, renderer = gifski_renderer())
orange_save <- anim_save('oranges.gif', animation = last_animation(), path = NULL)
...
# This data is a passenger list for the RMS Titanic, including information on the different passengers' names, if they survived, sex, age, number of siblings/spouses aboard, number of parents/children aboard, ticket number, ticket fare, cabin number, passenger class, and where they boarded the ship.
...
#It could cause confusion about the distribution of survivors. Did both spouses and one sibling survive? Or was it one spouse and two siblings? Same goes to the parents/children column.

# Question: What columns seem to be missing information?
#The Cabin and Age columns.
...
titanicTidyAge <- rawTitanic %>% 
  ...
  mutate(AgeDecadeLabel = case_when(
    AgeDecade >= 60 ~ "60s+",
    TRUE ~ paste0(AgeDecade, "s")
  ))
...
titanicTidyCabin <- rawTitanic %>% 
  ...
  na.omit() %>% 
  mutate(Deck = ifelse(grepl("[aA-zZ]+", Cabin), 
                       sub("[^aA-zZ]+.*", "", Cabin),
                       NA))
...
TitanicAge <- titanicTidyAge %>% 
  filter(Survived==1)

ggplot(TitanicAge, aes(x=Sex, fill = Sex)) +
  geom_bar() +
  transition_states(AgeDecade, state_length = 2, transition_length = 2) +
...
ggplot(titanicTidyCabin, aes(x = Deck, y = Age, color = as.factor(Survived), shape = Sex)) +
