...
ggplot(world, aes(long, lat, group = group)) +
  geom_polygon(fill = "orange", color = "navy") +
  coord_fixed(1.5) +
...
plot_usmap(data = countypop, values = "pop_2015", include = "NC", color = 'dark blue') + 
  scale_fill_continuous(low = "white", high = "dark green", name = "Poverty Percentage Estimates", label = scales::comma) + 
  labs(title = "2015 Population Estimates", subtitle = "United States") +
  theme(legend.position = "right")
...
ggplot() +
  geom_polygon(data = NY, aes(x=long, y=lat, group=group), fill = "lightblue", col = "black") +
  coord_fixed(1.5) +
  geom_point(data = starb_NY, aes(x = longitude, y = latitude), color = "red", size=0.5)
...
ggplot() +
  geom_polygon(data = CA, aes(x=long, y=lat, group=group), fill = "dark green", col = "black") +
  coord_fixed(1.5) +
  geom_point(data = starb_CA, aes(x=longitude, y=latitude), color = "light blue" , size = 0.1, shape = 7) +
  xlab("Longitude") +
  ylab("Latitude")
