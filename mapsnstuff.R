...
ggplot(world, aes(long, lat, group = group)) +
  geom_polygon(fill = "#00563f", color = "#b47ee5") +
  coord_fixed(1.3)
...
plot_usmap(data = countypop, values = "pop_2015", include = c("NC","SC","VA","WV","GA", "TN","AL","MS","KY"), color = "black") + 
  scale_fill_continuous(low = "turquoise", high = "red4", name = "County Population Southeast US", label = scales::comma) + 
  labs(title = "Southeastern States", subtitle = "Population Estimates for Southeastern US states") +
  theme(legend.position = "right")
...
ggplot()+
  ...
  geom_polygon(data=usa, aes(x=long, y=lat, group=group), color="black", fill = NA)+
  scale_fill_gradientn(trans = "log10", colors=rev(brewer.pal(9, "BuPu")))
...
# TASK:
# Now we are going to add our coordinates from the NYstarb
# to do this we are going to use "geom_point", just like from the example part 1
# Be sure to:
#   include the data set we are inserting (NYstarb)
#   specify that: x=longitude & y=latitude

ggplot() +
  geom_polygon(data = NY, aes(x=long, y=lat, group=group), fill = "lightblue", col = "black") +
  coord_fixed(1.5) +
  geom_point(data = NYstarb, aes(x = longitude, y = latitude), color = "darkgreen")
...
ggplot() +
  geom_polygon(data = NY, aes(x=long, y=lat, group=group), fill = "lightblue", col = "black") +
  coord_fixed(1.5) +
  geom_point(data = NYstarb, aes(x=longitude, y=latitude), color = "#Fe019a" , size = 1, shape = 25)
...
ggplot() +
  ...
  coord_fixed(1.5) +
  geom_point(data = CAstarb, aes(x=longitude, y=latitude), color = "#b47ee5" , size = 1, shape = 25)
...
ggplot(earth, aes(long, lat, group = group)) +
  geom_polygon(fill = "#429ef5", color = "black") +
  coord_fixed(1.3)
...
Countries and their capitals from across the world, with population data.

# Let's start by adding the capitals to the map as points. 

ggplot() +geom_polygon(data=earth, aes(x=long, y=lat, group=group), fill = "#429ef5", color = "black") +
  coord_fixed(1.3)+ geom_point(data = caps, aes(x= Longitude, y= Latitude))
...
ggplot() +
  ...
  coord_fixed(1.3)+ geom_point(data = caps, aes(x= Longitude, y= Latitude)) + geom_text(data=caps, aes(x=Longitude, y=Latitude, label=Capital.City), color= "black", size=1)
