...
lubridate::today() #The result was 2023-04-17
Sys.time()#The result is 2023-04-17 15:55:14 EDT
lubridate::now()#The result is 2023-04-17 15:55:51 EDT
lubridate::now(tzone = "UTC")#The result is 2023-04-17 19:56:49 UTC
lubridate::leap_year(today())#The result is FALSE
...
today_date
#The answer was 2002 which is not correct.
...
#The class of data is a character which is still not correct.
...
day(today_date) #It is the 17th of April so the answer is 17
year(today_date)#It is 2023 
month(today_date)#It is April so the month is 4
leap_year(today_date) #It is not a leap year
month(12, label = TRUE) #Lists the months of the year in order
ymd_hms(today_date) #Gave a warning for no format
hour(today_date)#The answer came out to zero
second(today_date) #The answer is zero again.
...
invoice_leap <- invoice %>%
  ...
  select(Due, is_leap)
#There were 4 payments that were conducted on February 29th.
...
#We would want to have a standardized/coordinated time so that if different countries are working on data, they should all be in standard time zone.
...
#Being able to convert time-zones helps us to understand data from different parts of the world and vice versa
# Task Time:)
...
#The format for SA and NZ is the day, month and then the year.
#Whereas the format for USA is the month, day, then the year.
...
#This matters because splitting the dates will allow us to visualize the data better.
