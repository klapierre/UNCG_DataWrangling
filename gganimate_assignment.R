...
ggplot(iris, aes(x = Species, y = Petal.Length)) + #puttin gin the dataset and selecting what factors to use
  geom_boxplot(aes(colour = Species)) + #setting up boxplot
  transition_states(Petal.Width, #splits data into multiple states based on levels in a given column - petal width in this case?
                    transition_length = 1, #shows how long we want the transition to be
                    state_length = 1) #the length of the pause between states
...
ggplot(iris, aes(x = Species, y = Petal.Length)) + 
  ...
  ylab("Petal Length") +
  xlab("Species") +
  ggtitle("Petal Length between Iris Species")
...
first_animation <- ggplot(iris, aes(x = Species, y = Petal.Length)) + 
  geom_boxplot(aes(colour = Species)) +
  transition_states(Petal.Width,
                    transition_length = 4,
                    state_length = 2) +
...
animation_please <- animate(first_animation, renderer = gifski())
...
anim_save("first_animation.gif", animation = last_animation(), path = NULL)
...
ggplot(ChickWeight, aes(weight, Chick)) + #adding dataset and sepecting factors
  geom_point(aes(color=Diet)) + #create scatter plot colored by diet
labs(title = 'Days: {frame_time}', x = 'Chick weight (grams)', y = 'Chick number') + #adding labels
  transition_states(Time,  transition_length = 1, #add transitions for the animation
                    state_length = 1) + 
  ease_aes('linear') #defined how a value cahnges to another during tweening
...
chick_graph <- ggplot(ChickWeight, aes(weight, Chick)) + #adding dataset and sepecting factors
  geom_point(aes(color=Diet)) + #create scatter plot colored by diet
  labs(title = 'Days: {frame_time}', x = 'Chick weight (grams)', y = 'Chick number') + #adding labels
  transition_states(Time,  transition_length = 1, #add transitions for the animation
                    state_length = 1) + 
...
chick_animation <- animate(chick_graph, renderer = gifski())
anim_save("chick_graph.gif", animation = last_animation(), path = NULL)
...
chick_bar <- ggplot(ChickWeight, aes(x = Time, y = weight)) +
...
chick_bar <- ggplot(ChickWeight, aes(x = Time, y = weight)) +
  geom_bar(stat='identity') +
  labs(title = 'Days: {frame_time}', x = 'Time', y = 'Chick Weight') + #adding labels
  transition_states(Time,  transition_length = 1, #add transitions for the animation
                    state_length = 1) + 
  ease_aes('linear')
##Task##
...
chick_animation_bar <- animate(chick_bar, renderer = gifski())
anim_save("chick_bar.gif", animation = last_animation(), path = NULL)
...
#it is an extention of ggplot so it works well together
...
#timelines would be great and population sizes
...
line_graph <- ggplot(orange, aes(age, circumference, group = Tree, color = as.factor(Tree))) + #selecting data
  scale_colour_brewer(palette = "Dark2") + #using the Dark2 palette for color
  geom_line() + #make a line graph
  labs(title = 'Orange Tree Circumference by Age', x = 'Age in Days', y = 'Circumference in mm') + #adding labels
  theme(legend.position = "top") + #changing the legend position
  geom_point(aes(group = seq_along(age)), size=2) #adding points along the line graph
line_graph #calling graph
...
#I think this would be great for trendlines - showing increases in the environment

#Now for graph 2!We're going to do a bar graph!

#Let's start by making a simple bar graph
bar_graph <- ggplot(orange, aes(age, circumference, fill=as.factor(Tree))) + #select date and color
  geom_bar(stat="identity", position=position_dodge()) + #set the grouping of the variable bars
  scale_fill_brewer(palette = "Accent") #set the palette colors
bar_graph
...
bar_graph <- ggplot(orange, aes(age, circumference, fill=as.factor(Tree))) + #select date and color
  ...
  coord_flip() +
  ggtitle("Orange Tree Circumference and Age")
...
#Question: When would this type of graph and animation be an appropriate way to communicate your data?
#it's a good want to show how different groups differ - demographics could be interesting with this
...
bar_graph <- ggplot(orange, aes(age, circumference, fill=as.factor(Tree))) + #select date and color
  geom_bar(stat="identity", position=position_dodge()) + #set the grouping of the variable bars
  scale_fill_brewer(palette = "Accent") + 
  transition_states(age,  transition_length = 1, #add transitions for the animation
                    state_length = 1) +
  labs(title = 'Circumference over Age : {closest_state}')
...
#family seperated could make some strange connections and maybe double counting


# Question: What columns seem to be missing information?
#Cabin number is definitely missing a lot of info
...
titanicTidyCabin <- rawTitanic %>% 
  ...
  mutate(cabin_letter = str_extract(Cabin, "[aA-zZ]")) %>% 
  na.omit()
...
ggplot(titanicTidyAge, aes(x = Sex, y = Survived)) +
  geom_bar(stat='identity') +
  scale_fill_brewer(palette = "Accent") +
  transition_states(age_group,  transition_length = 6, #add transitions for the animation
                    state_length = 4) +
  xlab("Sex") +
  ylab("Survived")+
  ggtitle("Sex and those who survived")
...
ggplot(titanicTidyCabin, aes(x= cabin_letter, y= Age, color = as.factor(Survived), shape = Sex)) +
  geom_point(position = "jitter") +
  transition_states(Pclass,  transition_length = 1, #add transitions for the animation
                    state_length = 1) + 
  labs(title = 'Passenger Class: {closest_state}', x = 'Deck Position (grams)', y = 'Age') +
  ease_aes('linear') 
