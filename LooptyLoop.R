...
# It will add another 50 to the previously transformed df.
...
#Nested loops are often used for multidimensional data and allow for more precise
#control of your data. 


### Example of Nested loops:
# While any kind of loop can be nested within other loops, it will be easiest to look at the same two loops nested within each other. 
# Nested loops allow us to look at multidimensional information. 
...
#We are going to create a scatterplot using the iris dataset, of sepal length vs. sepal width, with each point colored 
# according to its species (setosa in red, versicolor in blue, and virginica in green). The nested loops are going to be used to loop over each species in the dataset, 
# and then over each observation within that species to plot the points with the appropriate color.
...
#9. After the nested loop, use the legend() function to create a legend for the plot. Set the position to "topright", 
# the legend argument to the unique species in the dataset, the col argument to a vector of colors matching the species, 
# the pch argument to 1 to use solid point markers, and the title argument to "Species".
...
for(species in unique(iris$Species)) {
  ...
  for(i in 1:nrow(spdata)) {
    ...
    legend("topright", legend = c("Setosa", "Versicolor","Virginica"), col = c("red", "blue", "green"), pch = 16, title = "Species Name")
