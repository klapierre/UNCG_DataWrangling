...
lubridate::today() ##Pulls today's date
Sys.time()  ##Pulls the durrent date/time/timezone.
lubridate::now() ##As above.
lubridate::now(tzone = "UTC") ##The current time/date in UTC.
lubridate::leap_year(today()) ##Is this a leap year? True/False - in this case, false.
...
## It works for my today_date but todays_date is interpreted as a function.

#Check the class of today_date
class(today_date)
class(todays_date)

## It is a "date" class object for today_date, but todays_date is a "numeric" class.
...
# It's a character class, which is still not what we want.
...
class(Sys.Date()) ## This is a date class object.
class(lubridate::now()) ## This is a POSIXct object.
...
invoice_delay <- data.frame(invoice) %>%
  mutate(delay_date = Payment-Due) %>%
  filter(delay_date>0) %>%
  count(delay_date)
...
day() # Calls the day of the object.
year() # Calls the year of the object.
month() # Calls the month of the object.
leap_year() # Asks if the event occurs during a leap year. TRUE or FALSE.
month(12, label = TRUE) #Labels the months in order.
ymd_hms() # Calls the date as year-month-day-hour-minute-second format.
hour() # Calls the hour of the object.
second() # Calls the second of the object.
...
invoices_due <- data.frame(invoice) %>%
  mutate(Due, year = lubridate::year(Due), 
         month = lubridate::month(Due), 
         day = lubridate::day(Due))
...
invoice_leap <- data.frame(invoice) %>%
  mutate(due_day = lubridate::day(Due),
         due_month = lubridate::month(Due),
         due_year = lubridate::year(Due),
         is_leap = lubridate::leap_year(due_year)) %>%
...
## So that international colleagues can collaborate and combine their research consistently without having to worry about formatting errors or confusion.
...
# This process can also be done for many dates at once! 
...
## You can convert an unfamiliar timezone into your own to better understand what is going on, or you could use it to standardize a dataset that is occuring across multiple timezones.
...
B <- c("2027-09-11 6:00", "2007-05-01 6:00", "2013-01-29 6:00", "2020-11-04 6:00")

time_A <- as.POSIXct(B,"US/Mountain")
cbind(US=format(time_A),Singapore=format(time_A,tz="Singapore"))
...
## I'm not sure what I'm supposed to be running individually. :(
...
## New Zealand (NZ Research Federation), South Africa (Kreuger NP) and USA (Lincoln National Research Station & Fort Keogh)
...
USA_time <- date_time_untidy %>%
  filter(Location =="USA")

NewZealand_time <- date_time_untidy %>%
  filter(Location =="New Zealand")

SouthAfrica_time <- date_time_untidy %>%
  filter(Location =="South Africa")

NZ_SA_join <- full_join(SouthAfrica_time, NewZealand_time)
...
USA_time[c('Month','Day', 'Year')]<-str_split_fixed(USA_time$Date, '/', 3)

#Task: Create the correct function to split the dates of the international data

NZ_SA_join[c('Day','Month', 'Year')]<-str_split_fixed(NZ_SA_join$Date, '/', 3)

#Question: Why does this even matter?

## To make the dates consistent and compatible across the data.
...
USA_time<- USA_time %>%
  mutate(local=paste(USA_time$Month, USA_time$Day, USA_time$Year, sep = "/")) %>%
  mutate(international=paste(USA_time$Day, USA_time$Month, USA_time$Year, sep = "/"))
NZ_SA_join <- NZ_SA_join %>%
  mutate(local=paste(NZ_SA_join$Month, NZ_SA_join$Day, NZ_SA_join$Year, sep = "/")) %>%
  mutate(international=paste(NZ_SA_join$Day, NZ_SA_join$Month, NZ_SA_join$Year, sep = "/"))
...
new_USA_time <- USA_time %>%
mutate(Time24=format(strptime(USA_time$Time.Recorded, "%I_%M %p"), format="%H:%M:%S"))

#Task: Do the same for the International data

new_NZSA_time <- NZ_SA_join %>%
  mutate(Time24=format(strptime(NZ_SA_join$Time.Recorded, "%H_%M"), format="%H:%M:%S"))

#Now, let's put it all together in your chosen formats!

total_time <- full_join(new_USA_time, new_NZSA_time)


#Lets plot them all using ggplot!

ggplot(total_time, aes(x = Time24, y = Luz.Values)) + 
  geom_point(aes(color= as.factor(Site))) +
  xlab("Time") + 
  ylab("Luz Values")
