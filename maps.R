...
plot_usmap(data = countypov, values = "pct_pov_2014", include = c("FL", "VA", "TX", "NV", "CA"), color = "black") + 
  scale_fill_continuous(low = "white", high = "#810aff", name = "Poverty Percentage Estimates", label = scales::comma) + 
  labs(title = "Southwestern States", subtitle = "Poverty Percentage Estimates for Counties in Southwestern States 2014") +
  theme(legend.position = "right")
...
ggplot()+
  geom_polygon(data=usa, aes(x=long, y=lat, group=group), color = "red", fill=NA)+
  coord_fixed(1.3)+
  geom_polygon(data=states, aes(x=long, y=lat, group=group), fill=NA, color="white")+
  geom_polygon(data=age_pop_states, aes(x=long, y=lat, group=group, fill = Adults.55.64), color = "white")+
  geom_polygon(data=usa, aes(x=long, y=lat, group=group), color="red", fill = NA)+
  scale_fill_gradientn(trans = "log10", colors=rev(brewer.pal(9, "YlGnBu")))
...
starb_NY <- read.csv("starbucks_2018_11_12.csv", stringsAsFactors = TRUE) %>% 
  subset(country == "US") %>% 
  select(state, name, latitude, longitude)
...
ggplot() +
  geom_polygon(data = starb_NY, aes(x=long, y=lat, group=group), fill = "blue", col = "red") +
  coord_fixed(1.5)
# Congratulations!
...
ggplot() +
  geom_hexagon(data = CA, aes(x=long, y=lat, group=group), fill = "blue", col = "pink") +
  coord_fixed(1.5) +
  geom_point(data = starb_CA, aes(x=longitude, y=latitude), color = "#Fe0" , size = 5, shape = 25)
...
pines <- read.csv("observations-304633.csv")
...
#understanding the syntax of the programming language. Thankfully, you can always find help from online communities, 
#youtube tutorials or virtual courses in R programming.
>>>>>>> 8c461e7755e33379d4d22e326992e94c9404dea7
=======
<<<<<<< HEAD
############ 
#   MAPS   #
...
#youtube tutorials or virtual courses in R programming.
>>>>>>> 8c461e7755e33379d4d22e326992e94c9404dea7
>>>>>>> 4d5d3cc3ab15a853a90ebd568ff42327e92af0b8
