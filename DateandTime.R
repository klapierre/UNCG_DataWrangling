...
lubridate::today() #retrieves todays date
Sys.time() # retrieves systems day, time and time zone
lubridate::now() #retrieves todays date/time, same as above
lubridate::now(tzone = "UTC") #retrieves todays date and time in the UTC time zone
lubridate::leap_year(today()) # checks if it is currently a leap year
...
#it was originally stored as a date
...
class(Sys.Date()) #class is date
class(lubridate::now()) #class is POSIXct
...
day(today_date) # displays the day in console
year() # displays the year in console
month() # displays the month in console
leap_year() # gives a yes/no output on whether it is a leap year
month(12, label = TRUE) #lists all 12 months
ymd_hms() #transforms characters or numeric vectors into date based on year, month, day, hour, minute, seconds.
hour() # displays the hour in console
second() # displays the second in console
...
invoice_leap <- invoice_due %>% 
  mutate(leap = leap_year(due_year)) %>% 
...
#so that anyone globally can have a reference point to know times in other regions
...
#less math for the reader in another time-zone if it is adjusted to their time, such as a meeting time.
#not really useful for data observations.

# Task Time:)
# Create and name some dates and times named happy_time it should look similar to the E<- that is above!
# Once that is done pick some dates and times of your choice ex. US/Alaska time, GMT, PRC, ect. 
# If you need help just remember we have a code that will pull-up all the time-zones:) OlsonNames(tzdir = NULL).
# This should look very similar to the second code named time_1

happy_time <- c("2009-06-12 4:00", "2010-03-08 16:30", "2015-11-28 10:00", "2006-04-29 12:00", "2011-10-04 18:00", "2009-10-25 12:00", "2009-10-31 12:00", "2009-11-01 12:00")
happy_time2 <- as.POSIXct(happy_time,"US/Alaska")
...
#NZ and Kreuger are formatted day/month/year 
#Lincoln NRS and Fort Keogh are formatted month/day/year 
...
Not_USA_untidy <- full_join(NZRF,KNP)
...
Not_USA_untidy[c('Month','Day', 'Year')]<-str_split_fixed(Not_USA_untidy$Date, '/', 3)


#Question: Why does this even matter?
#so that the dates are arranged the same way in each dataset so that it can be joined correctly.
