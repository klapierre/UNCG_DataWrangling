...
# included in the output? the code above only generate a coordinate environment and label for the x-axis. However it didn't pull data from the .csv file.
...
# about the labels for the x- and y-axes? The code above made a historgram. A very sad and grey one. the x and y axes are labeled, however there is no indication of units for the values.
...
#it rearranged the data distribution with data being grouped in increments of 50 units, giving it a very different appearance
...
#the majority of this population is young and under 2.5 
...
#It means it removed data points where there was no numerical value (logical, NA) it is okay to proceed but we should remove the non numerical data to avoid the annoying warning message

# ---------------------------------------------------------- #
#### PART 1.3 GEOMETRIC OBJECT: POINTS                    ####
# ---------------------------------------------------------- #

# QUESTION: In the code below, what is the dataframe being examined? redband
# What are the aesthetics? What will the resulting graph be plotting? graph on x and y coordinates
...
#They are correlated;the older the fish, the longer the length
...
#dropping the NAs to avoid error message
redband %>%
...
#The graph is all in black now that there is no statement linking color value to the variable ScaleAge

# TASK: Visit the ggplot Cookbook webpage at http://www.cookbook-r.com/Graphs/ done!
...
#color = outline of solid objects, or lines on the graph, as well as, data points
#fill = it will be used to fill a bar with selected color

# QUESTION: Why did we have to specify as.factor() for ScaleAge in the initial
# aes() statement? Because ggplot treats numerical and non-numerical data differently. the as.factor() statement makes ScaleAge a non-numerical variable
...
# in the above graph for our statistical transformation fit? Generalized additive model (GAM)
...
ggplot(redband, aes(x = Length, y = Weight, color = as.factor(ScaleAge))) +
  geom_point() +
  geom_smooth(formula = y ~ poly(x, 2), method = "lm", color = "black", size = 2)
...
# The graphic representation of the weight of fish in relation ScaleAge. The y-axis is skewed. It seems that ggplot is adding up the weight of the individuals by age category.
...
redbandSummary <- redband %>%
  filter(!is.na(Weight), !is.na(ScaleAge)) %>%
  group_by(ScaleAge) %>%
...
# geom_bar() help or google to find the answer.stat='identity' tells geom_bar() to use the Weight_mean values directly for the height of the bars, instead counting the number of observations in each group.
...
ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + #creates a coordinate system with a designated variable for each axis
  geom_bar(stat='identity') + #tells ggplot to use the numerical values within the variable Weight_mean
  geom_errorbar(aes(ymin=Weight_mean-Weight_se,
                    ymax=Weight_mean+Weight_se,
                    width=0.2)) #defines minimum and maximum values of the error bar and determines the line width
...
# it and seeing what happens. It tells ggplot what is the width of the line of the error bar
...
# QUESTION: What is different about this graph from before? It has linear regression lines plotted over the data point distribution, and the color of the lines and dots are the same, since we moved the color statement
...
ggplot(redband, aes(x = as.factor(ScaleAge), y = Length)) +
  geom_boxplot(fill = "dodgerblue", color = "darkred") +
  scale_y_log10() +
...
# refer to? What does the aesthetic element_blank() do? In the first figure, the minor grid lines have been removed. In the second figure, the entire panel background has been removed. The element_blank() function sets the appearance of a theme element to be completely blank or invisible.
ggplot(redband, aes(x = Length, y = Weight)) + 
  geom_point() +
...
# QUESTION: What does element_text() refer to in the code above? It refers to the label for the y-axis
...
#It showed up in the files window, the graph is a histogram
...
#Correlation = dot plot with a trend line (visualize the relationship between two numeric variables)
#Deviation = boxplot (deviations from a baseline)
#Distribution = histogram, bar graph (display the distribution of categorical variables)
...
#Because most of the observations in the file are about the Redband Trout, making it ideal for data analysis. The other species were underrepresented in the data. 
