...
### The first is Peabody Park, "pb"
```{r}
filter_pb_cover <- land_cover %>%
  filter(str_detect(box,"pb"))
```

### Next is Price Park, "pp"
```{r}
filter_price_cover <- land_cover %>%
  filter(str_detect(box,"pp"))
```

### Then Gateway Park, "gnc"
```{r}
filter_gnc_cover <- land_cover %>%
  filter(str_detect(box,"gnc"))
```

### Finally, Country Park, "cp"
```{r}
filter_cntry_cover <- land_cover %>%
...
pbp_join <- left_join(clean_pbp,filter_pb_cover, by="box", copy=FALSE)

```

```{r}

price_join <- left_join(clean_price,filter_price_cover, by="box", copy=FALSE)

```

```{r}
gnc_join <- left_join(clean_gnc,filter_gnc_cover, by="box", copy=FALSE)

```

```{r}
cntry_join <- left_join(clean_cntry, filter_cntry_cover, by="box", copy=FALSE)
