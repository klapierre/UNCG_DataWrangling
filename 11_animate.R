...
change over time animation shows how a variable changes across different time points in your observation 
...
ggplot(airquality, aes(x = Day, y = Temp)) +
  geom_line() +
  ggtitle("Daily Temperature in the Airquality Dataset")
...
the first codes both render the animation and saves it asa gif file called temperature.gif in my working directory 
##QUESTION: What is the animate() function doing?
the animate() function takes the gganimate plot and creates the animation frame by frame 
##QUESTION: What happens if we take out the transition_time() function?
If you remove transition_time(Month), the plot will no longer animate cross months.
...
the is dynamic it changes to show the month for each frame 
##QUESTION: Why would someone choose to use the {frame_time} argument instead of 
##just typing a whole new title?
because it makes the title update automatically for each frame of the animation. instead of one fixed title. 
...
some helpful characteristics to modify are the speed, number of frmaes and pause length, smoothness of movement between frames 
##Run the code below
create a line plot using the airquality dataset 
airquality_temp <- ggplot(airquality, aes(x = Day, y = Temp)) +
  draw a line connecting temp values across days 
  geom_line() +
    make the animation transtion at a constant even speed 
  ease_aes('linear')+
    add a title that updates for each frmae to show the current month 
  labs(title = "Month: {frame_time}") +
    animate the plot over time using month as the time variable 
  transition_time(Month)
  render the animation and save it as a GIF file
...
shadow trail does motion while shadow_mark does prevouis postion or states 
...
you would use transition states for grpahs where the animation moves through separate categories or groups instead of a continous time variable 
...
transition time the animation follows a time varibale and updates by time while transition states switches between disrinct groups and lables 
...
makes the grpah build up porgressilvey as the variable changes.
...
is the most benefical when you want to show how multiple individual groups or objects move indepedently over time 
...
nframes,fps,duration,detail and many more would let you control things like how many frames are made,how fast the animation plays and how long it lasts
