...
setwd('C:/Users/sma1104/Desktop/BIO 471/UNCG_DataWrangling/gganimate')
getwd()
...
# the first one saves the pictures in a gif, the second one saves 100 individual images
##QUESTION: What is the animate() function doing?
# puts the individual photos into an animated graph
...
anim_save("temperature_titles.gif", animate(airquality_temp, renderer = gifski_renderer())) +
  animate(airquality_temp, renderer = gifski_renderer())
...
# it changes the month with the graph
...
# the speed or number of frames
##Run the code below
airquality_temp <- ggplot(airquality, aes(x = Day, y = Temp)) +
  # creates a plot of the airquality dataframe with Day vs Temp
  geom_line() +
  #specifies what type of plot
  ease_aes('linear')+
  #controls the speed of transition between frames
  labs(title= "Month: {frame_time}") +
  # specifies the title of the graph 
  transition_time(Month)+
  # tells R to animate based on each month
anim_save("temperature_ease.gif", animate(airquality_temp, renderer = gifski_renderer()))
# renders and saves the animation (does not show it)
...
# shadow_mark shows all the data points, whereas shadow_trail shows fewer evenly spaced data points
...
# transition_states should be used on graphs that use categorical data like bar charts or box plots
...
# you dont have a shadow function or an ease_aes function
...
# it allows you to let the data points appear gradually
...
animate(airquality_group, renderer = gifski_renderer(),  nframes = 150,fps = 15,width = 600,height = 400)
...
# if you need to visualize each group's data transition separately
...
# rewind, end_pause, start_pause, ref_frame, fps, etc.
...
animate(airquality_temp,height=400,width=600,nframes=100,fps=10,renderer=gifski_renderer())
...
airquality_discrete <- ggplot(airquality, aes(x = Day, y = Temp)) +
  geom_point() + #changed from geom_line to geom_point
  transition_states(Month) +
  labs(title = "Month: {closest_state}")#changed title to Month to match the transition state, and frame time to closest state since it's a discrete variable
...
setwd('C:/Users/sma1104/Desktop/BIO 471/UNCG_DataWrangling')
getwd()
...
gapminder_NA <- gapminder %>% 
  filter(country %in% c('United States','Mexico','Canada','Puerto Rico'))
...
gap_plot <- ggplot(gapminder_NA,+aes(x=gdpPercap,
                                     y=lifeExp,
                                     size=pop,
                                     group=country,
                                     color=country))+
  ...
  scale_x_log10()+
  labs(title='GDP vs. Life Expectancy by Country',
       x='Percapita GDP',
...
# because it would be impossible to tell which data points represent which countries without grouping
...
# it follows the point closely at a more zoomed in scale
...
# there are no visible data points
...
iris_anim <- ggplot(iris,
                    aes(x=Petal.Length,
                        ...
                        color=Species))+
  geom_point()+
  labs(title='Petal Length vs. Petal Width by Species:{closest_state}',
       x='Petal Length',
       y='Petal Width')+
  transition_states(Species)+
  theme_classic()+
  theme(legend.position = 'none')
  animate(iris_anim,renderer = gifski_renderer())
...
#points slide into place from the left and then exit left
iris_anim + enter_drift(y_mod = 1) + exit_drift(x_mod = 1) 
#points slide into place from above and then slide out to the right
iris_anim + enter_recolor(color = "pink") + exit_recolor(color = "brown")
#points fade in as pink and change to brown before disappearing
iris_anim + enter_grow(size = 10) + exit_shrink(size = 0.1)
#the points start really big and shrink to a very small size before disappearing
...
#a single dot hits all of the data points for each group, data is revealed gradually rather than all at once
...
# so that you can see how variables change in each area over time
## QUESTION: What are some potential weaknesses of animating. 
## Hint: Think about how slow your computer probably ran!
#it takes a long time because it's very involved, you cant pause it so it may be hard to actually see the trend if the animation isnt slow enough
...
# appear on the graph?
#theres an empty column called "X"and there would be a lot of overlap of points on the same date
...
#the second one because the labels are more descriptive
...
# purpose?
# i like them because you can use them to display multiple variables at a time (2+)
...
# it would make. 
# there would be far too many data points otherwise
...
ggplot(hourMonthAvg, aes(x = hour,y = mean_temp,color = month,size = mean_temp)) +
  ...
  labs(x = "Hour of Day (0–23)",y = "Average Temperature (°C)",color = "Month",size = "Temperature (°C)",title = "Hourly Average Temperature by Month") +
  theme_bw() +
...
# it's measured in degrees Celsius
...
# the y-axis now ranges from 0-125ish
...
# ggplot is creating an animated graph of average temperature vs date 
...
#it creates a directory, in the UNCG_DataWrangling folder
...
# scatter plot?
# you have to start on day 2 because it is the first day with a previous comparison (day1)
...
# if you needed to edit something on an animation after the animation has already been made, making smoother transitions
