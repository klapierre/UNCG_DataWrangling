...
##columns in could we use to show this? # we can use month and day 
...
ggplot(airquality, aes(x = Day, y = Temp)) +
  geom_line() +
  labs(title = "Daily Temperature Changes Over Time")
...
# One code likely just creates the animation object, while the other actually renders and saves it
##QUESTION: What is the animate() function doing?
# animate() is the engine that renders the animation frame by frame
##QUESTION: What happens if we take out the transition_time() function?
# if you remove the function, the plot loses it's sense of time progression. 
...
##QUESTION: What is the new title of the graph? # frame_time

##QUESTION: Why would someone choose to use the {frame_time} argument instead of 
##just typing a whole new title? # automatically updates the title as the animation moves through time
...
# helps with speed, duration, number of frames, pause length 
##Run the code below
airquality_temp <- ggplot(airquality, aes(x = Day, y = Temp)) + #creates a line plot showing temperature changes across days 
  geom_line() +
  ease_aes('linear')+ # sets a smooth, constant (linear) transition between frames
  labs(title = "Month: {frame_time}") + # Dynamic title that updates with the current month in the animation 
  transition_time(Month) # animates the plot over time using the Month variable
anim_save("temperature_ease.gif", animate(airquality_temp, renderer = gifski_renderer()))
# saves the animation as a GIF file
# animate() renders the animation frame-by-frame
# gifski_renders() converts the frames into a GIF format 
...
# shadow_trail shows a fading trail of previous frames and shadow_mark keeps all past data points visible
...
# use transition_states() when your data moves through distinct categories or groups, not continuous time
...
# the main difference in the code is what kind of variable you pass into the transition function and how the animation is labeled
...
# transition_reveal() is like slowly drawing your plot onto the screen instead of jumping between frames
...
# transition_components() is most useful when your data has multiple parts that make up a whole, 
## and you want to show how those pieces change individually over time
...
# you can use nframes(), fps(), duration, width/height, renderer(), start_pause (), rewind()
...
anim_save("temperature_ease.gif",
          animate(airquality_temp, 
                  nframes = 100,
                  fps = 10,
                  width = 600,
...
airquality_discrete <- ggplot(airquality, aes(x = Day, y = Temp)) +
  geom_point() + # FIX 1: changed from geom_line() to geom_point() since this is discrete states, not a continuous line
  transition_states(Month) +
  labs(title = "Month: {closest_state}") # FIX 2 & 3: changed label text and replaced {frame_time} with {closest_state} for transition_states
...
airquality_ozone <- ggplot(na.omit(airquality), aes( x = Day, y = Ozone)) +
  geom_line()+
  transition_time(Day)+
  shadow_trail()+
  ease_aes("linear") +
  enter_fade()+
  labs(title = "Ozone Levels Over Time (Day: {frame_time})")
anim_save("ozone_animation.gif",
...
# the gapminder dataset is inherently time-based because it includes a year column that tracks changes over time. 
# it matters because it allows for smooth animations using transition_time() 

## TASK: Create a filtered gapminder dataset for North America only
gapminder_na <- subset(gapminder, continent == "Americas")
...
gap_plot <- ggplot(gapminder, aes(x = gdpPercap, y = lifeExp,
...
# Grouping is important because it tells the animation engine that these points belong to the same entity over time

## TASK: Animate by year using transition_time(). Save this as gap_anim.
gap_anim <- gap_plot +
  transition_time(year)
animate(gap_anim, renderer = gifski_renderer())
anim_save("gap_anim.gif", gap_anim)
...
gap_anim + ease_aes('cubic-in') # start slowly, then speeds up smoothly (no reverse or bounce effects)

gap_anim + ease_aes('elastic-in') # starts with a stretchy, rubber-band-like motion before settling into speed

gap_anim + ease_aes('circular-in') # begins slowly following a curved acceleration path

gap_anim + ease_aes("bounce-in") # starts with a bouncing effect before moving into the animation flow

# -out applies the easing function in reverse
gap_anim + ease_aes('elastic-in') # movement overshoots and wiggles at the end like a spring settling

gap_anim + ease_aes('circular-in') # slows down smoothly toward the end of each transition

gap_anim + ease_aes("bounce-in") # ends with a bouncing effect before coming to rest

# we can combine them into -in-out
gap_anim + ease_aes('circular-in-out') # slow start, fast middle, slow end

gap_anim + ease_aes("bounce-in-out") # bounces at both the start and end of each transition

## QUESTION: What does the -in-out easing argument do to our animation? # -in() slows down at the beginning / -out() slows down at the end
## Hint: Check ?ease_aes().
## QUESTION: How does easing change the perception of movement over time? # by controlling how movement is experienced, not just when it happens
...
# automatically makes the plot camera follow the data as it changes over time. It's useful when you want to shift your data across a wide range over time
...
# transition_time() is continuous, but view_step() is designed for discrete, step-like transitions 
...
iris_anim <- ggplot(iris, aes(x = Petal.Length, y = Petal.Width, color = Species)) +
  ...
  labs(
    title = "Iris Species: {closest_state}",
    x = "Petal Length",
    y = "Petal Width"
  ) +
  theme_minimal()+
...
# enter_fly(x_loc = 0): points fly into the plot starting from the left side (x = 0)
# exit_drift(x_mod = 1): points fly out toward the right side (x =1); creates horizontal "whoosh" motion across the screen
iris_anim + enter_drift(y_mod = 1) + exit_drift(x_mod = 1) 
# enter_drift(y_mod = 1): points drift upward as they enter the plot
# exit_drift(x_mod =1): points drift sidewats as they leave creates a floating, drifting motion effect
iris_anim + enter_recolor(color = "pink") + exit_recolor(color = "brown")
# enter_recolor(color = "pink"): new points appear as pink
# exit_recolor(color = "brown"): departing points fade/shift to brown; changes color during entry/exit for visual emphasis
iris_anim + enter_grow(size = 10) + exit_shrink(size = 0.1)
# enter_grow(size = 10):points start very large and shrink 
# exit)_shrink(size = 0.1): points shrink down before disappearing; dramatic size transition
iris_anim + enter_grow(size = 0.1) + exit_shrink(size = 10)
# enter_grow(size = 0.1): points start tiny and grow into full size
# exit_shrink(size = 10): points grow large before leaving the plot
iris_ease <- iris_base + 
  transition_states(Species) + # animates between iris species categories
  ease_aes("bounce-in-out") # adds bounce motion at start and end of transition 
...
# instead of switching between species or frames, the plot draws the points progressively along Petal.Length
...
# animations are important for spatial data because space isn't just what is happening, it also where and often how things move or shift across locations
...
# make this data not ideal to graph? Why or why not? Yes, there is something that makes this data not ideal to graph which the very granular time data which creates too many unique x-values for a simple ggplot
# Hint: Think about how we would write the ggplot code. How would the datapoints 
# appear on the graph? # you would get tons of overlapping points, x-axis would be extremely dense and unreadable, and it would look like a cluttered cloud rather than a clear pattern
...
ggplot(dailyAvg, aes(x =  lubridate::month(date), y = mean_temp) +
  geom_point()+
  labs(
...
# dailyAvg dataframe? Why? # the second graph is better because it meant to show overall patterns, not individual raw measurements
...
# purpose? # bubble charts show relationships between variables, highlights differences in magnitude, and help identify clusters/trends/outliers 
...
rangeTemp <- tempData %>%
  mutate(time = hms::as_hms(time), hour = hour(time),
         ...
         # instead of plotting each hour for this graph? # it simplifies the data and reveals the overall pattern more clearly
 ...
 ## the temperature looks very low because it's likely being measured in Celsius, not Fahrenheit        
         ...
         ## stands for directory create; makes a nee folder
         ...
         ## incomplete or unstable date; a baseline/initialization period
...
## helps you edit and enhance the final visual output
