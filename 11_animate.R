...
##You could Month and Day for time and track either temp or ozone.
...
##First code saves as gif file in the wd and the second one only displays it does not save.

##QUESTION: What is the animate() function doing?

##Makes the animation and displays in R

##QUESTION: What happens if we take out the transition_time() function?

##Wont show changes over time.
...
##Changes to show the month thats in the frame.

##QUESTION: Why would someone choose to use the {frame_time} argument instead of 
##just typing a whole new title?

##Title is updated for each frame automatically which is much easier.
...
##Speed, smoothness and timing between frames.

##Run the code below
airquality_temp <- ggplot(airquality, aes(x = Day, y = Temp)) +   ##Creates the base plot with Day on the x-axis and Temp on the y-axis
  geom_line() + ##Adds line graph
  ease_aes('linear')+ ##Makes animation move steadily
  labs(title = "Month: {frame_time}") + ##Adds title that changes to show current month
  transition_time(Month) ##Animates graph over Month column
anim_save("temperature_ease.gif", animate(airquality_temp, renderer = gifski_renderer())) ##Saves as GIF
...
##shadow_trail shows a fading trail of previous frames, while shadow_mark shows all previous points without fading.
...
##Use for discrete graphs like bar plots or scatter plots - not continuously.
...
##Uses discrete states instead of continuous time, and uses {closest_state} instead of {frame_time} in titles.
...
##Gradually reveals the data over time.
...
##When you have more groups and want to show how each group changes separately.
