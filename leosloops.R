...
## For the columns containing the word 'Width' in the title, 1000 was added to the
## initial value. As such, if 50 is added instead - the sums only have 50 added instead.
...
## The correct way (after talking with Lowie)
x<- 0     
while (x < 10) {x <- x + 1 
print(x)
if (x != 2) 
...
## Repeat loops are good for a singular, static set of data that we want to make multiple
## adjustments to, while While loops are good for several datasets that need to be
...
plot(x = iris$Sepal.Length, y=iris$Sepal.Width, 
     ...
     main ="Sepal Length v. Width in Iris Species")
for(species in unique(iris$Species)) {
  ...
  points(species_data$Sepal.Length, species_data$Sepal.Width, col=col, pch = 16)
}

legend("topright", 
       legend =unique(iris$Species),
       col =c("red3", "royalblue3", "seagreen"),
       pch = 16,
       title = "Species")
