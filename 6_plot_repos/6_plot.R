...
# 2. To becomecomfortable using ggplot2 for exploratory data visualization.
...
# A histogram using the dataframe was made. The Y axis was automatically labeled
# and the X axis label stayed the same as indicated
...
# It combined data between the X axis points to make less segmented bars. 
# They combined to change every 50 points
...
# Most of the fish from the total count are one year old, with a downward trend 
# from 2-7 years 
...
# Th warning indicates that the values of NA were deleted by ggplot. I think 
# it can be ignored since it removed them automatically.
...
# There would be a graph containing the correlation between the data for fish age 
# and their lengths. This would help us know how the fish may be longer or shorter
# based on their ages. However, we have to indicate what kind of graph to plot 
...
# As the fish increase in age, they increase in length (They grow!)
...
# Without the as.factor, ggplot gives us a gradient with no definative values 
# for age, only the range given in the data fram (0-7). This is not helpful, as 
# there are only ages recorded in whole numbers. As.factor fixes this by dividing 
# the colors by whole numbers within the scale
...
ggplot(redband, aes(x= Length, y=Weight)) + geom_point(aes(color=as.factor(ScaleAge),size=as.factor(ScaleAge)))
...
# Color is the color that the outlyers/lines will be, fill is the color within the boxes
...
# The same reason as before, without as.factor does the range of the data instead of
# splitting it into whole numbers 
...
# Generalized additive model (GAM) since there are more than 1000 observations
...
ggplot(redband,aes(x=Length, y=Weight)) + geom_point(aes(color = as.factor(ScaleAge))) + geom_smooth(method = "gam", size=2, color="black")
...
# What looks like a bar graph is created, however the Y axis range is wayyy to high. 
# Instead, a stacked graph is created making the scale too high. 
...
redbandSummary <- redband %>% group_by(ScaleAge) %>% summarize(Weight_mean = mean(Weight, na.rm = T), Weight_sd = sd(Weight, na.rm = T)) %>% mutate(Weight_se = Weight_sd*1.96) %>% ungroup()
...
# It leaves the data as it is in the dataframe
...
# ggplot(redbandSummary, aes(x = as.factor(ScaleAge), y = Weight_mean)) + 
# geom_bar(stat='identity') -> makes the bar graph as indicated with each age having
# its own bar
# + geom_errorbar(aes(ymin=Weight_mean-Weight_se,ymax=Weight_mean+Weight_se, width=0.2))
# ymin is the mean weight and mean standard error subtracted, and ymax is the opposite, 
# indicating the highest and lowest points of the error bars. The indicated width 
# is the width of the error bars

# QUESTION: What does the statement width=0.2 do? If you're unsure, try removing
# it and seeing what happens.

# It shortens the size of the horizontal lines in the error bars. While their position
# doesnt change, the default size is too long visually 
...
ggplot(redbandSummary, aes(x=as.factor(ScaleAge), y=Weight_mean)) + geom_bar(stat="identity", color = "darkgreen", fill = "lightgreen") + geom_errorbar(aes(ymin=Weight_mean - Weight_se, ymax = Weight_mean + Weight_se, width = 0.4, color = "darkorange"))  
...
ggplot(redband, aes(x = Length)) + geom_histogram() + facet_wrap(~ScaleAge, scales = "free") 
...
# It saved in the working directory I have opened 
...
ggsave("Redband_histogram_facet.png",dpi = 600, width = 10, height = 8, units = "in")
...
# Correlation: dot plot; Deviations: boxplot; Distribution: histogram and bar graph
...
LowerSpokaneFish <- read_csv("/cloud/project/5_transform_repos/LowerSpokaneFish.csv")
View(LowerSpokaneFish)
ggplot(LowerSpokaneFish, aes(x= as.factor(ScaleAge), y = Length)) + geom_point(shape = 17, aes(color = as.factor(ScaleAge))) + facet_wrap(~Species) + scale_y_log10() + xlab("Fish Age (Years)") + ylab("Length (cm)")
ggsave("LowerSpokaneFish_Plot.png", dpi = 450, units = "in", width = 9, height = 7)

# QUESTION: Why do you think we focused on Redband Trout for most of this assignment?

# They have data for scale age and the other species do not.  
