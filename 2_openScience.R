...
# an overview of the specifics of the data set
...
# so you can apply the script to new data that is collected
...
# If it is ongoing, it is still subject to change
...
setwd("C:/Users/Kaysa/OneDrive/Desktop/BIO 400X/UNCG_DataWrangling")
...
conSME_abundance_dataframe <- read.csv("C:/Users/Kaysa/OneDrive/Desktop/BIO 400X/UNCG_DataWrangling/Vaarre-Lamoureux_conSME_data/CME011.csv")
...
#it read the data frame from the repository using the hyper link

## TASK: Check to see if the two dataframes are identical using an R function.
## (Hint: remember the Week 1 assignment?)
identical("conSME_abundance_dataframe", "conSMEcoverAlt")
# they are not identical
## QUESTION: Why might it be better to source data straight from the data portal?
## Why might it be worse?
# changes might have occurred when saving the data to your computer, so it might be better to source data from the data portal
...
#I'm not sure yet, I think I will just use the same naming convention for everything
...
duplicates <- rbind(conSME_abundance_dataframe, conSMEcoverAlt)
duplicates
...
#it has double the number of observations
...
#rows 4934 and 12173 are the same
...
#noDuplicates has less than half of the number of original observations for duplicates. It even has a little less than the
#observations for the original dataframes
