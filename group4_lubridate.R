...
# A Unix timestamp measures the number of seconds that have passed since January
#1st, 1970 at 00:00:00 UTC; this is referred to as the 'Epoch'. This is useful 
#for computing systems because it stores all time measurements as one large number,
#rather than more complex formats such as month/day/year. The date-times functions
#in lubridate allow users to quickly convert Unix measurements to more 
#user-friendly formats.  

dt_practice <- as_datetime(946684860)

# QUESTION: Run the code above to convert the Unix measurement to ymd_hms format
#using the ‘as_datetime’ function. What date and time does the timestamp correspond to?

#Jan 1, 2000, 00:01:00 UTC

# Unix timestamps are also measured in days since January 1st, 1970. The ‘as_date’
#function can be used to convert these measurements to ymd format.
# QUESTION: What holiday does the 20392 Unix days timestamp correspond to? What 
#year? HINT: Use the ‘as_date’ function to convert days to ymd format. 

dt_holiday <- as_date(20392)

#Halloween, 2025.

# Unix timestamps can also measure seconds passed since 00:00:00 (with no 
#corresponding date or time zone). The ‘as_hms' function can be used to convert 
#these measurements to hms format. 
# TASK: Run the following code to convert 10,000 seconds to hours. 

print(dt_time <- hms::as_hms(10000))

#QUESTION: How many hours are 86,400 seconds? HINT: Use the ‘as_hms’ function to 
...
# Parsing is the process of converting strings or numbers to into standard 
#date-time format (ymd_hms). This can be done one value at a time, or for an 
#entire column. 
# You can parse date-times with several different functions, so it's important 
#to use the function that corresponds to your input value!
# TASK: Run the following lines of code to view how lubridate converts different 
#date-time formats.
...
# TIP: If your data only includes dates without a timestamp, you can use the same
#functions by removing '_hms' from the function. 
...
#TASK: Run the following line of code to convert "07-04-12" (July 4th, 2012) to 
#standard date-time format. 

ydm("07-04-12")

# QUESTION: Is the output value correct? Why not? Rewrite the code with the same 
#input using the correct function below. 

#The output value is incorrect because we used the wrong format, that being y/d/m
#instead of m/d/y.

mdy("07-04-12") 

#TASK: Use any of the parsing functions to convert your birthday into standard 
#date format in the space below. 

mdy("December 8th, 2004")

# Parsing data is super convenient, but what if we want to parse an entire column at once?
#TASK: Load the nycflights13 dataset. Run the following code to "break" the dataset
#(turning the timestamps into strings) so we can practice parsing date-times.
...
#TASK: Run the following code to convert flight times back to formal time objects
#in the flight_data_parsed dataframe.
...
#HINT: Use the mutate function with the appropriate parsing function form the 
#previous section.
...
#Before exploring more functions in lubridate, lets check out one more useful
#function for setting a date-time value.The 'now()' function allows us to find
#the date-time value for this exact moment in time. 
#TASK: Run the following code to create the 'todays_timestamp' object with the
#'now()' function.

todays_timestamp <- now()

# Lubridate helpful for finding specific components of date-time data.
# TASK: Run the following lines of code to isolate specific components of the 
#date-time value. 
...
#Using the help files, I determined that "week" is the number of weeks since 
#January 1st at the given date plus one and "wday" is simply a numerical for
#which day of the week it is (I got a value of 7 since it is Saturday).

#TASK: What if we wanted to create new column in our flights dataset listing the
#day of the week of the flight? Run the following code to create a flight_day 
#column in our flight_data_parsed dataset.
...
#"Label" indicates that the values in the column should be the name of the weekday
#rather than a number. "abbr" abbreviates the name of the day (ex. "Tue" for "Tuesday").

#TASK: Create a new column called 'flight_month' in the flight_data_parsed dataset
#that lists the names of the months that the flights took place. 
...
# There might be times where you want to round your data. For example, what if 
#you wanted to round to the nearest month? 

# TASK: The functions 'floor_date' and 'ceiling_date' will round down and up to 
#the nearest unit, respectively. Run the following lines of code and take note
#of the output values.

floor_date(mdy("April 15 2026"), "month")
ceiling_date(mdy("April 15 2026"), "month")

# QUESTION: The 'round_date' function is a general rounding function. Run the
#following line of code. Does 'round_date' round up or down?
...
# QUESTION: Run the following line of code. What do you think the 'rollback' function
#does? What do you think the 'roll_to_first' and  'preserve_hms' arguments do?
...
#"roll_to_first" rounds to the nearest 1st day of a month. "preserve_hms" removes
#the hour, minute, and second numbers from the output.

# TASK: Create a dataframe named 'flight_data_rounded' from our 'flight_data_parsed'
#dataframe that includes a column named 'rounded_flight_date' that rounds the flight
#date to the nearest month. 

flight_data_rounded <- flight_data_parsed %>% 
  mutate (rounded_flight_date = rollback(flight_date, roll_to_first = TRUE, preserve_hms = FALSE))
...
# In this section, we will practice creating durations, adding durations to times,
# creating intervals, and measuring how long an interval lasts.

# TASK: Create a smaller dataframe called 'flight_timespans' using the first 20 
# rows of flights.

flight_timespans <- flights[1:20,]
...
# Lubridate creates durations with functions like dseconds(), dminutes(), dhours(),
# and ddays().
...
#It breaks the minute values into minute durations.
...
# TASK: View just these columns (carrier, flight, departure_time, flight_duration,
# arrival_time_estimate).

# QUESTION: What happens when we add flight_duration to departure_time?

#It creates an estimate for arrival time.
...
#The longest flight was 361 minutes long.
...
#You need a beginning and an end time.
...
# TASK: Create an interval from 2:00 PM to 4:50 PM on April 13th, 2026. Use 24-hour
#(military) time with dmy_hms(). Name the dataframe 'class_interval'.
...
# TASK: View only the interval-related columns (carrier, flight, departure_time, 
# arrival_time_estimate, flight_interval).

# QUESTION: What does flight_interval represent?

#It represents the interval between the start and end of the flight, listed as
...
#Because they both measure how long the flight was.

# TASK: Find the longest flight interval in this dataframe.

#361 minutes or 6.02 hours.
...
short_or_long <- flight_timespans %>% 
  mutate(category = if_else(duration_minutes > 179, "long", "short"))

# TASK: Count how many flights are short and how many are long.

#7 are long and 13 are short.

# QUESTION: What is the difference between a duration and an interval?

#A duration is the amount of time between two points while an interval is simply
#represented by the start and end times.

# QUESTION: Explain why intervals are useful.

#Intervals are useful for measuring durations and other values.
...
#We did this to make copies of the start datetime columns we made before, maybe
#so we can manipulate the copies in some way.
...
#I'm not seeing a difference in column one, but the new last column is a duration
#calculation.
...
#It is the duration between the two dates given.
...
#They can help with manipulating dates in datasets where calculations may need
#to be made on them.
...
## Knowledge of time zones is important as the same clock time can have different
#meanings globally.
## Lubridate provides tools for checking, assigning, and converting time zones.
## In this section, we'll practice
## 1) Using tz() to check a time zone ## 2) Converting a time with with_tz() ## 3) 
#Assigning a zone with force_tz() ## 4) Extracting date time elements  
...
#They are 4 hours apart.

##TASK: Determine the time zone for both items.
tz(time_with_tz)
tz(time_force_tz)

#Both are in the America/New York time zone.
...
#This is useful for easily determining when an event takes place across
...
#Because they are different time zones.
...
## TASK: Determine how many of these practice flights depart during the course 
#of each New York hour.
flight_timezones %>%
  count(dep_hour_ny)

## QUESTION: Why is local time better than UTC for human scheduling?

#Because local time is what actually shows on people's clocks.

##QUESTION: In one sentence, explain time zone conversion in lubridate.

#In lubridate, various time zone conversion functions can be used to manipulate
#the time zone of a given date and time.

# ---------------------------------------------------------- #
#### Part 2.0: Practicing your skills                       ####
# ---------------------------------------------------------- 

# Great! Now that you have learned many of the functions that lubridate has to 
# offer, let's put your skills to the test!
...
#TASK: You will notice that this data has a lot of redundant observations. Load
#the "beneficials_unified.csv" into R using the read.csv function and name the 
#dataframe "beneficials".

beneficials <- read_excel("beneficials_unified.xlsx")

# TASK: Using the same workflow, lets make a dataframe of only the data that we 
# will be using for lubridate. 
# (1) name the new dataframe "arthropods" and use the beneficials data
# (2) there is a lot of redundant data and again, we want to make a dataframe that 
# only has the data we will need. For this dataframe, we will only be using the columns
# "arthOrder" through "deployedhours". Use the select function to select all columns
# between (and including) "arthOrder" and "deployedhours". Hint: you should have a
# total of 27 columns after this is done. Revisit previous assignments to select for
# large chunks without having to type all the column names you wish to include. 
# (3) unite the genus and species names and name this new column "species". Separate
# the obsevations with a single space (" ")
# (4) using the select function, remove "family", "genus", "subgenus", "longTrap",
# "latTrap", and "elevTrap"
...
#QUESTION: How many variables does the "arthropods" dataset have? Why does this 
#number differ from the "beneficials" dataset? 

#It has 21 variables. This is because we removed many columns.
...
# (2) using the mutate function make a two new columns called "start_datetime" and
# "end_datetime". Use the "make_datetime()" function to do this. Add the startYear,
#startMonth, startDay, startHour, startMinute to make the "start_datetime". Add the
#endYear, endMonth, endDay, endHour, endMinute to make the "end_datetime".

arthropods_clean <- arthropods %>% 
  mutate(start_datetime = make_datetime(year = startYear, month = startMonth,
                                        day = startDay, hour = startHour,
                                        min = startMinute)) %>% 
  mutate(end_datetime = make_datetime(year = endYear, month = endMonth, day = endDay,
                                      hour = endHour, min = endMinute)) 

# Great! Now we want to parse out the columns that we just made. 

# TASK: First in order to do this, we will need to mess up the columns then put them
# back together. This dataset was already cleaned before it was uploaded. We We will
# "mess" up the data, so we can clean it again. By using "format(start_datetime, "%B %d, %Y")"
# and "format(start_datetime, "%I:%M:%S %p")" we are purposefully changing the clean
# data so we can practice more!
# (1) using the arthropods_clean data, call this new dataframe "arthropods_broken"
# (2) use the format function on the "start_datetime" and make new columns for "start_date",
# "start_time", "end_date" and "end_time" and format the dates using " = format(x, "%B %d, %Y")"
# and for the times use " = format(x, "%I:%M:%S %p")"
...
# (1) call this new dataframe "arthropods_parsed" but pull data from the arthropods_broken
# dataframe. 
# (2) make two new columns using the mutate function. Name these new columns 
# "start_datetime_parsed" and "end_datetime_parsed"
...
#TASK: Let's determine how my months each Order of insect was trapped for. Annotate
#each line of the code below to describe what we are telling R to do. 

total_trap_time <- arthropods %>% 
#Makes a new dataframe using the arthropods data.
  mutate(start_datetime = make_datetime(startYear, startMonth, startDay, startHour, startMinute),
#makes a new column that combines the start day and time columns into one.
         end_datetime   = make_datetime(endYear, endMonth, endDay, endHour, endMinute)) %>%
#makes a new column that combines the end day and time columns into one.
  group_by(order) %>%
#groups the dataframe by the order column.
  summarize(first_trap = min(start_datetime, na.rm = TRUE),
#establishes the beginning of the trap time for each order.
            last_trap  = max(end_datetime, na.rm = TRUE), 
#establishes the end of the trap time for each order.
            time_trapped = interval(first_trap, last_trap) %/% months(1)) %>% 
# HINT: "%/% months(1)" is counting whole months
#makes an interval out of the beginning and end times we established earlier,
#counted in whole months
  ungroup()
#undoes the order grouping from earlier.


#QUESTION: How many months were the traps set for each order?

#araneae- 14 months, coleoptera- 15 months, diptera- 1 month, hymenoptera- 49 months,
#opiliones- 14 months

#TASK: Plot the total_trap time for each order using geom_col. Color fill the bars
#by order. Label each axis, use theme_bw(), and position the legend on the bottom 
#right of the graph. 

ggplot(data = total_trap_time, aes(x = order, y = time_trapped, fill = order))+
  geom_col()+
  theme_bw()+
  theme(legend.position=c(1,0), legend.justification=c(1,0))+
  xlab("Order")+
  ylab("Time Trapped (Months)")
