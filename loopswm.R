...
nums<-length(iris_loop)
for(i in iris_loop) {print(i)}
...
It will only add what you specify to each observation
...
While loops will break once a condition is met, a repeat loop will not break unless that condition is set
While loops are contingent on conditions set, and repeat loops depend on a dataframe. With large dataframes that are hard to look through, repeat loops are more useful
...
plot(x=iris$Sepal.Length, y=iris$Sepal.Width, xlab="Sepal.Length", ylab="Sepal.Width", main='Sepal Length vs Width')
...
for (species in unique(iris$Species)) {
  ...
  for(i in 1:nrow(spdata)) {
    ...
    legend("topright", legend=c("Setosa", "Versicolor", "Virginica"),
           col=c("green","blue","yellow"), pch=15, title= "Species")
