...
#str_pad determines length of all strings, with 15 being the length that is specified.
#"left" and pad=" " shows that spaces will be added to the left side of each string
#so that it is length 15. use_width = false means that it does not matter the width
#as long as the charcater count is 15.
...
#str_trim can remove spaces and "clean" the data and str_pad can help make strings
#the same length to it is a standard size for all strings making it easier to work 
#with the data and easier to visualize or be more aesthetically appealing.
...
#We see there are only two fruits with a true statement returned so only two have the letter q
#in them. The rest of the fruits are false so they do not have the letter q.
...
#Yes there are fruits containing double letters
...
#Counts how many times one or more characters is in a string. In this example,
#the amount of times a is in each string in the fruit vector.
...
#There is no letter "a" in this string.
...
#TO see if the string starts/ends with the designated character string provided 
