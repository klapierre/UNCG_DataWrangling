...
#column? 
#ANSWER: The row names and the data within the "State" column are the same.
...
#ANSWER: The previous code removed the duplication of states in row names and the "States" column by writing rownames as NULL to remove rownames.
...
# ANNOTATED ANSWER: 
ggplot(us_states_frost) + #Selects the us_states_frost dataset for mapping
  geom_sf(aes(fill = Frost), color = "blue") + #Draws the map, selects Frost as the data being plotted, plots using the color blue
  theme_minimal() + #Selects the theme. There are many themes to choose from!
  labs(fill = "Number of Frost Days", title = "United States Frost Data") #Labels the legend and the title of the map
...
#ANSWER: The map is very small making it difficult to visualize and interpret the results. States and islands such as Alaska, Hawaii, Guam, American Samoa, United States Virgin Islands, Puerto Rice, and the Commonwealth of the Northern Mariana Islands may be removed.


#To filter out states or islands not connected to the mainland US, we can use the dplyr function.
...
#ANSWER: Dark blue states represent a lower number of frost days and light blue states represent larger number of frost days.

#Question: Does florida or NC have more frost days?
#ANSWER: NC has more frost days. 

#Queston: Does there appear to be a relationship between latitude/longitude and 
#number of frost days? Why or why not?
#ANSWER: There appears to be a relationship between latitude and number of frost days as you increase latitude there are an increase in frost days. This is due to a colder climate as we move poleward.
...
ggplot(us_states_population) +
  geom_sf(aes(fill = Population), color = "any color") +
  theme_minimal() +
  labs(fill = "relevant to population", title = "Relevant to states and population")


#Task: write three things you can infer from the map that you created:
#1 ANSWER: Any relevant inference
#2 ANSWER: Any relevant inference
#3 ANSWER: Any relevant inference
...
# ANSWER: this function draws a shape based on specific x and y coordinates provided by the dataframe 
...
# TASK: plot the map with your new nc_map dataframe, using the previous map code as your guide
...
# ANSWER: this function adjusts the shape of the map. Without it, the map would be slightly distorted, pulled longer than it appears on an actual map. 
...
# ANSWER: because many data points are overlapping, we can see where this happens a little bit easier than we would if the dots were completely opaque. 
...
# I colored the points by order because if you were to color them by genus or family, we would have a ton of different colored points, and our key would take up a majority of our figure. Particularly with genus, there are 42 different genera without our data set, and it would be pretty inconvenient to try to find 42 significantly different colors for our map. 
...
# ANSWER: It changes the color palette, which can make patterns in the data
# easier or harder to interpret visually.
...
# ANSWER: It helps improve readability and prevents the legend from covering
...
# ANSWER: Higher species richness (more unique genera observed).

# QUESTION: Why might some counties have lower richness values? 
# ANSWER: Possible reasons include less sampling effort, fewer habitats,
# or naturally lower biodiversity.
