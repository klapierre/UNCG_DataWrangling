...
dt_holiday <- as_date(20392)
#The holiday is Halloween (2025-10-31)
...
#I got an output of 2007-12-04 because it encoded 2007 as the year and December. It followed a year-day-month format instead of the month-day-year format. 

mdy("07-04-12") 
mdy("07-04-12") 
#TASK: Use any of the parsing functions to convert your birthday into standard date format in the space below. 
mdy("September 24, 2004")
#I got "2004-09-24"
...
flight_timespans<-flights %>% slice(1:20)
...
#d stands for double in durations (a numeric type that allows decimals and whole number)
...
#dminutes converts the minutes into seconds by multiplying minutes of air_time by 60s
...
#The values in the new column 'arrival_time_estimate' is the output of seconds from 'flight_duration' added to the time from 'departure_time'. 
...
#Flight 1124 (carrier UA) has longest air_time of 361
...
class_interval<-interval(dmy_hms("13-4-2026 14:00:00"),
                         dmy_hms("13-4-2026 16:50:00"))
...
#Flight_interval represents the start-time for departure_time and the end-time for arrival_time_estimate/
...
#Both contain values for the number of minutes of plane's time in air 
# TASK: Find the longest flight interval in this dataframe.
flight_timespans %>%
  select(carrier, flight, flight_interval, duration_minutes, air_time) %>% 
  arrange(desc(air_time))
#Carrier UA, flight 1124 has the longest flight interval of 361 minutes.
...
flight_timespans %>% 
  count(short_or_long)
#7 long flights, 13 short flights
# QUESTION: What is the difference between a duration and an interval?
#A duration is a fixed length for an exact amount of time (ex: minutes). An interval is the time between the start date-time and end date-time.
# QUESTION: Explain why intervals are useful.
#Intervals track real-world time spans like flights and account for time zones and daylight saving. Allows you to calculate arrival times and departure times.
# ---------------------------------------------------------- #
...
#No, the exact moment was changed into the NY timezone (14:00:00 EDT)
...
#Each time is in a different time zone.
##QUESTION: Are these different moments?
#No these are not different moments.
...
# The with_tz() function shows the exact same moment in a different timezone. The force_tz() maintains the same clock time while changing the attached zone.
#time_with_tz the exact moment was not messed with, only the time zone was. In time_force_tz the clock time was the same, except the time zone.
...
#Organizes the data to better anaylze the times. We can convert into different time zones and determine the hour and day for each time.
...
#The hours changed based on the different time zones, however they re are still the exact same moment
...
flight_timezones %>%
  count(dep_hour_ny)
#5 flights depart at 5am
#5 flights depart at 6am
## QUESTION: Why is local time better than UTC for human scheduling?
#The local time is better for human scheduling because it aligns with the sun, human circadian rhythms, and local business hours. The UTC causes daily routines to shift arbitraily, making it impractical for coordinating sleep, work, or social events.

##QUESTION: In one sentence, explain time zone conversion in lubridate.
#If you use "with_tz()" function, the time zone will change but keeping the same point in time, the "force_tz()" function changes the time zone and a different point in time.
# ---------------------------------------------------------- #
