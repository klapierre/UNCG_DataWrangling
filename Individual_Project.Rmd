...
I will be using the packages tidyverse, ggplot2, sf, and rnaturalearth. <br>

Part One: I will tidy the data (making the afro_clean and pala_clean dataframes) before working on my figures. <br>
  - Create a ‘kelvin_to_celsius’ function to use on the temperature values in the dataset. <br>
  - Tidy both dataframes using a pipe with the select (to select the columns of interest: mean temp, max temp, min temp, precipitation,   population density, distance to cities, distance to railroads, distance to roads), rename (renaming columns), mutate with         ‘kelvin_to_celcius’ (on the temperature columns), mutate with as.numeric (on all non-numeric column), drop_na, and mutate(Region = “”)  (changing the values in the Region column to “Afrotropical” or “Palaearctic”) functions. <br>
  
  
Part Two: My first figure will be a bar graph comparing the correlation between select ecological and population factors on WNV incidence in both AFR and PAL regions. <br>
  - Make a wnv_combined dataframe using the bindrows function on afro_clean and pala_clean. <br>
  - Make a wnv_corr dataframe from the wnv_combined dataframe. Group by Region. Use the summarize function to make new correlation columns using the cor command with “complete.obs” (to delete any missing values) to find the correlation between each factor and WNV frequency. Use pivot longer on all columns (except for Region) to turn the variable columns into rows that correspond to their respective correlation in a new “Correlation” column. Use the mutate function to create a “Type” column assigning each variable to “Ecological” or “Population”. <br>
  - Make the bar graphs using ggplot with wnv_corr. Set the x axis as “variable”, using the reorder argument to list them in ascending order based on correlation value. Set the y axis as correlation, filling with ‘Region’ for optimal comparison. Use geom_col to make the bar graphs, and coord_flip to make them horizontal for optimal virtualization. Use facet_wrap to generate two bar graphs for both ecological and population data. Change the appearance of the figure manually with theme functions. <br>
  
  
Part Three: Make a map of the average WNV frequency for all ecological/population variables for both AFR and PAL regions. Uses Z-score to standardise the data for clearer comparison. <br>
  - Make the wnv_avg_corr dataframe from the wnv_corr dataframe. Group by Region and Type, then use the summarize function to find the mean correlation for each Region and Type. Use the mutate function to create a column for Z-scores. <br>
  - Make a wnv_continent_corr dataframe from the wnv_avg_corr dataframe. Use the mutate function to assign AFR and PAL to their respective continents with the case_when function. Make an asia_data dataframe from the wnv_avg_corr dataframe with the filter and mutate to manually assign the PAL values to the asian continent. Update the wnv_continent_corr dataframe by binding the original dataframe with the asia_data dataframe. <br>
  - Create a world dataframe using the ne_countries functions from the sf package. Create a map_data dataframe for the plot by using left_join to join the wnv_continent_corr data with the world data by continent, using the “many-to-many” argument since both dataframes are matching several rows of data. Use the filter function to remove any countries outside of the regions of interest. <br>
  - Use ggplot with map_data to create a map. Use geom_sf to fill the map with zscore data and facet_wrap to make two maps for ecological and population data. Change the appearance of the figure manually with theme functions. <br>
  
  
Part Four: Make pie charts comparing the estimated contribution of each variable on WNV in each region and in both regions combined. <br>
  - Create  a pie_normalized dataframe from wnv_corr that uses the mutate function to make a correlation column including squared correlation values (to make positive); group by Region and use the mutate function to make a percentage column that shows the estimated percent contribution of each factor (contribution of single factor / sum of all factors). <br>
  - Create a combined_pie dataset from the pie_normalized dataset; group by variable and use the summarize function to find the mean percentage for each variable. Use the mutate function to change the “Region” values to “Both.” <br>
  - Create the final_pie_data dataframe using the bindrows function on pie_normalized and combined_pie to combine both datasets. <br>
  - Use ggplot on final_pie_data with y as estimated percentage filled with variable values for optimal identification. Use geom_col and coord_polar to make the pie charts. Use facet_wraps to make three pie charts for each region. Change the appearance of the figure manually with theme functions. <br>
