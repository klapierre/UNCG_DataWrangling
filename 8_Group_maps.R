...
#column? 
# They are all repeated. (There are two copies of each state name within the same column.)
...
head(state_data)
# It deleted the second copy of the names and there are row numbers instead.
...
#Question: What do you notice about the size of this map? Suppose we only want to 
# study frost data of mainland US states. Are there states we could omit from
...
# The map is very small, the scale seems off. We could omit Hawaii and Alaska and 
# any other territories that are not a part of the mainland. 
...
# Darker blue is representative of states with fewer frost days, and lighter 
# ones represent states with more frost days. NC has more frost days than Florida.
#Question: Does there appear to be a relationship between latitude and 
#number of frost days? Why or why not?
# Yes, there is a relationship between the two. As the latitude gets further from the equator it gets colder.
...
state_pop_data <- state_data %>% select(State,Population)
states <- states(cb = TRUE)
states <- states %>% rename(State = NAME)
...
#1 California has the highest population of all of the mainland US states.
#2 New York is the second most populated state.
#3 The midwestern US states are the least populated.
...
# It's drawing out the boundaries of counties on the map.
# TASK: Create a new dataframe titled 'nc_map' that only has North Carolina counties 
# by assigning 'north carolina', typed exactly as it is shown in the in the 'counties' 
# dataframe previously built. The code should look like this, with region following county:
...
# It changes the ratio of the x-axis to the y-axis. It makes the map look to scale.
...
ggplot() +
  ...
  geom_point(data = clean_data,
             ...
             size = 1,
             alpha = 0.7)+
  labs(title = 'Mammal Species Captures in North Carolina',
       x= 'Longitude', 
       y= 'Latitude')
...
# The plot looks totally wrong. There are too many genera. It's better to use order because there are fewer, and they can each be assigned to a distinct color.
...
# It tells R which color palette to use. (The inferno palette is used above.)
# ---------------------------------------------------------------------------- #
...
# In this case, it got the legend out of the way of the title text.
# ---------------------------------------------------------------------------- #
# INTERPRETING THE MAP --------------------------------------------------------

# QUESTION: What does a lighter color indicate on this map?
# Lighter colors indicate more species types in a specific county.
# QUESTION: Why might some counties have lower richness values? 
# They have fewer species in the county due to fewer captures. This might be because of infrastructure, climate, ecosystems, etc.
