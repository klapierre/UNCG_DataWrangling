...
#The info shown in the row names and in the "State" column are repeated and thus
#redundant.
...
head(state_data)
#The previous code got rid of the row names.
...
#This line establishes that we want to make a graph using the us_states_frost data.
  geom_sf(aes(fill = Frost), color = "blue") +
#This line creates the graph itself, fills in each area separated by the "Frost"
#column, and makes the outlines blue.
  theme_minimal() +
#Sets the visual theme to minimal.
  labs(fill = "Number of Frost Days", title = "United States Frost Data")
#Sets the title of the legend and the graph. 
...
#The map is too small to make out. We may want to omit Alaska, Guam, American
#Samoa, Puerto Rico, the Virgin Islands, the Northern Mariana Islands, and Hawaii.
...
#Dark blue states represent fewer frost days and light blue states represent more. 
...
#Visually, it does appear that states with a higher latitude have more frost days.
...
#1: California and New York have the greatest populations.
#2: The midwest is an area with generally low populations.
#3: Population in US states is not obviously correlated with latitude or longitude.
