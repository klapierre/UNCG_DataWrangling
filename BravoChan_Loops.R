...
# conditions which we will later establish are met (Grapple with the grepl() function)
  if(grepl("Width",colnames(iris_loop)[x]))
#
...
#It adds the number 1000 or 50 to the value in the column
...
#It returns only odd numbers from 1 to 19
...
#Repeat loops run the code chunk first, then check the condition to determine whether it should be repeated. The code chunk will be run at least once.
#The repeat loop executes the code block until the condition is changed to FALSE or a break statement is encountered.
#Repeat loops are used when it is unknown how many times the loop must be executed or when the loop must be executed indefinitely until a certain condition is met.
...
plot(x=iris$Sepal.Length,y=iris$Sepal.Width, 
     ...
     main="Scatterplot of Sepal Length vs. Width")
