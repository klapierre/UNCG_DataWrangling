...
#column? 
# Answer: The row names and the new State column have the same state names.
...
head(state_data)
# Answer: The code got rid of the row names from the dataset. Setting them to NULL means the state names are not stored as state names anymore, only the state column keeps that information. 
...
# Answer: (ggplot(us_states_frost_2) +) this line starts the plot using the us_states_frost dataset.
# (geom_sf(aes(fill = Frost), color = "blue") +) this line draws the state shapes and fills them with blue based on the frost values.
# (theme_minimal() +) this line gives a theme to the plot and cleans it up. (labs(fill = "Number of Frost Days", title = "United States Frost Data")) this line creates the legend title and map title. 

#Question: Do you see a map under the "plots" tab?
# Answer: Yes, a map appears in the Plots tab. 
...
#in the dataframe.
# Answer: The map looks small/zoomed out because it includes Alaska, Hawaii etc. We could omit Alaska, Hawaii, Guam, American Samoa, Puerto Rico, United States Virgin Islands, and the Commonwealth of the Northern Mariana Islands to better visualize the mainland US data. 
...
# Answer: The light blue states represent states with more frost days, and the dark blue states represent states with less frost days. NC has more frost days than Florida.

#Question: Does there appear to be a relationship between latitude and 
#number of frost days? Why or why not?
# Answer: Yes, there is a relationship between latitude and number of frost days. States farther north experience more frost days because they experience colder temperatures more often than states closer to the equator. 
...
us_states_population_2 <- us_states_population %>%
  dplyr::filter(State != "Alaska" & State != "Hawaii" & State != "Guam" & State != "American Samoa" & State != "United States Virgin Islands" & State != "Puerto Rico" & State != "Commonwealth of the Northern Mariana Islands")
...
#1 States with darker red/pink colors have higher populations.
#2 Population is not evenly distributed across the United States.
#3 Many of the most populated states are along the coasts. 
...
# Answer: geom_polygon connects coordinate points to draw the shapes of each county, creating the map boundaries. 
...
# Answer: coord_fixed keeps the map proportions normal so it does not look stretched or distorted. 
...
# Answer: Slightly transparent points are useful because many points might overlap a bit in the same area. It also keeps the map from looking too cluttered.
...
# Answer; coloring the points by order is useful because there are there are fewer orders. if the points were colored by genus instead, there would be many more categories, which would make the legend larger and the map more cluttered. 
...
# Answer: Changing the 'option' in scale_fill_viridis_c() changes the color palette used in the map. Different options line inferno adjust the range of colors. 
...
# Answer: Changing the legend position can make the map easier to read. 
# ---------------------------------------------------------------------------- #
# INTERPRETING THE MAP --------------------------------------------------------

# QUESTION: What does a lighter color indicate on this map?
# Answer: A lighter color means higher species richness, meaning more unique species (genera) are in that county. 

# QUESTION: Why might some counties have lower richness values? 
# Answer: Some counties might have lower richness values because fewer samples were collected there, or the environment has fewer species because of factors like habitat, climate etc. 
