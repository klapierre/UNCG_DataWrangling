...
ggplot(world, aes(long, lat, group = group)) +
  geom_polygon(fill = "aquamarine", color = "cornsilk") +
  coord_fixed(1.3) +
  ggtitle ("US Map")
  xlab ("longitude") +
  ylab ("latitude")
...
#Step 8: TASK: Try it yourself. Create a map similar to the one above using the information 
...
plot_usmap(data = countypop, values = "pop_2015", include = c("VA", "NC", "SC", "GA", "FL"), color = "chocolate4") + 
  scale_fill_continuous(low = "white", high = "cyan3", name = "South East", label = scales::comma) + 
  labs(title = "Southwestern States", subtitle = "Population Density for Counties in Southeastern States 2015") +
  theme(legend.position = "right")
...
age_pop_tidy2 <- age_pop %>% 
  rename(region = Location) %>% 
  mutate(region = tolower(region)) %>% 
  select(region, Adults.55.64)
age_pop_states2 <- inner_join(age_pop_tidy2, states, by = "region")
...
ggplot() +
  geom_polygon(data = CA, aes(x=long, y=lat, group=group), fill = "darkolivegreen4", col = "black") +
  coord_fixed(1.5) +
  geom_point(data = starb_CA, aes(x=longitude, y=latitude), color = "antiquewhite" , size = 1, shape = 4)
...
ggplot(global, aes(x = long, y = lat, group = group)) +
  geom_polygon(fill = "antiquewhite", color = "black") +
  coord_fixed(1.3)
...
view(caps)
#country, capital, latitide, longitude, and population, and capital type. 234 observations of these 6 variables.
...
ggplot() +
  ...
  coord_fixed(1.3) +
  geom_point(data = caps, aes(x=Longitude, y=Latitude), color = "red" , size = .5) +
  labs(title = "Global Map with Capital Locations") +
  xlab ("longitude") +
  ylab ("latitude") +
  geom_text(data = caps, aes(x=Longitude, y=Latitude, label = Capital.City), size = 2)
...
#153,439 observations and 39 variables
