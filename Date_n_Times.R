...
invoice_due <- invoice %>% mutate(due_date = day(Due), month = month(Due), year = year(Due))


#This new knowledge still isn't enough. We now want to know if a particular year is a leap year. 

#TASK: Make another new data frame called invoice_leap with the information you find out. Once you have extracted your dates, 
#filter down and find out how many payments were conducted on February 29th? 
#*Hint: the first part of this task is almost identical to what we did in the previous. 


leap_year_get <-  invoice %>% mutate(due_date = day(Due), month = month(Due), year = year(Due), is_leap = leap_year(year)) %>% 
  filter(month == 2 & due_date == 29) %>% select(Due, is_leap) 
...
# For international collaboration or calibrating tools?
...
# To be able to relate gathered data across global scales



# Task Time:)
# Create and name some dates and times named happy_time it should look similar to the E<- that is above!
# Once that is done pick some dates and times of your choice ex. US/Alaska time, GMT, PRC, ect. If you need help
# just remember we have a code that will pull-up all the time-zones:) OlsonNames(tzdir = NULL).
# This should look very similar to the second code named time_1

happy_time <- c("2070-03-07 9:00", "2070-03-08 10:00", "2070-03-28 11:00")
time_2 <- as.POSIXct(happy_time, "America/Belem")
cbind(US=format(time_2),UK=format(t1,tz="Europe/London"))
...
Kruger <- date_time_untidy %>% filter(Site=="Kreuger National Park")
...
USA_1 <- date_time_untidy %>%
  filter(Location=="USA")

NZ_2 <- date_time_untidy %>%
  filter(Location== "New Zealand")

SA_3 <- date_time_untidy %>%
  filter(Location== "South Africa")

Globe_1<- full_join(SA_3, NZ_2)
...
USA_1[c('Month','Day', 'Year')]<-str_split_fixed(USA_1$Date, '/', 3)

#Task: Create the correct function to split the dates of the international data

Globe_1[c('Day','Month', 'Year')]<-str_split_fixed(Globe_1$Date, '/', 3)
...
#Great! Now, we need to format our times. Let's start with the 12-hour format. We can use this formula to change the 12-hour format to the 24-hour.
#Now from 24 hours to 12-hour format

mutate(Time24=format(strptime(USA_tidy$Time.Recorded, "%I_%M %p"), format="%H:%M:%S"))

USA_clean <- USA_1 %>% 
  mutate(Time24=format(strptime(USA_1$`Time Recorded`, "%I_%M %p"), format="%H:%M:%S"))

#Task: Do the same for the International data

Globe_clean <- Globe_1 %>% 
  mutate(Time24=format(strptime(Globe_1$`Time Recorded`, "%H_%M"), format="%H:%M:%S"))
...
whole_clean <-  rbind(USA_clean,Globe_clean)


#Lets plot them all using ggplot!


ggplot(whole_clean, aes(x =Time24, y =`Luz Values`)) + geom_point(aes(color= as.factor(Site))) + xlab("Time") + ylab("Luz Values") + ggtitle("Tidy Data")
