...
library(dplyr)
LongbabyNames<-filter(.data=babyNames, str_length(name)>=6)
...
#Answer: So the first argument is the data set that is being used, then the next is how many characters you want each string to be, next is the direction the space is being added, and pad is what the pad is made of, in this case a space, and then last is if you are considering use_width, and false means we are not.

##TASK: Run str_length on our new vector. Did we successfully make all the lengths
##consistent?
#Answer: Yes, now all of the strings are 15
...
##functions?
#Answer: Str_pad and str_trim functions are for abraviating names or terms perhaps. Maybe if you are trying to make an ID that has a set number of characters you can make sure all of the IDs are of the same length or same format.
...
#Answer: Now all of the strings are 4 characters or lelss.
...
babyNamesTrunc<- mutate(.data=babyNames, name = str_trunc(name,5))
...
#Answer: The result is a vector of three numbers which I believe are the order the fruit is in the fruit vector.
...
#Answer: The string count function appears to tell us how many of the specified character are in each string in the vector
...
#Answer: The fifth fruit returns NAs becuase it contains no 'a's
...
#Answer: The fourth fruit has as in the 2nd, 4th, and 6th positions


## QUESTION: Other functions that are similar to str_detect are str_starts and 
## str_ends. What do you think each of these does?
#Answer: I believe that these functions can determine what strings either start with a given set of characters, or in the other one, which strings end with the given set of characters.
...
#I have zero idea if I did this correctly, but this is what I could figure out considering fruit is just a vector

## QUESTION: How many fruits containing the word "berry" are there in the fruit vector?
#Answer: There are 14 fruits with berry
...
NYC_Baby_Names<-read.csv("Popular_Baby_Names_20260330.csv")%>%
...
#Answer: The output is both a dataset vector that is a list with all of the string character lists, and a console output that is a single object vector that tells us that the highest number of characters is 14
...
NYC_Baby_Names_2021<-filter(.data=NYC_Baby_Names, Year_of_Birth=="2021")
...
#Answer: It is very hard to read, but it does not look like there is
...
#Answer: I think that the ^ is sort of like using a ! so it is saying to look for what is not there, and then becuase stringr is case sensitive there is one of each value so that the code looks for both capital and lowercase
