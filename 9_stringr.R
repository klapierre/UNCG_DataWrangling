...
#ANSWER:str_to_upper(case_names) capitalized all letters
# str_to_lower(case_names) changed all letters to lowercase
# str_to_title(case_names) capitalized the first letter of each word
# str_to_sentence(case_names) capitalized only the first letter of the first word
# str_to_camel(case_names) combined the words and capitalized the first letter of the second word
# str_to_snake(case_names) combined the words and separated them with an underscore
# str_to_kebab(case_names)combined the words and separated them with a hyphen
...
#ANSWER: the results for the first three names are the same as str_camel, but the last one, instead of looking funky, looks how it should using this function. all lowercase but the first letter of the second name capitalized 
...
#ANSWER: yes, but why can str_to_upper and str_to_lower but used and get the same result?
...
uppercaseBabyNames <- babyNames %>% #1
  select(name, year) %>% #2
  mutate(name_upper = str_to_upper(name)) %>% #3
  select(-name) #4
...
oldBabyNames <- babynames %>% #1
  filter(year == 1880) %>% #2
  mutate(name_title = str_to_title(name)) %>% #3
  select(-name) #4
...
#ANSWER: it counts how many letters that we asked in each string. for example it counted how many 'a' were in each work of the first five fruits. 
...
#ANSWER: There are no a's in that word.
...
## (Hint: There are three.)
 ...
 #ANSWER: the 2nd, fourth,and 6th letters are a's.

## QUESTION: Other functions that are similar to str_detect are str_starts and 
## str_ends. What do you think each of these does?

#str_detect() Returns TRUE if a pattern is detected
#str_starts() specialized version of str_dectect. Only looks at the beginning.
#str_ends() specialized version of str_dectect. Only looks at the end.
...
NYC_Baby_Names <- read.csv("C:\\Users\\kelly\\Documents\\data wrangling\\UNCG_DataWrangling\\Popular_Baby_Names_20260405.csv") %>% 
...
Female_ia <- str_count(NYC_Baby_Names_2021_List[["FENALE"]]$Childs_First_Name,"ia")
...
#ANSWER: The carrot says to ignore the vowels and count the consonants. there are lower and upper cased bc the function is case sensitive.
...
taco_ingredients
#ANSWER: all the words run together.
...
taco_ingredients_2 <- str_replace(taco_ingredients, "cilantro", "rice") # I don't like cilantro or guac so I'm replacing it with rice. 
...
taco_ingredients_3 <- str_replace(taco_ingredients, ", beans", "")
taco_ingredients_3
