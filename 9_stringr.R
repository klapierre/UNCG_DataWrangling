...
# Answer:  strlengthtest is the original dataframe, 15 is the target length for each string, "left" is pad on the left side, pad = " " uses spaces to fill, use_width = FALSE counts characters. This whole line makes all strings exactly 15 characters long by adding spaces to the left.

##TASK: Run str_length on our new vector. Did we successfully make all the lengths
##consistent?
str_length(strlengthtest2)
# Answer: Yes, all strings are now length 15. 
...
##functions?
# Answer: str_pad aligns texts and str_trim cleans messy data (removes extra spaces etc).
...
# Answer: Each string is now max 4 characters long (longer ones are shortened with ...). 
...
# Answer: str_to_upper() is all caps. str_to_lower() is all lowercase. str_to_title() the first letter is capitalized. str_to_sentence() first word is capitalized. str_to_camel() camelCase. str_to_snake() snake_case. str_to_kebab() kebab-case. Each function changes how capitalization and spacing are formatted.
...
# Answer: Using str_to_upper() first is weird formatting because everything is capitalized. using str_to_lower() has the camelCase formatting. 
...
# Answer: Yes, it is also possible to fix the last name with str_to_snake() and str_to_kebab() the same way by using str_to_lower() first. 
...
# Answer: It returns the positions (43, 46, 67) of fruits that contain the letter "q".
...
# Answer: The str_count function counts how many times a pattern appears in each string. 
...
# Answer: Because that fruit does not have the letter "a" so there is no position to return. 
...
# Answer: The fourth fruit has three positions where "a" appears (2, 4, 6)

## QUESTION: Other functions that are similar to str_detect are str_starts and 
## str_ends. What do you think each of these does?
# Answer: str_starts() checks if string begins with pattern, and str_ends() checks if string ends with pattern. 
...
berry_count <- fruit %>%
  tibble(fruit_name = .) %>%
  filter(str_detect(fruit_name, "berry")) %>%
  nrow()

berry_count

## QUESTION: How many fruits containing the word "berry" are there in the fruit vector?
# Answer: There are 14 fruits in the fruit vector that contain the word "berry". 
...
NYC_Baby_Names <- read.csv("Popular_Baby_Names_20260414.csv") %>%
...
# Answer: The output is a single number. The highest number of characters contained in a given name is 14. 
...
# Answer: Yes, some names have multiple "ia" occurrences like name number 46 and 136 etc. 
...
# Answer: The value of "Male_ia_Total" is 60. 
...
# ANswer: 118 contain the string "ia". 
...
# Answer: The ^ means not. It counts all characters except vowels. 
...
taco_ingredients
# Answer: No, it is not legible because everything is combined into one long string with no spaces or separators.
...
taco_ingredients_2 <- str_c("tortilla", "meat", "beans", "lettuce", "guacamole", "cheese", "salsa", sep = ", ")
...
taco_ingredients_3 <- str_c("tortilla", "meat", "beans", "guacamole", "cheese", "salsa", sep = ", ")
