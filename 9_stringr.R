...
# 15 is the final string length, "left" specifies what side ofthe string to pad,
# pad= specifies what characters to pad with

##TASK: Run str_length on our new vector. Did we successfully make all the lengths
##consistent?

str_length(strlengthtest2) # All 15!!!!!!!!!!!!!!!
...
# Potential use for standardizing weird data, very context based
...
babyNamesTrunc <- babyNames %>% 
  mutate(names = str_trunc(names, 5))
...
str_to_upper(case_names) # all to upper case

str_to_lower(case_names) # all to lower case

str_to_title(case_names) # Cased Like This

str_to_sentence(case_names) # Cased like this

str_to_camel(case_names) # casedLikethis

str_to_snake(case_names) # cased_like_this

str_to_kebab(case_names) # cased-like-this
...
# The weird upper/lowercase text in the middle of each string is fixed
...
# The ids of each fruit that has the letter q in the name
...
# str_count returns the length of the string
...
# The 5th fruit's name does not contain a lowercase a
...
# These pull the bginning and end of the string,respectively
...
# Our output is a vector of string lengths for all of the names in our
# data frame. The longest  name is 14 characters.
...
str_which(NYC_Baby_Names_2021_List[["MALE"]]$Childs_First_Name,"ia")
# nope
...
# The ^ tells R to look at all cases of the string regardless of case
...
taco_ingredients
# Nope, it's all the words crammed together into one
...
taco_ingredients_2 <- str_replace(taco_ingredients, "guacamole", "cilantro")
...
str_replace(taco_ingredients_2, "beans", "meat, beans")
...
taco_ingredients_3 <- str_remove(taco_ingredients_2, "cheese")
