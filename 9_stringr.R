...
##consistent?
# strlengthest2 assigns the results to a new object named
# str_pad() function that pads each string to a fixed width by adding characters
## on the left, right, or both sides
# strlengthest is the input vector of strings to be padded
# 15: is the target total width for each padded string 
# "left" is the side on which padding is added
# pad "": the character used for padding
# use_width: tells str_pad to measure width using plain character count rather 
## than visual/display width
...
##functions?
# We use str_trim() to remove accidental spaces and str_pad() to format all IDs
## to the same length
...
##QUESTION: How long is each string in the vector now? #4 characters long
...
# str_to_upper() makes everything in all caps
# str_to_lower() makes everything in lowercase 
# str_t0_title() makes the first letter of each word capitalized
# str_to_sentence() makes only the first letter of the sentence capitalized
...
# even though one is capitizaled and the other is lowercase, the final camelCase
...
# Yes, you can fix them the same way. 
...
double_letters <- str_detect(fruit, "(.)\\1")
double_letters
fruit[double_letters]
...
# str_count() function tells you how amny times something appears in your strings
...
# Because it's missing a string input
...
# 2,4,6 positions contain "a" 

## QUESTION: Other functions that are similar to str_detect are str_starts and 
## str_ends. What do you think each of these does?
# str_detect() looks for a pattern anywhere in the string
# str_starts() pattern must be at the beginning
# str_ends() pattern must be at the end
...
library(stringr)
library(dplyr)
berry_count <- sum(str_detect(fruit, "berry"))
berry_count
## QUESTION: How many fruits containing the word "berry" are there in the fruit vector?
# 14 fruits contain the word "berry" in the fruit vector
...
library(dplyr)
NYC_Baby_Names <- read_csv("Popular_Baby_Names_20260330.csv") %>%
  rename(
...
# QUESTION: What does our output look like? #14
...
Mele_ia_Total <- sum(Male_ia)


# QUESTION: What is the value of "Male_ia_Total"? # 60L
...
# QUESTION: How many female names in 2021 contained the string "ia"? # 118L
...
# It means match any character except vowels

# Why do you think that an upper and lower case variant of each vowel is included?
# They are included to handle mixed case 
...
# No, because all the words run together with no spaces or separators
...
taco_ingredients_vec <- c("tortilla", "beans", "lettuce","guacamole"," cheese", "salsa")
taco_ingredients_vec <- append(taco_ingredients_vec, "meat", after = 1)
...
taco_ingredients_3 <- taco_ingredients_vec[taco_ingredients_vec !="cheese"]
