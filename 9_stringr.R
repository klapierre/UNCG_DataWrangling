...
# 15 = makes all strings length 15
# "left" = adds padding to the left side
# pad = " " = uses spaces as padding
# use_width = FALSE = counts characters, not display width

##TASK: Run str_length on our new vector. Did we successfully make all the lengths
##consistent?

## Yes, all strings are now the same length 
...
## They can be used to format text so it lines up evenly and to remove extra spaces from messy data.
...
## Each string is now at most 4 characters long
...
## Each function changed the capitalization or formatting of the text.
# Upper made everything uppercase, lower made everything lowercase,
# title capitalized each word, sentence capitalized the first word,
# and the others changed the format to camelCase, snake_case, and kebab-case.
...
## The result is the same for both because str_to_camel converts everything into camel case, so the original case doesn't matter.
...
## Yes, using str_to_lower first helps fix formatting issues before applying snake or kebab, similar to how we used it with camel.
...
# It returns the positions of the fruits that contain the letter "q".
...
# It counts how many times a pattern appears in each string
...
# The fifth fruit has no "a", so there is no position to return, which gives NA
...
# The positions are 2,4,6

## QUESTION: Other functions that are similar to str_detect are str_starts and 
## str_ends. What do you think each of these does?

# str_starts checks if a string begins with a pattern nd str_ends checks if a string ends with a pattern.
...
# Write the code necessary to do so below, and if necessary check the rename()
# function's help file if you need to refresh yourself on how it works.
NYC_Baby_Names <- read.csv("Popular_Baby_Names.csv") %>%
  rename(
...
## It shows the maximum number of characters in any name.
# The highest number of characters is 14
...
### Yes, some names can have more than one instance of "ia".
...
## The ^ means "not", so it counts everything except the vowels.
# names can have both uppercase and lowercase letters, so we need to include both to count correctly.
#-------------------------------------------------------#
...
taco_ingredients
# It is not very legible because all the words are combined into one string with no spaces.
...
taco_ingredients_2 <- str_replace(taco_ingredients_2, "tortilla", "tortilla, meat")
...
taco_ingredients_3 <- str_remove(taco_ingredients_2, ", salsa")
