...
# Each function changes the capitalization in a different way.
...
# Only the last name is capitalized. 
...
# yes, we can use str_to_lower along with our snake and kebab functions. 
...
# It counts how many times a specific character appears in the string. 
...
# Because there is no a's in the fifth fruits name. 
...
# Str_detect looks for patterns in the string. Str_starts looks in the beginning of the string
# Str_ends looks at the end of the string.
...
# The output is one number and that number is the highest number of characters contained
# in a given name. The max number of characters is 14.
...
# ^ signifies to exclude these characters. Upper and lower case variants of each vowel is included
# to include all vowels regardless of capitalization. 
#-------------------------------------------------------#
...
"tortillabeanslettuceguacamolecheesesalsa"
# It is not legible because there is no space in between each ingredient. 
## TASK: Now make the same vector, but formatted as a list (as in with ", "
## after each word).
taco_ingredients <-str_c(c("tortilla","beans", "lettuce", "guacamole", "cheese", "salsa"), collapse = ", ")
print(taco_ingredients)

## TASK: Not everyone likes cilantro, so use the replace function to
## replace it with guacamole and save it as taco_ingredients_2 (little confused so i will be changing
# guacamole with cilantro since cilantro is not in our current data set)
taco_ingredients_2 <- str_replace(taco_ingredients, "guacamole", "cilantro")
print(taco_ingredients_2)
...
taco_ingredients_2 <- str_sub(taco_ingredients, 1, 9) %>% 
  str_c(" meat,", str_sub(taco_ingredients, 10))
taco_ingredients_2
