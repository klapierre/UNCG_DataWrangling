...
#Approximately 10% of the total observation number (around 20-30)
...
#It changed the placement of the dots on the scatter plot. 
...
#The data points keep being placed in slightly different places. The _jitter part of this command introduces a random variation to each point.
...
#Set1 used the first three colors of the palette.While RdBu used intermediate colors within the palette. 
...
#The colors are a little muted. With gray, pink and blue. Lots of data point overlaps.
# HINT: remember to remove the quotation marks when calling numbers.
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c(628,610,544))
...
color_df <- as.data.frame(colors()) # create a dataframe from the colors() function
color_df[552,] # print the dataframe to see the color names and numbers
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#22f545', '#1b55f5', '#e00724'))
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#22f54500', '#1b55f580', '#e00724FF'))
...
#It pulled the names for the axis from the column header, while the legend with the type of cars, was pulled from the "class" column.
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) + #this line creates a plot from a subset of our original data designates the x, y axes, it tells what variable will be used to apply color;
  geom_jitter() + 
  scale_color_manual(values=c('#FCBA03', '#380754', '#496916'), #manually sets the color for each category in class, 
                     name='Class of Car', #specifies the name of the color scale
                     breaks=c('suv', 'midsize', 'compact'), #specifies the breaks
                     labels=c('SUV', 'Midsize', 'Compact')) #specifies th legends to be used
...
#The labels are listed in the incorrect order, that will cause problems when analyzing the data since it will be incorrectly identified.
...
# above?
#After trying it out and not noticing a difference, I googled it and found that the default 0.5 placement is used.
...
ggplot(data = highwayMPG, aes(x = class, y = hwy_mean, fill = class)) +
  ...
  scale_fill_manual(values = c("#FCBA03", "#380754", "#496916","#eb7023","#eb23b9","#23eb3a","#1be9f7")) +
...
#From the 'class' column where it arranges the cars by type/category
...
# think the coord_cartesian() statement does? It changed the limit of the y-axis. The "coord_cartesian function set the range the axis will have. For example, in this case the y-axis had a limit at 50.
ggplot(data=highwayMPG, aes(x=class, y=hwy_mean, fill=class)) +
  geom_bar(stat='identity') +
  scale_y_continuous(breaks=seq(0, 50, 10)) 
#coord_cartesian(ylim=c(0,50))
...
my_palette <- c("#F8766D", "#00BFC4", "#7CAE00", "#C77CFF", "#C5B0D5", "#FF7F00","#f7f71b")
...
#It gave me a histogram however, it ignored the parameters "binwidth","bins", and "pad".
...
#Violin plots are a mix of box and density plots. It shows peaks in the dataset.
...
ggplot(manufacturerFreq, aes(x="", y=frequency, fill=manufacturer)) + #creates a new ggplot object, assigns variables to each axis, sets the variable to be filled by colors.
  geom_bar(stat="identity", width=1) + #add bar layer with width of 1
  coord_polar(theta="y", start=0) + #change the bar plot into a pie chart plot
  theme_void() + #remove all items that are not data from the plot
  theme(legend.title = element_text(size = 12.5), # customize appearance of legend 
        legend.text  = element_text(size = 8.5), #when it comes to size
        legend.key.size = unit(.75, "lines")) #and position within the plot
