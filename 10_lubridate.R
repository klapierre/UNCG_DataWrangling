...
#Answer: Not it is not correct because it put the  year as 2007 when it should be month 7 or july
...
#Answer: It appears that it rounds up to the first of may.
...
#Answer: rollback prints the last day of the month before todays month. Then the roll_to_first is asking if you want the first day of the month which would have an argument value of TRUE, and FALSE for the last day of the month. Finally, preserve_hms shows the same hour minute and seconds, when true it starts at 00:00:00 which is midnight


rollback(todays_timestamp, roll_to_first = FALSE, preserve_hms = TRUE)
rollback(todays_timestamp)
...
flight_data_parsed <- flight_data_parsed %>% mutate(flight_data_rounded = round_date(flight_date, "month"))
...
flight_timespans <- slice(flights, 1:20)
...
#Ansewr: it seems to take air_time which is originally minutes then convert to seconds 
...
#Answer: It looks like it takes the flight_duration and and adds those hours, but then keeps the date in check
...
#Answer: Flight 1124 has the longest duration
flight_timespans %>%
...
#You need a starting time and an ending time along with the interval function of course
...
class_interval<- interval(dmy_hms("13-04-2026 14:00:00"),dmy_hms("13-04-2026 16:50:00"))
...
#Answer: Flight interval shows the starting day and time of the flight and the ending day and time of the flight.
...
#Answer: because air_time is the same as duration and air_time was made to be in minutes which makes them the same data.
...
#Answer: 7 were long, and 13 were short
flight_timespans %>%
  count(short_or_long)

# QUESTION: What is the difference between a duration and an interval?
#Duration is simply just the amount of time in an interval. So interval is just the start time and end time where as duration is how much time there is inbetween those points.

# QUESTION: Explain why intervals are useful.
#Answer: Intervals are useful because intervals can visualize duration. If you are only given that a flight is 6 hours long, it is easier to understand that you start the flight at noon and will be in your location by 6pm or dinner time.
...
#Answer: Essentially everything is just a number you see a number for a month instead of a specific month for example.
...
#Answer: Now the data is a lot easier to understand because instead of a row of numbers you have columns with the month and day
...
#Answer: Well now there is a new column that is called start_plus_7 that shows the original time plus a week
start_day 
##  question why would this be helpful ? 
#Answer: Now there is a new column that can give an idea of a future date, and it means that in experiments you can compare one time frame to another.
...
#Answer: Now there is a new column that has a date from a month ago 
## why is this useful ?
#Answer: This is helpful ina similar to adding 7 days, now we can compare begginning or start to now, long term changes though time.
...
# dataset 
#Answer: Now ther eis a column that has the full duration of time that passed from the start to the end date.
...
## hint ymd 
my_birthday <- ymd("2005-03-11")
# now we will create data set called my birthday this year 
my_birthday_this_year <- ymd(paste0(year(today_date), "-03-11"))
...
#Answer: This value is either negative for how many days past your birthday, which my birthday was indeed 34 days ago, or if it is positive, it is how many days until your birthday this year.
birthday_time
...
#Answer: 30 days from now will be May 14th
...
## what is the date ?
#Answer: Marth 15 of this year

## question: How is lubridate and arithmetic useful? 
#Answer the lubridate arithmetic is most useful when it comes to being able to compare two times to eachother. This can be if you are trying to predict what something might be in the future. It might be to see if two factors have different levels of results over two periods of time, or it can be to see how data has changed over a long period of time.
