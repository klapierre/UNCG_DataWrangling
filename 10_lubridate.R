...
departure_time_example + dhours(2) #2013-01-01 07:00:00 UTC
...
#Flight 1124 EWR-SFO with airtime=361
...
# A starting date-time and ending date-time
...
# TASK: View only the interval-related columns (carrier, flight, dep_time, arrival_time_estimate, flight_interval).
...
#air_time measures amount of time in the air in minutes and duration measures 
#the time interval between departure_time and arrival_time_esitmats which is total time
#in the air.
...
flight_timespans1 <- flight_timespans %>% 
...
#A duration shows amount of time between events but interval show the start and end 
#time of when an event happens. The duration can be found from the interval by determining
#time between the start and end time of the interval

# QUESTION: Explain why intervals are useful.
#Intervals are useful to telling us when something happens with a date and time.
...
#They are showing different time zones.

##QUESTION: Are these different moments?
#No, these are the same moments, just shown in a different time zone.
...
#with_tz() just switches the time zone with the same moment in time while force_tz() 
#changes the moment in time by keeping the same time and switching time zones.
...
#It is helpful in converting between timezones when trying to do scheduling for personal, 
#school, or business purposes. This could be for appointments, meetings, transportation 
#services, or even deadlines.
...
#They display different hours because it is listed in different time zones but reflect
#the same actual moment in time.
...
#Local time is what most personal electronics, public clocks, and people in a local
#area will use to communicate times and scheduling and it is most convenient in day
#to day life.

##QUESTION: In one sentence, explain time zone conversion in lubridate.
#with_tz within lubridate allows conversion between time zones where the same moment
#in time can be converted to different time zones that may be helpful in scheduling 
#appointments, transportation, deadlines etc to help keep track of time depending on
#the time zone you are in or are going to.
# ---------------------------------------------------------- #
...
#21 variables. This differs from the beneficials dataset because we only selected columns between 
#arthOrder and deployedhours, united two columns, and then removed 5 columns.
...
total_trap_time <- arthropods %>% #use arthropds dataset and name new dataset total_trap_time
  mutate(start_datetime = make_datetime(startYear, startMonth, startDay, startHour, startMinute),
         end_datetime   = make_datetime(endYear, endMonth, endDay, endHour, endMinute)) %>% 
...
group_by(order) %>% #groups by order for summary
  ...
  #time_trapped shows the interval (in months) between first and last trapping  
     ungroup()# ungroups since it was grouped by order in the code above
...
#TASK: Plot the total_trap time for each order using geom_col. Color fill the bars by order. Label each axis, use theme_bw(), and position the legend on the bottom right of the graph. 
  ...
  geom_col() +
    xlab("Order") +
    ylab("Trapping Time by Month") +
    ggtitle("Total Trap Time for each Order") +
    theme_bw() +
