...
# QUESTION: Run the following line of code. What do you think the 'rollback' function does? 
# What do you think the 'roll_to_first' and  'preserve_hms' arguments do?
...
flight_data_rounded <- flight_data_parsed %>% 
  mutate (rounded_flight_date = round_date(ymd(flight_date), "month"))
...
# We get an estimated arrival time.
...
# Carrier- UA, Flight - 1124, Origin - EWR, Destination - SFO, Air time - 361
# 
...
# It represents the entire date and time data between departure and arrival time estimates.
...
time_length(as.duration(practice_interval), "hour") #3.5

time_length(as.duration(practice_interval), "minute") #210
...
# Because both are the time interval between departure and arrival time.

# TASK: Find the longest flight interval in this dataframe.
flight_timespans <- flight_timespans %>%
  arrange (desc(duration_minutes))

# Carrier UA, Flight= 1124, Origin = EWR Destination = SFO, Air-time = 361, Duration in minutes = 361, Duration in hrs = 6.02 
...
# Duration does the length of time.
# Interval is between two time points.

# QUESTION: Explain why intervals are useful.
# To understand what changed between two time points.
...
## TASK: check each new dataset what do you see? ## It has two new columns: start_datetime and end_datetime.
...
# To check if our code worked and also make day of week visible with its name.
...
clean_time_data2
# None of the existing ones changed, but we have a new column start_plus_7 that 
# has 7 days added to what was in start_datetime column.
...
# We have a new column start_minus_1month  that 
# has 1 month deducted from what was in start_datetime column.

## why is this useful ?
# It can help us look at previous dates and can help us compare a time window .
...
# dataset 
# We have a new column that shows duration between end_datetime and start_datetime in days (including decimals)
...
# To calculate how much time has passed since a particular time period or how 
# much time is left for a particilar time period to arrive.
...
# with_tz keeps the same moment of time while force_tz keeps the clock time same
# but changes the timezone.

##TASK: Determine the time zone for both items.
tz(time_with_tz) # America/New_York
tz(time_force_tz) # America/New_York
...
## QUESTION: How is this useful? It helos us determine date and time so it helps!
...
#Because it is displaying different clock time at different cities while it is the
#same moment of time.
...
# It is more relatable to our bodily clock, sleep schedules, working hours, etc.

##QUESTION: In one sentence, explain time zone conversion in lubridate.
# Helps convert actual moment of time or keep the clock time and just change
# the time zone.
...
total_trap_time <- arthropods %>%  #creating new dataframe total_trap_time from arthropods
  mutate(start_datetime = make_datetime(startYear, startMonth, startDay, startHour, startMinute), # creating start_datetime
         end_datetime   = make_datetime(endYear, endMonth, endDay, endHour, endMinute)) %>% # creating end_datetime
  group_by(order) %>% # grouping data by insect order 
  summarize(first_trap = min(start_datetime, na.rm = TRUE), # it is finding the earliest trap date time using minimum function on start_datetime
            last_trap  = max(end_datetime, na.rm = TRUE),  # it is finding the last trap date time using maxium function on end_datetime
            time_trapped = interval(first_trap, last_trap) %/% months(1)) %>% # HINT: "%/% months(1)" is counting whole months # it is counting full months trapped
  ungroup()
...
total_trap_time %>% 
  ggplot(aes(x = order, y = time_trapped, fill = order)) +
  geom_col() +
  labs(
    x = "Insect Order",
    y = "Total Months Trapped",
    title = "Total Trap Time by Insect Order"
  ) +
  theme_bw() +
