...
## It corresponds to Halloween 2025
...
## 24 hours are in 86,400 seconds 
...
## No, the output is not correct, because it reads it in the format of year-day-month 
mdy("07-04-12") 
## Correct one 
mdy("07-04-12")
...
mdy("February 4, 2005") # My birthday
...
flight_data_parsed <- broken_flights%>%
  mutate(
    flight_time = hms::as_hms(flight_time),
...
date(todays_timestamp) # the full date 
year(todays_timestamp) # just the year
month(todays_timestamp) # the month
day(todays_timestamp) # the day of the month
hour(todays_timestamp) # the hour
minute(todays_timestamp) # the minutes 
second(todays_timestamp) # the seconds 
...
## label means give me words not numbers and abbr means make it shorter or keep it long
...
## rollback() function adjusts dates/times backwards when an operation causes something to "overflow" or become invalid.
## roll_to_first() controls where the data gets rolled to within the month and preserve_hms() controls whether
...
flight_data_rounded <- flight_data_parsed %>%
  mutate(rounded_flight_date = lubridate::round_date(flight_date, unit = "month"))
...
library(dplyr)
flight_timespans <- flight_data_parsed%>%
  slice(1:20)
...
# takes the values in the air_time column and converts each one into a duration object measures in minutes
...
# QUESTION: What happens when we add flight_duration to departure_time? # calculates the arrival time
...
# QUESTION: What two things do you need to make an interval? #  a start and end time
...
# QUESTION: What does flight_interval represent? # the actual time span of each flight from start to finish 
...
# QUESTION: Why are the columns air_time and duration_minutes the same? # the code made them the same thing but with different labels. 
...
# QUESTION: Explain why intervals are useful. # they allow us to represent, measure, and analyze the span between two points in time as a single object
...
## Question: why did we do this step? # to extract individual components of time to make it easier to analyze the patterns
...
## why is this useful? # it allows us to compare each datetime to a previous time period
...
## TASK: View this dataset you just create ? what does this value mean? # it represents the number of days between today's date and my birthday this year; basically how far away the date is
...
## Both allow you to easily calculate, manipulate, and analyze time-based data using operations like subtraction, addition, and intervals
# ---------------------------------------------------------- #
#### Part 1.3: Time Zones                                 ####
# ---------------------------------------------------------- #
...
## QUESTION: What does the 'tz' argument produce? # produces a datetime that is anchored to a specific time zone 
...
##QUESTION: Why do the shown clock times appear different? # each time zone has a different offset from UTC


##QUESTION: Are these different moments? # no, they represent the same moment in time, just displayed in different time zones
...
## with_tz() keeps the moment the same, changes the display 
## force_tz() keeps the clock time the same, changes the meaning
...
## QUESTION: How is this useful? # allows us to extract the hour and weekday from datetime values
...
## QUESTION: Why is local time better than UTC for human scheduling? # it aligns with daily human activities and requires no time zone conversion


##QUESTION: In one sentence, explain time zone conversion in lubridate.
## allows you to change how a moment in time is displayed in different time zones 
...
total_trap_time <- arthropods %>% # start with the arthropods dataset and pass it into the pipeline
  mutate(start_datetime = make_datetime(startYear, startMonth, startDay, startHour, startMinute),# create a new column that combines separate date + time columns into one dataset
    end_datetime   = make_datetime(endYear, endMonth, endDay, endHour, endMinute)) %>% # create another datatime column for when trapping ended
  group_by(order) %>% # group the data by insect Order so calculations are done for each group separately
  summarize(first_trap = min(start_datetime, na.rm = TRUE), # find the earliest start time for each Order
    last_trap  = max(end_datetime, na.rm = TRUE), # fins the latest end time for each Order
    time_trapped = interval(first_trap, last_trap) %/% months(1)) %>% # HINT: "%/% months(1)" is counting whole months
  ungroup() # remove grouping so the dataframe returns to normal 
