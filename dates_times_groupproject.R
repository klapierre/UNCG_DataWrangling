...
lubridate::today() #prints today's date
Sys.time() #system date and time
lubridate::now() #time right now
lubridate::now(tzone = "UTC") #time right now in the UTC time zone
lubridate::leap_year(today()) #answers if this year is a leap year
...
today_date #I hope so
 

#Check the class of today_date
class(today_date) #Mine says date, idk if thats right tho
...
year(today_date) #date year
month(today_date) #date month
leap_year(today_date) #Leap year? no
month(12, label = TRUE) #prints month and the order
?ymd_hms() #Transform dates stored as character or numeric vectors to POSIXct objects.
?hour(today_date) #get/set hour component of date-time
second(today_date) ##get/set second component of date-time
...
invoice_due <- invoice_delay %>% 
  group_by(Due) %>% 
  mutate(Day = day(Due)) %>% 
  mutate(Month = month(Due)) %>% 
  mutate(Year =year(Due)) %>% 
  ungroup()
