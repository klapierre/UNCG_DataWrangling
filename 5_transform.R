...
# column?
#Each column (calispell_length, smalle_length, winchester_length) contains the number of observations for each column (61100).

# QUESTION: How does this number compare to the number of observations listed by
# the dataframe in the R environment tab?
#The streamTempLength dataframe in the R environment tab lists 1 observation. Each column of the dataset lists 61100 observations.

# QUESTION: Based on your previous answers, what do you think the length 
# function does?
#I think the length function can lists the number of total observations for each column. 
...
#The first line looks at the 'streamTemp' dataset and creates a new dataset called 'streamTempLength'. The second line would identify the columns ('calispell','smalle', and 'winchester') and summarize for the length or number of observations for each column. 
...
# function above?
#Based on the code above, 'maximum', 'minimum', and 'mean' are listed for each column ('calispell', 'smalle', and 'winchester').

# QUESTION: What values do you see for the columns when you open up the 
# dataframe streamTempSummary? Why do you think this is?
#'NA' is listed under each column in the streamTempSummary dataframe. I think the function for max, mean, and min equations was not part of the code above. For now R thinks max, min, and mean are just terms and not mathematical significant. Also, R does not know how to take the mean, max, and min of 'NA' or missing data.  
...
# our data?
#Under 'calispell_maximum' 22.38 is listed, 'calispell_mean' contains 7.985702, and 'calispell_minimum' contains -0.28. 'smalle_maximum' contains 20.05, 'smalle_mean' contains 5.928555, and 'smalle_minimum' contains -0.1. 'winchester_maximum' contains 18.65, 'winchester_mean' contains 6.089913, and 'winchester_minimum' contains -1.75. The last line 'na.rm=T' removed the NAs from our data.

# QUESTION: What happened to the column we created in the beginning called 
# data_type? Where did the date and time columns go?
#The date and time columns are listed in the streamTemp dataframe. We did not list date and time in our code for summary dataframe to avoid confusion with R when calculating for mean, max, and min of calispell, smalle, and winchester.
...
#We should avoid LGA airport if we want the shortest delays.
...
timeDelaySummary<-flightData %>% 
  filter(dest=="RDU") %>% 
  group_by(hour) %>% 
  summarize(across(.cols=('arr_delay'),.fns=list(mean, max),na.rm=T)) %>% 
  ungroup()

# QUESTION: What is the earliest hour of the day that flights leave New York for 
# Raleigh?
#Six is the ealiest hour of the day. 

# QUESTION: Which hour of the day has the longest mean delay? What about the 
# longest maximum delay?
#At hour 22, it has the longest mean delay (65.5). At hour 12, it has the longest maximum delay (430).
...
#EWR in March has the longest mean delay (30.2).
# ---------------------------------------------------------- #
### PART 2.0: INTRO TO TIDY DATA                          ####
# ---------------------------------------------------------- #

# QUESTION: What are three characteristics of tidy data?
#Each variable forms a column, each observation forms a row, and each cell is a single measurement.
...
rm(list = ls())
read.csv("Niwot_Salix_2014_WillowSeedlingSurvey.csv")
...
# into R.
#I think 'skip=10' means to not enter data for row 10 and beyond. 
...
#We would want to clean up and fill in the block column since it has a lot of NAs.
...
# function above?
#The code 'block:temp' means every NA should be filled in between the block column to temp column.

# QUESTION: Looking at the dataframe willowFill, describe what happened compared 
# to our initial dataframe.
#In the willowFill dataset, every cell in the block column was replaced by '1'. In the willow dataframe, the block column contain many NAs and a couple 1's.
...
#PlotInfo dataframe should contain the following columns: block, plot. code, snow, n, and temp. The willowData dataframe should contain the following columns: block, plot, willow_ID, ht1, ht2, cnpy1, cnpy2, notes, status, and year.
# Let's do it! Run the following code:
plotInfo <- willowClean4 %>%
  select(block:temp) %>%
  unique()
plotInfo <- willowClean4 %>%
  select(block:temp) %>%
  unique()
willowData <- willowClean4 %>%
...
#     different columns
read.csv("e001_Plant aboveground biomass carbon and nitrogen.csv")
