...
# It isnt included because we didn't summarize them because they werent grouping other 
#variables they weren't summarized.
...
streamTempMDY <- streamTemp %>% separate(data, into = c("month", "day", "year"), 
  sep = "/") %>% mutate(year = paste0("20", year))
...
streamTempJan <- streamTempMDY %>% filter(as.integer(month) == 1)
...
jan_means <- streamTempJan %>% summarize(calispell_mean = mean(calispell, na.rm = TRUE),
  smalle_mean = mean(smalle, na.rm = TRUE), winchester_mean = mean(winchester, na.rm = TRUE))
...
#There are 12 means for each stream

# QUESTION: In your own words, what do you think the group_by() function does 
# when used before the summarize() function?

#The group by function before the summarize function means to summarize each group separate than
#each other
...
#We grouped by the year and the month, the dataframe now shows the temperature mean for 
#calispell, smalle, winchester
...
airportDelaySummary <- flightData %>% filter(dest == "RDU") %>% group_by(origin) %>%
  summarize(mean_arr_delay = mean(arr_delay, na.rm =T)) %>% ungroup()
...
timeDelaySummary <- flightData %>% filter(dest == "RDU") %>% group_by(hour) %>% 
...
# The hour 22 has the longest mean delay and hour 12 has the longest maximum delay
...
monthlyDelaySummary <- flightData %>% filter(dest == "RDU") %>% group_by(month, origin) %>% 
  summarize(mean_arr_delay = mean(arr_delay, na.rm =T)) %>% ungroup()

# QUESTION: Which month and airport has the longest mean delay?

# March at the EWR airport has the longest mean delay
...
#The block, plot, code, snow, n, and temp, almost every column
...
#It means to fill in the columns from block to temp

# QUESTION: Looking at the dataframe willowFill, describe what happened compared 
# to our initial dataframe.

#In willowfill the columns with the NA's were replaced/filled so there were no longer gaps in the data
...
#in plotInfo everything from block to temp should be in it and for willowData everything
#from the willow_ID data to the year shold be included.
...
willowDataTrt <- willowData %>% left_join(plotInfo, by = c("block", "plot"))
...
cdr <- read_csv("e001_Plant aboveground biomass carbon and nitrogen.csv") %>% 
  rename(C = `% Carbon`,N = `% Nitrogen`) %>% filter(Strip %in% c("1", "2")) %>% 
