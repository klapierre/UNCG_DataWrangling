...
# function above?
# R uses the original names provided and they are kept because of the length function.
...
timeDelaySummary <- flightData %>%
  ...
  group_by(hour) %>%
  summarize(mean_arrival_delay = mean(arr_delay, na.rm = TRUE),
            max_arrival_delay = max(arr_delay, na.rm = TRUE)) %>%
   ungroup()
...
# 22 hour has the longest mean delay and hour 12 has the longest maximum delay. 
...
# Month 3 and airport EWR has the longest mean delay. 
...
# The three characteristics of tidy data are that variables form columns, observations
#form rows and observational units form tables. 
...
# into R.
# Skip 10 tells R to skip the first 10 rows of the original data. 
# ---------------------------------------------------------- #
...
# plotInfo will have block, plot, treatment and temp 
# willow data will have block, plot, treatment, height and year. 
