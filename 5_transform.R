...
# functions.
streamTempRenameSep <- streamTempRename %>%
  tidyr::separate(
    col = Date,
    into = c("Month","Day","Year"),
    sep = "/"
    )
streamTempMDY <- streamTempRenameSep %>%
...
streamTempJanAvg <- streamTempJan %>%
     summarize(across(c(calispell, smalle, winchester), mean, na.rm = TRUE))
...
# It tells R which things to group, either by looking at characters or numbers
...
# dataframe show?
# Now we have averages for a given month in each year rather than months across
...
# 22:00 for longest mean delay, 12:00 for longest max delay.
...
# A column should only contain a single variable, with rows delineating observations,
# each cell should be only a single observation.
...
rm(list = ls())
willow <- read.csv("C:/Users/nsalt/Documents/DataWrangling/UNCG_DataWrangling/Niwot_Salix_2014_WillowSeedlingSurvey.csv", skip = 10)
...
# into R.
# The skip function means the read.csv isn't reading the first 10 rows.
...
# function above?
# It tells R to fill in the NAs in the columns from block to temp.

# QUESTION: Looking at the dataframe willowFill, describe what happened compared 
# to our initial dataframe.
# The NAs have been replaced with the equivalent block number.
...
willowClean <- willowFill %>% # Tells what dataframe to use / new name
  pivot_longer(cols = w_1:w_C, #pivots the columns from willow fill into rows
               names_to = "willow_id", #sets the name for the new column
               values_to = "value") %>% #takes the data placed under w1:w_C and places it next to the willow id
  separate(col = willow_id, #
           into = c("remove", "willow_ID"), #creates two columns from willow_id
           sep = "_") %>% #tells which inputs to use for each column
  select(-remove) #Deletes the "remove" column
...
# for these variables?
# The value column contains multiple variables, while the variable column tells us the date.
...
willowCleaner  <- willowClean %>% #Tells what dataframe to use / new name
  pivot_wider(names_from = variable, #Tells R to use the names from the variable column to create new columns
              values_from = value) #Tells R which set to correspond with the new column
...
# (i.e., pseudocode).
# It would be necessary to create a new column which gives us information on
# whether the plant is alive or dead and then a column containing the information
# on the plants height should it be alive. 
...
# of the syntax.
# If else has three segments, a logical expression and lets you assign a value
# when both the logical expression is true and when it is false.
...
willowClean3 <- willowCleaner %>%
  ...
  mutate(ht1 = ifelse(status == 'dead', NA, ht1)) #cells that say dead are made into NAs, while those not saying it retain their previous value
