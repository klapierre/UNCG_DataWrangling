...
##Gives the number of values in a column
...
# the following code:
streamTempLength <- streamTemp %>%  ##Take the streamTemp dataframe
  summarize(across(.cols=c('calispell', 'smalle', 'winchester'), ##Create a new summary table choosing these columns
                   .fns=length)) ##Count how many values are in each column
...
##Combines the original name with the name of the function

# QUESTION: What values do you see for the columns when you open up the 
# dataframe streamTempSummary? Why do you think this is?

##NA values 
##Because the columns have NA in them we cannot calculate mean, max, etc.
...
willowClean <- willowFill %>% ##Start with willowFill
  pivot_longer(cols = w_1:w_C, ##Select these columns
               names_to = "willow_id", ##old column names into willow_id
               values_to = "value") %>% ##data goes into column called value
  separate(col = willow_id, ##Split this column
           into = c("remove", "willow_ID"), ##into these two columns
           sep = "_") %>% ##at the underscore
  select(-remove) ##get rid of remove column
...
willowCleaner  <- willowClean %>% ##Take willowClean
  pivot_wider(names_from = variable, ##Turn the variable names into columns
              values_from = value) ##Fill the new columns with the values from the value column
...
##The values such as ht1, ht2, etc. in the variable column are now their own separate columns.
