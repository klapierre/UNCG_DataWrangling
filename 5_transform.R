...
summarize(.data=streamTempJan, across(.cols=c('calispell', 'smalle', 'winchester'), 
                 .fns= list(maximum=max, mean=mean, minimim=min),
                 na.rm=T))
...
# each stream?
# There are 12 different mean values for each stream. One for each month.

# QUESTION: In your own words, what do you think the group_by() function does when used
# before the summarize() function?
# The group_by() function allows us to specify which group by which we wish to summarize and use a variety of functions on.
...
# We grouped our means by our Months and then Years. The new data frame shows 
# our means, maximums, and minimums from the highest Month to lowest Month
...
# I would avoid EWR. I would want to avoid Newark
...
Earliest_deptime <- flightData%>%
  filter(dest=="RDU")%>%
  summarize(min(dep_time, na.rm=TRUE))

# QUESTION: Which hour of the day has the longest mean delay? What about the longest maximum delay?
Longest_meantime <- flightData%>%
  group_by(hour)%>%
  summarize(across(.cols=c('dep_delay'), 
                          .fns=list(mean,max=max),
                          na.rm=T)) %>% 
              ungroup()
...
# 1. Each column contains a variable
# 2. Each observation is formed into a row
# 3. Each cell contains a measurement.
...
# It seems as though one row, likely row 10 would be skipped.
...
# have lots of NAs.
# A lot of the columns have NA's, but primarily those for Plot, Code, Snow, and Temp.
...
# The ":" tells R to fill in columns Block-Temp in the fill() function.

# QUESTION: Looking at the dataframe willowFill, describe what happened compared to our initial dataframe.
# All of the columns with a lot of empty space had their cells filled in with appropriate data.
...
# The "value" column contains multiple variables, with the variables columns indicating the corresponding data.
...
# The values column has been converted into new rows and columns that stretch out, widening the dataframe
# into more specific data.
# For pivot_wider, the number of columns is increased while the rows decrease.
# The names_from specifies which column to grab the names of the values from,
# while the values_from indicates which values to place into the cells.
...
# We would want to parse out the data of "Dead vs. Alive" and the heights of the living specimens.
# Then we would want to parse out the information of the willow_ID's and the dates when the seedlings were planted.
...
# ifelse allows you to test if a statement is true or false within a column or value.
# The syntax goes ("What you're looking for", "The value that should come back as True", "The value which will come back as False")
...
# The mutate() function creates a new column named "status" that has an ifelse effect,
# The ifelse function states that if ht1= to dead then to keep it as dead, but if it does not come back as dead, to change it to alive.
#In the second mutate(), ht1 has an ifelse function which states that if status equals to dead
# then to keep it, but if it comes back as NA use it for the height, but anything not dead should be used for ht1
...
str(willowClean3)
#ht1 now only includes data which measures height.
...
# Given this information, it seems as though those identified in 2006 were identified with letters, and those in 2007 with numbers
