...
# function does?
##ANSWER: I think that it determines the total number of observations in a column. 
...
# Assigns the following function to the 'streamTempLength' dataframe and starts a pipe for the streamTemp dataframe.
streamTempLength <- streamTemp %>% 
  # Uses the 'summarize' function to create a new, single row for each column (using the 'across' function).
  summarize(across(.cols=c('calispell', 'smalle', 'winchester'), 
                   # Tells the 'summarize' and 'across' functions to sumarize on the basis of length. 
                   .fns=length))
...
# function above?
##ANSWER: R names the columns by adding an underscore in front of the names that are assigned to the mathematical functions for R to perform (ex. calispell_maxiumum takes its name from maximum=max).

# QUESTION: What values do you see for the columns when you open up the 
# dataframe streamTempSummary? Why do you think this is?
#ANSWER: It says NA, I think that this is because there are several NA values in the dataframe, and R does not know how to assign values to them. 
...
# our data?
#ANSWER: calispell_maximum = 22.38 calispell_mean = 7.99 calispell_minimim = -0.28 smalle_maximum = 20.05 smalle_mean = 5.93 smalle_minimim = -0.1 winchester_maximum = 18.65 winchester_mean = 6.09 winchester_minimu = -1.75. The 'na.rm = T' line removed the NA values from the dataset.


# QUESTION: What happened to the column we created in the beginning called 
# data_type? Where did the date and time columns go?
#ANSWER: The date and time were not included in the new dataset because they were not included in the 'across' function, but they remain in the original dataset. 
...
streamTempMDY <- streamTemp %>% separate(Date, c('month', 'day', 'year'), sep = '/') %>% mutate(year = paste('20', year, sep =""))
...
#ANSWER: The block, plot, code, snow, n and temp columns have a lot of NAs. 
...
# function above?
#ANSWER: 'block:temp' means that the function will apply to all of the columns from block through temp. 

# QUESTION: Looking at the dataframe willowFill, describe what happened compared 
# to our initial dataframe.
#ANSWER: The 'NA' values were replaces with X in the block through temp columns. 
...
willowClean <- willowFill %>%
  # Creates the willowClean dataframe from the willowFill dataframe.
  pivot_longer(cols = w_1:w_C,
              # Applies the pivot_longer function to the w_1 through w_C columns.
               names_to = "willow_id",
              # Converts the names of the columns to a column called 'willow_id.'
               values_to = "value") %>%
              # Converts the values of the columns to a column called 'value.'
  separate(col = willow_id,
           into = c("remove", "willow_ID"),
           sep = "_") %>%
  # Separates the column 'willow_id' into new 'remove' and 'willow_ID' columns, separated via an   underscore.
  select(-remove)
  # Removes the new 'remove' column from the dataset. 
...
# for these variables?
#ANSWER: The willow_ID column contains labels telling us there are multiple variables stored in one column, while the value column contains the corresponding data. 
...
willowCleaner  <- willowClean %>%
  # Creates the new 'willowCleaner' dataframe from the willowClean dataframe.
  pivot_wider(names_from = variable,
              values_from = value)
              # Separates out the variables from the 'variable' column into their own individual               columns, and gives them their values from the 'value' column. 
...
# (i.e., pseudocode).
# I would want to take the alive/dead data from the ht1 column to give that data its own column while leaving height in the ht1 column. I would want to take the time information from the willow_ID column and give that its own column as well. 
...
# of the syntax.
# Within the ifelse function you first specify the 'test' that you want to apply to the dataset, something that should result in true or false. 'Yes' indicates the return values for the test if it results in TRUE, while 'no' indicates the desired return value for the test if it results in FALSE.
...
willowClean3 <- willowCleaner %>%
  # Creates a new dataset named willowClean3 from willowCleaner (changed name since willowClean2   does not exist). 
  mutate(status = ifelse(ht1 == 'dead', 'dead', 'alive')) %>% 
  # Uses the mutate function to make a new column called 'status.' The ifelse function tests for   whether the tree is dead, and assigns it the 'dead' value if true and the 'alive' value if     false.
  mutate(ht1 = ifelse(status == 'dead', NA, ht1))
  # Uses the mutate function to alter the 'ht1' column. The ifelse function tests for   whether   the tree is dead, and assigns it the 'NA' value if true and its original ht1 value if false.
...
str(willowClean3) # The ht1 column's class is character. 
...
# ANSWER: The willow seedlings from A, B, and C were planted in 2006 while the rest of the seedlings were planted in 2007. 
...
# ANSWER: 'plotInfo' will contain the columns relating to the conditions of the plot (block, plot, code, snow, n, temp), while 'willowData' will contain the columns relating to the willow saplings' conditions (block, plot, willow_ID, ht1, ht2, cnpy1, cnpy2, notes, status, year).
...
cdr <- read.csv('e001_Plant aboveground biomass carbon and nitrogen.csv') %>% rename('C' = X..Carbon, 'N' = X..Nitrogen) %>% filter(Strip %in% c(1, 2)) %>% pivot_longer(cols = C:N, names_to = 'element', values_to = 'percentage') %>% group_by(Date, Plot, NTrt, Species, Field, Strip, element) %>% summarize(percentage_mean = mean(percentage)) %>% ungroup() %>% pivot_wider(names_from = element, values_from = percentage_mean)
