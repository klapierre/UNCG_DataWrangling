...
# column?
# there is a single value of 61100 in each column
...
# function does?
#tells you how many items are contained within that variable
...
# the following code:
streamTempLength <- streamTemp %>% #pulls data from streamTemp and creates a new dataframe named streamTempLength
  summarize(across(.cols=c('calispell', 'smalle', 'winchester'), #applies a function across the three columns
                   .fns=length)) #specifies which function to use
...
# function above?
# it combines each of the original column names with the name of each function

# QUESTION: What values do you see for the columns when you open up the 
# dataframe streamTempSummary? Why do you think this is?
# only NAs in each column
# because the original dataset contained missing values which can't be used mathematically
...
# our data?
# Now there are numerical values in each column.The last line removed the NAs.

# QUESTION: What happened to the column we created in the beginning called 
# data_type? Where did the date and time columns go?
# We didn't create that column in this assignment as far as I can tell. However, 
# the data and time columns aren't really numerical values that calculations can be run on
# so they were not included in the previous code.
...
# it divides the data into group so that the summarize function is performed separately on each group
...
# dataframe show?
# We grouped by the month and the year. It shows the average mean temps for each month in each year that data was recorded.
...
# variables are in columns, observations are in rows, and each cell contains one measurement
...
# into R.
# it took out the first ten lines of the excel sheet that were just for information about the dataframe
...
# the plant identifier column and the columns below dead plants
...
# function above?
# it means to fill each of the cells in each of the columns 
# from the block column to the temp column

# QUESTION: Looking at the dataframe willowFill, describe what happened compared 
# to our initial dataframe.
# In the plant identifier columns, each cell was filled with the same data from 
# the most recent previously filled cell above it.
...
willowClean <- willowFill %>% 
  # uses data from the willowFill dataframe
  pivot_longer(cols = w_1:w_C,
               names_to = "willow_id",
               values_to = "value") %>% 
  #pivots the w1 to wC column data from horizontal to vertical, creating a 
  #column called value that notes the values of each variable for each row in the
  #willow id column that specific plant
  separate(col = willow_id,
           into = c("remove", "willow_ID"),
           sep = "_") %>%
  #separates the 'w' from the number/letter that identifies the plant into two 
  #columns called remove and willow_ID
  select(-remove)
# deletes the remove column containing the letter 'w'
