...
lubridate::today() #this will show you todays date
Sys.time() # this will show you the systems current date with and without time
lubridate::now() # this shows you todays date, the current time, and your timezone
lubridate::now(tzone = "UTC") # this will show you the data nd time in the UTC timezone (universal time)
lubridate::leap_year(today()) # this will tell you if the current year is a leap year
...
class(Sys.Date()) # Date
class(lubridate::now()) # POSIXct and POSIXt
...
#Great, now that we've got some of the basics for dates, lets detour to look at the basics of time. Below are some basic commands for times and a short example. Annotate by each what they do on each line. 
day() #Get the amount of days or set the amount of days.  
year() #Get the amount of years or set the amount of years.
month() #Get the amount of months or set the amount of months. 
leap_year() #Ask R if the given year is a leap year. 
month(12, label = TRUE) #Sets a 12 month year with labels. 
ymd_hms() # turns dates stored as character/numeric vectors into POSIXct objects. year, month, day, hour, minute, and second components
hour() # get or set hours component of a date-time
second() # get or set seconds component of a date-time
...
invoice_due <- invoice %>% 
  mutate(dueDate=day(Due), dueMonth=month(Due), dueYear=year(Due))
...
invoice_leap <- invoice %>% 
  mutate(dueDate=day(Due), dueMonth=month(Due), dueYear=year(Due), leapYear=leap_year(dueYear)) %>% 
  filter(dueMonth==2 & dueDate==29)
## 4 payments were conducted on Feb. 29th
...
## if you are working with people from multiple different timezones, it would be smart to set one as the standard so everyone is aware of when meetings are or when something they're working on is due.
...
## If you are working with people from different timezones, it would be very convenient if everyone had access to due dates for things, meeting times, etc. in their own time zone rather than converting it themselves.
...
G <- c("2011-05-15 12:00", "2001-05-07 12:00", "2001-09-27 12:00", "2000-11-4 12:00")

happy_time <- as.POSIXct(G,"Canada/Central")
cbind(Canada=format(happy_time),Helsinki=format(happy_time,tz="Europe/Helsinki"))
...
##USA - month/day/year (I've always thought we're weird)
##South Africa - depends which part but I think Kreuger follows Day/Month/Year
...
USA_dt <- date_time_untidy %>% 
  filter(Location=="USA")

NZ_SA_dt <- date_time_untidy %>% 
  filter(Location %in% c("New Zealand", "South Africa"))
...
USA_dt[c("Month", "Day", "Year")] <- str_split_fixed(USA_dt$Date, "/", 3)
NZ_SA_dt[c("Day", "Month", "Year")] <- str_split_fixed(NZ_SA_dt$Date, "/", 3)

#Question: Why does this even matter?
## So you can see the date split into columns so it eliminates the confusion when putting the dates back together into one data frame.


#Task: Decide what format you want to put the dates into. Hint:The paste() function may be useful here
USA_dt<-USA_dt %>%
  mutate(DayMonthYear=paste(USA_dt$Day, USA_dt$Month, USA_dt$Year, sep='/')) %>% 
  select(Site:Time.Recorded, DayMonthYear)
NZ_SA_dt <- NZ_SA_dt %>%
  mutate(DayMonthYear=paste(NZ_SA_dt$Day, NZ_SA_dt$Month, NZ_SA_dt$Year, sep='/')) %>% 
  select(Site:Time.Recorded, DayMonthYear)
...
USA_dt_time <- mutate(Time24=format(strptime(USA_dt$Time.Recorded, "%I_%M %p"), format="%H:%M:%S"))
...
Combined_dt<-full_join(USA_dt, NZ_SA_dt)



#Lets plot them all using ggplot!
ggplot(Combined_dt, aes(x = Time.Recorded, y = Luz.Values)) + 
  geom_point(aes(color= as.factor(Site))) +
  xlab("Time") + 
  ylab("Luz Values")
