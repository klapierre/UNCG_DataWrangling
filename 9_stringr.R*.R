...
strlengthtest2
strlengthtest
#The str_pad takes the characters from strlengthtest and adds a width of 15 padded strings. Each padding character (" ")is added to the left-end side of each character. Measured by number of characters in string length, instead of the width. The output is put into the strlengthtest2 dataset.
...
##functions?
#A practical application of str_pad is making labels line up. When printing data names, padding will make everything aligned and readable. Practical application for str_trim is cleaning messy data. Sometimes data has extra spaces when importing from outside files, removing unnesscary spaces makes everything readable.
...
#First letter for each first and last name is capitalized
str_to_sentence(case_names)
#Only first letter is capitalized in each first name, and the rest is lowercase
str_to_camel(case_names)
#Each first word is lower-cased. Each subsequent word starts with a capital letter.No separation between words.
str_to_snake(case_names)
#Words are converted to lowercase and separated by underscores.
str_to_kebab(case_names)
#Words are lowercased and separated by dashes
...
#Both results are the same. The first word lowercased and each subsequent word starts with a capital letter. No separation between words.
...
NYC_Baby_Names<-read.csv("Popular_Baby_Names_20260406.csv") %>% 
...
Name_Length
#991 rows filled with numbers showing how long each child's first name is from the NYC_Baby_Names data frame, 14 is the highest number of characters contained in a given name.
...
Male_ia
#No, no names have more than one instance of the string "ia".
...
Male_Vowelless
#Since ^ is inside [], in this case this means the listed characters are "excluded". Its counting how many non-vowel characters are in each name.
#Upper and lower case are included since some names begin with a capitalized vowel or have lowercase vowels in their name.
#-------------------------------------------------------#
