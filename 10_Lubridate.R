...
# 2000-01-01 00:01:00 UTC, new years day
...
#Week gives the week number of the year and wday gives the day of the week
...
#Label gives day names and abbr gives the full name
...
#rollback moves the date to the previous month, roll to first can either go to the 
#last day of the month or the first day of the month, and preserve keeps the hour, minute, and second.
...
flight_data_rounded <- flight_data_parsed %>% mutate(rounded_flight_date = round_date(flight_date, "month"))
...
flight_timespans <- flights %>% head(20)
...
flight_timespans <- flights %>% head(20) %>% mutate(departure_time = time_hour)
...
#Turns the air time into duration measured in minutes
...
#It gives the estimated arrival time
...
flight_timespans %>% select(carrier, flight, origin, dest, air_time) %>% arrange(desc(air_time))
...
#we need the start and end time
...
#It represents the time span from when the flights leaves and when its estimated to arrive
...
#Because the meaure of length and flight is in minutes. airtime gives the flight time in 
#minutes and duration minutes is the interval converted into minutes 

# TASK: Find the longest flight interval in this dataframe.

flight_timespans %>% select(carrier, flight, origin, dest, air_time, duration_minutes, duration_hours) %>%
...
flight_timespans %>% count(short_or_long)

# QUESTION: What is the difference between a duration and an interval?

#Duration is the exact amount of time for something whereas interval is the span of time with a start and end point

# QUESTION: Explain why intervals are useful.

#intervals are useful because they can show the full timespan of two dates and times.
...
#to take specific parts of the date and tim so that we can analyze the data better
...
# start plus 7 was added

##  question why would this be helpful ? 

#It helps us with predicting and comparing windows of time 
...
#I see a new column  called start minus 1 month

## why is this useful ?

#This is useful because it helps with comparing dates before and after an event
...
#shows the difference between the start and end date time and how much time passed
...
## hint ymd 
my_birthday <- ymd("2004-11-13")
# now we will create data set called my birthday this year 
my_birthday_this_year <- ymd(paste0(year(today_date), "-11-13"))
...
#Birthday time shows the difference between todays date and my birthday this year
...
# They are helpful because they help with easily cleaning up and manipulating data
#when it comes to comparing times and identifying patterns
