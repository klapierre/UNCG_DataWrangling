...
# So transition_length controls about how long the transitions is and 
# state_length is the pauses at each state
...
lilgiffy <- ggplot(iris, aes(x = Species, y = Petal.Length)) + 
  geom_boxplot(aes(colour = Species)) +
  xlab("Species") +
  ylab("Petal Length") +
  transition_states(Petal.Width,
...
anim_save("lilgiffy.gif", lilgiffy)
...
ggplot(ChickWeight, aes(weight, Chick)) + 
  geom_point(aes(color=Diet)) +
labs(title = 'Days: {frame_time}', x = 'Chick weight (grams)', y = 'Chick number') + #makes some labels
  transition_states(Time,  transition_length = 1,
                    state_length = 1) #define transitions 
+ ease_aes('linear') #smooth out that bad boy
...
anim_save("goombagraph.gif", animation = last_animation())
...
anim_save("movingboy.gif", animation = last_animation())
...
# Moving graphs could be useful when presenting certain types of information. More enganging!
...
# A violin graph might look neat if it were animated
...
line_graph <- ggplot(orange, aes(age, circumference, group = Tree, color = as.factor(Tree))) +
  ...
  geom_point(aes(group = seq_along(age)), size=2) #scale colour picks the colors, labs is basically an aes function, theme puts the titles and legends in
# specific places, geom_points adds those lil dots.
...
#When something follows a linear path?
...
# Maybe for presentations to people outside of science? Moving bars are fun!
...
# Lol that's a lot of code down there.
