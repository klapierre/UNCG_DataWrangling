...
#The new data frame only has 3 column names which are calispell_temp, date, and time.
#The smalle_temp and winchester_temp are absent.
#The date and time were first and now calispell_temp is first.
...
all.equal("calispellTemp3", "calispellTemp4", "calispellTemp5")
# ---------------------------------------------------------- #
...
#I would expect the new dataframe to have fewer observations because you are filtering
#The temperatures lower than 15C which results in less data.
...
# R environment tab.
#It worked because it shows only values that are 15 degrees or above.
...
str(calispellTemp)
#There are 61100 observations in calispellTemp
str(calispellHighTemp)
#There are 7703 observations in calispellHighTemp
...
#The code above separates the columns into their own structures.
#The col= creates a new column.
#sep= puts the two colons between the two names in the contents of that column created.
...
#Because the column was already created.
# QUESTION: Describe in your own words what the code above does.
#The code took away the colons in the column for type ecosystem.
