...
#Only calispell_temp , Date, and Time column names are present while smalle_temp 
# and winchester_temp are absent. They are not in the same order, because 
#calispell_temp is now written first, followed by Date and then Time.
...
calispellTemp2 <- select(streamTempRename, Date:calispell_temp)

# TASK: Write code to check your column names again to see what happened in your
# new dataframe.
colnames(calispellTemp2)
#The column for Date, Time, and calispell_temp has been sequenced in that order
...
#fewer observations that the original dataframe.
...
# R environment tab.
#To knoe if it worked I checked the environment tab.
...
#The original had 61100 and the new has 7703 observations. This shows I am correct
#that the new dataframe has fewer observations.
...
#52330 observations are in the datafile calispellData. 
#There are no NA values in the calispell_temp column
sum(is.na(calispellData$calispell_temp))
...
calispellTempSum <- calispellTempSum %>% select(-sum)
...
calispellTempFaquatic <- mutate(.data=calispellTempF,
                                type='aquatic')
#It adds a column that is named type and lists aquatic for the observations.
...
# the sep= argument do?
#It combines type and ecosystem into the same column separated by ::. col= 'type_ecosystem'
#creates the new column name while c('type', 'ecosystem'), shows what columns to combine
#and sep='::') shows what to separate the combined column names by within the new column name.
...
#Seperates the column type_ecosystem into two columns named type and ecosystem.
