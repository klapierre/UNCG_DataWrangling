...
# It gives you the names of each column in the data set. This is useful because it allows 
# you to easily view the names of each columns and to change the names.
...
# All spaces and parentheses were converted to periods.
...
# before or after the =?
# The function renamed the chosen columns with names that are easier to understand. 
# Each line in the code renamed a different column. This is useful because 
#you can change the names of each chosen column all at once instead of running
#the function multiple times. The new column name comes before the = in the above code.
...
# order as before?
# (used colnames function) The new dataframe only contains three columns-- calispell_temp, data, and time.
# They are in a different order than the original dataset with the calispell_temp column listed first.
...
# fewer observations than the original
...
# The original had 61100 observations, and the new one has 7703 observations.
...
sum(filter(.data=calispellTemp, is.na("calispell_temp")))
# There are 52330 observations in the calispellData datafile.
# ---------------------------------------------------------- #
...
calispellTemp6 <- select(.data=calispellTempSum,
                         -sum)
...
calispellTempFaquatic <- mutate(.data=calispellTempF,
                                type='aquatic')
# A column named "type" was added which lists the type of environment the data was taken from.
