...
cdr <- read.csv("e001_Plant aboveground biomass carbon and nitrogen.csv") %>% mutate(Exp = 'e001') %>% rename(C=X..Carbon, N=X..Nitrogen) %>% filter(Strip==1) %>% unite(col='NTrtInfo', c('NTrt', 'NAdd'), sep='_') %>% separate(col=Species, into=c('genus', 'species'), sep=' ') %>% mutate(CN = C/N) %>% select(Exp, Date, Plot, NTrtInfo, genus, species, Field, C, N, CN)
