...
## It means the character strings will be used as factors for the data.
...
## It reads the data in the specified file.
...
## Now it only has the calispell_temp, Date, and Time  columns. The rest are absent.
## They're in the same order as the one used in the original function.
...
# dataframe.
calispellTemp2 <- select(.data = calispellTemp, Date, Time, calispell_temp)
...
## The original had 61,100 observations while the high temp one has 7,703 observations.
...
str(highTempTributaries)
## Yes, there's 8,093 observations.
...
str(calispellData)
## 52,330 observations are in calispellData
...
calispellTempFaquatic <- mutate(.data=calispellTempF,
                                type='aquatic')
## It adds a new column called type with aquatic being listed for each observation.
...
# the sep= argument do?
## The col portion creates the new column, the c within it tells R which
## columns to combine. The sep part just tells R what to seperate the 
...
## It's because it's refrencing an existing column.

# QUESTION: Describe in your own words what the code above does.
## It turns the one column into 2 and specifies what to call them.
