...
#This dataframe only contains the columns for calispell_temp, date, and time.
#Calispell now comes first and winchester and smalle are missing
...
identical(calispellTemp5, calispellTemp4)
...
#We would expect the new dataframe to have less observations if we are taking some out
...
#61100 obs originally, 7703 obs in new dataframe 
...
#52330 obs
calispell_temp_NA <- nrow(calispellTemp)-nrow(calispellData)
calispell_temp_NA
...
# from the dataframe.
calispellTempNoSum <- select(.data=calispellTempSum,
                           -sum)
...
calispellTempFaquatic <- mutate(.data=calispellTempF,
                                type='aquatic')
#This function added a column that lists the type of measurement for each observation
...
calispellTempF_eco <- mutate(.data=calispellTempFaquatic,
                                ecosystem='stream')
...
calispellTempF4 <- unite(data=calispellTempF_eco,
                         col='type_ecosystem',
                         c('type', 'ecosystem'),
                         sep='::')
...
#The code above combines the type and ecosystem columns
#unite creates the new column, c('type', 'ecosystem') tells R what to combine
#sep :: means the two combined variables are seperated by ::

# Another very useful function is separate, which takes apart a column into two or
# more pieces.

# TASK: Run the following code:
calispellTemp5 <- separate(data=calispellTempF4,
                            col=type_ecosystem,
