...
#ANSWER: It gives you the column names. It is useful so you can identify the variables.
...
#ANSWER: The spaces were deleted and replaced with periods.
...
#ANSWER: Some of the col names are abbreviated and the periods are replaced with underscores. The first line made a new df and called the data that we will use. The second line renamed the the third column. The third line renamed the fourth column, and the fourth line renamed the fifth column. 
#This is useful if the data file that you are using was not organized in an "easy-to-use" way. Renaming them in R will help make a more reproducable code (as long as you rename it appropriately).
#The renamed column comes before the =. (new_name = old_name)
...
#ANSWER: The columns that are present are "calispell_temp", "Date", and "Time". The columns that are absent are "smalle_temp" and "winchester_temp". No they are not in the same order as before. The "calispell_temp" col is before "Date", and "Time".
...
#Answer: I would expect the df to have fewer observations.
...
str(calispellTemp)
#ANSWER: The original dataset had 61100 observations and the calispellHighTemp has 7703 observations.
...
#ANSWER: There are 52330 observations in the calispellData df. 
sum(is.na(calispellTemp$calispell_temp)) #8770 were NA
...
calispellTempFeco <- mutate(.data=calispellTempFaquatic,
                                ecosystem='stream')
head(calispellTempFeco)
...
# of code:
calispellTempF4 <- unite(data=calispellTempFeco,
                         col='type_ecosystem',
                         c('type', 'ecosystem'),
                         sep='::')
...
#ANSWER: The above code combines the data from the 'type' and 'ecosystem' columns. "col = 'type_ecosystem'" makes a new column. 'unite()' combines the columns. 'sep =' tells R what to separate the merged data with. in this case we are separating with '::'. 
...
#ANSWER: it is separating teh 'type_ecosystem' column into two different columns ' type' and 'ecosystem.
