...
#maybe 60-70 points on the graph, much less than in the dataframe
...
#all data points are present in the graph
...
#the points move slightly everytime the code is ran
...
# it uses distant colors for eachother within the palette
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c(117, 305, 555))
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#14D418', '#5994B0', '#E137E5'))
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#14D418FF', '#5994B080', '#E137E500'))
...
#from the 'class' column in the mpg dataset
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) + #call dataset and set x/y variables and color by class
  geom_jitter() + #jitter/dot plot
  scale_color_manual(values=c('#FCBA03', '#380754', '#496916'), #set colors for classes
                     name='Class of Car', #title legend
                     breaks=c('suv', 'midsize', 'compact'), #set legend with correct points
                     labels=c('SUV', 'Midsize', 'Compact')) #rename legend contents
...
#the graph shows SUVs getting better gas milage than compact cars which is generally incorrect. Its important to make sure your legend and data match up
...
# above?
#the legend is only partially visible, going beyond the view of the graph
