...
#The legend ends up in weird positon, covering part of the graph.
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) + 
  geom_jitter() + 
  theme(legend.position=c(1,0), legend.justification=c(1,0)) + theme(legend.position="none")
...
highwayMPG <- group_by(mpg, class) %>% summarise(hwy_mean = mean(hwy, na.rm= T), hwy_sd = sd(hwy, na.rm = T)) %>% 
...
ggplot(highwayMPG, aes(x = hwy_mean, y = class, fill = class)) + 
  geom_bar(stat='identity') +
  geom_errorbar(aes(xmin=hwy_mean-hwy_se, 
                    xmax=hwy_mean+hwy_se, 
                    width=0.2))  
...
ggplot(highwayMPG, aes(x = hwy_mean, y = class)) + 
...
#The class column of the highwayMPG dataset. 
...
#The graph appears to have a different max for it's scale. I think the Cartesian coordinate
#system sets a coordinate system for the bar graph to be mapped in.
...
cityMPG <- mpg %>% group_by(class) %>% summarize(city_mean = mean(cty), city_sd = sd(cty),city_se=1.96*city_sd)
...
ggplot(cityMPG, aes(x = fct_reorder(class, city_mean, .desc = TRUE), y = city_mean, fill = class)) +
