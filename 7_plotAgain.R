...
str(mpg)
# There are 234 obs. in 11 columns
...
# There are roughly 78 points on the graph, which is a lot smaller than
...
# A LOT more observations were added to the plot and some points were added in randomly.
...
# The graph slightly shifts because the geom_jitter() function randomly moves the points 
# along the x and y axises.
...
ggplot(data=mpg, aes(x=cty, y=hwy)) + 
  geom_jitter(height= F, width=0.5)
...
# For Set1, the first three colors are assigned to the three variables.
# For RdBu, the extremes and a middle ground color were assigned to the variables.
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('388', '213', '515'))
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#40db42', '#000000', '#cc5e1f'))

#I call it "Spooktacular"
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#40db42ff', '#00000080', '#cc5e1f00'))

# QUESTION: What happened to the point that you set to 100% transparent?

# Those points are completely transparent
...
# The titles were provided in the aes() function and the values come from the mpgSubset dataframe
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) + # This creates our scatterplot with the assigned x and y axises, and colors scaled by class
  geom_jitter() + # This tells R to insert the data in a spaced out manner. 
  scale_color_manual(values=c('#FCBA03', '#380754', '#496916'), # This assigns a color to each value
                     name='Class of Car', # This renames the title of the variables 
                     breaks=c('suv', 'midsize', 'compact'), # This breaks the data up by variable
                     labels=c('SUV', 'Midsize', 'Compact')) # This gives labels to the broken up variable data
...
# The labels for the break data is assigned to the wrong variables and has a misleading output.
...
# above?
# The legend is placed in the wrong area, to the bottom right outside of the graph
...
highwayMPG <- mpg%>%
  group_by(class) %>%  
  summarize(across(.col="hwy", .fns=list(mean=mean, sd=sd),na.rm=T)) %>% 
...
ggplot(data=highwayMPG, aes(x=class, y=hwy_mean))+
  ...
  geom_errorbar(aes(ymin=hwy_mean-hwy_se, ymax=hwy_mean+hwy_se), width=.2)
...
cityMPG <- mpg%>%
  group_by(class) %>%  
  summarize(across(.col="cty", .fns=list(mean=mean, sd=sd),na.rm=T)) %>% 
...
# (6) no legend.
ggplot(cityMPG, aes(x=reorder(class, cty_mean, decreasing = T), y=cty_mean, fill=class)) + 
  geom_bar(color="darkgrey", stat="identity")+
  scale_fill_brewer(palette="Spectral")+
  scale_x_discrete(name = "Class of Vehicles",
                   labels = c("Pickup", "SUV", "2 Seater", "Minivan", "Mid-size", "Compact", "Sub-compact"))+
  geom_errorbar(aes(ymin=cty_mean-cty_se, ymax=cty_mean+cty_se), width=.3)+
  scale_y_continuous(breaks=seq(0, 30, 5),
                     name = "City Mean (MPG)") +
  coord_cartesian(ylim=c(0,30))+
  theme(legend.position = "none")
...
ggplot(manufacturerFreq, aes(x="", y=frequency, fill=manufacturer)) + #Inputs data and applies axises to variables
  geom_bar(stat="identity", width=1) + # This gives us a barplot instead of a histogram, with a width of 1
  coord_polar(theta="y", start=0) + # This circularizes the data output
  theme_void() + #Removes the any background
  theme(legend.title = element_text(size = 12.5), # Edits the legend title text size
        legend.text  = element_text(size = 8.5), # Edits the text size within the legend
        legend.key.size = unit(.75, "lines")) # Size and type fo legend units
...
ggplot(economics, aes(x=date, y=unemploy, color=date))+
  geom_point()+
  geom_line()
