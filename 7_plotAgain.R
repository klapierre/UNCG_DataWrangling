...
# that compare to the number of observations in the dataframe?
    ...
    # about 78 points on the plot, which is much much smaller
...
# QUESTION: What happened when you created the plot with geom_jitter?
    ...
    # it plotted the data but added some noise to the point positions
...
# again and again. What happens each time? Why is this happening?
    ...
    # each time its very slightly different, probably because thhe jitter moves the points randomly by a small amount, rather than by a set amount for each point
...
# three? Some other combination?)
    ...
    # for set 1 it just used the first 3 colors, but for RdBu it used the middle three
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c(48, 579, 333))
...
colorFrame <- as.data.frame(colors())
    # you can get the number by just seeing the position of the color's name in the colors() list
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#4fb1db', '#4fd9db', '#4fdbb1'))
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#4fb1db00', '#4fd9db80', '#4fdbb1ff'))

# QUESTION: What happened to the point that you set to 100% transparent?

    # it disappears because its fully transparent
...
# QUESTION: Where did ggplot get the legend title and values from?
    ...
    # it pulled the title from the "class" column's name and the values from the values of that column
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) + 
  geom_jitter() + 
  scale_color_manual(values=c('#4fb1db', '#4fd9db', '#4ff5b1'),
                     # setting the colors for the legend and points
                     name='Class of Car', 
                     # setting the title of the legend
                     breaks=c('suv', 'midsize', 'compact'), 
                     # setting what values from the data frame are being used
                     labels=c('SUV', 'Midsize', 'Compact'))
                     # renaming the labels on the legend for those values
...
# careful with the order you pass information into ggplot?
    ...
    # while the points are colored the same as they were before, the labels have been arranged incorrectly so that the legend misrepresents the class of the points
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) + 
  ...
  theme(legend.position=c(1,0))
    # the legend goes off the screed
