...
# ANSWER: For the Qualitative palette, it used the first 3 colors. In the Diverging Palettes the colors were chosen from the middle going outward.
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c(612, 2, 86))
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#b7ebc5', '#ddb7eb', '#84a7bd'))
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#b7ebc5FF', '#ddb7eb50', '#84a7bd00'))

# QUESTION: What happened to the point that you set to 100% transparent?
# ANSWER: It is no longer visible in the graph
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) + #create a plot from mpgSubset dataframe, make the x axis the values from cty, and the y axis the values from hwy, distinguish the class of car by color
  geom_jitter() + #prevent overlapping of points
  scale_color_manual(values=c('#FCBA03', '#380754', '#496916'), #choose hex values
                     name='Class of Car', #legend text
                     breaks=c('suv', 'midsize', 'compact'), #original labels
                     labels=c('SUV', 'Midsize', 'Compact')) #new labels
...
# ANSWER: Now the legend is saying that the wrong colors represent the wrong class of car.
...
ggplot(manufacturerFreq, aes(x="", y=frequency, fill=manufacturer)) + #use data from manufacturerFreq data set, with no x value, and frequency for the y value, fill the color by manufacturer
  geom_bar(stat="identity", width=1) + #create a bar chart
  coord_polar(theta="y", start=0) + #convert bar chart to pie chart
  theme_void() + # get rid of extra text on chart
  theme(legend.title = element_text(size = 12.5), #change size of text of legend title
        legend.text  = element_text(size = 8.5),#change size of text of legend text
        legend.key.size = unit(.75, "lines"))#change size of legend
