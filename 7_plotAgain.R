...
# It seems to pick a somewhat random set of colors, otherwise the colors would
# be rather close to each other. For instnce the RdBu set starts with red and 
# shifts to blue.
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('653', '420', '129'))
...
# By using as.data.frame you can find out which number corresponds to each
# color name.
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#8f4540', '#488f43', '#4b4a91'))
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#8f454000', '#488f5050', '#4b4a91FF'))

# QUESTION: What happened to the point that you set to 100% transparent?
# It became transparent, i.e. invisible on the graph.
...
# From our dataframe (in this case mpgSubset)
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) + #creates a plot and specifies data/axes/how points will be colored
  geom_jitter() + #jitters the points on the plot so they're not overlapping
  scale_color_manual(values=c('#FCBA03', '#380754', '#496916'),#specifying colors for each class
                     name='Class of Car', #adds a label to our legend
                     breaks=c('suv', 'midsize', 'compact'), #says which labels in our legend we're changing
                     labels=c('SUV', 'Midsize', 'Compact')) #changes the previously mentioned labels 
...
# The classes have been mislabeled as compared to the dataframe, i.e. suv has been 
# changed to compact and so on.
...
# above?
# The legend bleeds off the chart, and is not fully visible.
...
cityMPG <- mpg %>%
  ...
  summarize(
    ...
    city_se=(sd(cty, na.rm=TRUE))*1.96) %>%
  ungroup()
...
# It makes a count of the number of cars of each manufacturer.
...
# A violin plot shows the distribution of individual data points by a subset.
