...
ggplot(data = mpg, aes(x = cty, y = hwy, color = class)) + geom_point() + 
...
#There are around 80 or 90 dots in the graph and its ledd than the number of actual 
...
#The graph showed more dots on the graph than before
...
#the dots get randomized, this happens because the jitter code randomizes the dots
...
#It got the legend from what was inside aes() and from the names in the class column in the dataframe
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) + #Makes a plot and changes the x and y axis and adds color to class
  geom_jitter() + scale_color_manual(values=c('#FCBA03', '#380754', '#496916'),#To jitter points and pick colors for classes
    name='Class of Car', breaks=c('suv', 'midsize', 'compact'), #Changes title of legend and pick order of categories in legend
                     labels=c('SUV', 'Midsize', 'Compact')) #Pick how the names appear
...
#The legend becomes mislabeled and its important because it matches the labels by how we put them.
...
#If not specified, the default will be used for where the legend goes.
...
#It says stat_bin() requires a continuous x aesthetic.

# QUESTION: What happens when you follow the advice of the error message and 
# make stat='count'?
ggplot(mpg, aes(manufacturer)) + geom_histogram(stat="count")

#A bar chart is created instead of a histogram
...
#It uses the density of the values and combined a box plot and density, which makes the shape
#Look like a violin.
