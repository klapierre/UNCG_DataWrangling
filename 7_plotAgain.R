...
# There are 234 observations in the mpg data frame. 
...
# There are approximately 78 dots in the graph created. We have about one third 
# of the observations in the graph. This is because many points overlap in the 
# graph.
...
# When we created a plot with geom_jitter all the points that were on top 
# of each other spread out. This gives us more information from 
# the data set. 
...
# When running the code again the points change position each time. This happens because 
# geom_jitter adds random variation to the points. 
...
# ggplot got the legend title from class and the values from the categories wiithin our 
# variable. 
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) + 
  ...
  scale_color_manual(values=c('#FCBA03', '#380754', '#496916'),
                     ...
                     name='Class of Car', 
                     # legend name class of car
                     breaks=c('suv', 'midsize', 'compact'), 
                     # The order they are in the legend 
                     labels=c('SUV', 'Midsize', 'Compact'))
                    # Renames the legends to have capitalization 
...
# The order of breaks is not the same as the order of labels. It is important to 
# be careful with the order you pass information into ggplot because data could be 
# interpreted differently if it is not done in the correct order. 
...
# The legend is in the bottom far right of the graph, it is not properly adjusted in the 
#graph without the legend.justification. 
...
ggplot(cityMPG, aes(x = reorder(class, -city_mean), y= city_mean, fill = class)) +
  ...
  geom_errorbar(
    aes(ymin = city_mean - city_se, ymax = city_mean + city_se),
    width = 0.3
  ) +
...
# It removes the error but the data looks different than our previous graphs because
# manufacturer is categorical and not continuous. 
...
# The violin plot shows the distribution of numeric variables for each category.
# It helps you see the overall shape and where the data is concentrated. 
