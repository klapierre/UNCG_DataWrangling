...
##ANSWER: For the "Set1" color palette, it uses the first 3 colors. For the "RdBu" color palette, it uses a combination of colors in the "middle" of the palette, one on the red side (light orange), one white, and one on the blue side (light blue).
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#f274ab', '#c4a2f5', '#a2c0f5'))
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#f274abFF', '#c4a2f580', '#a2c0f500'))

# QUESTION: What happened to the point that you set to 100% transparent?
##ANSWER: The points are not visible on the graph. 
...
ggplot(highwayMPG, aes(x = class, y = hwy_mean, fill = class)) +
         ...
         geom_errorbar(aes(ymin = hwy_mean - hwy_se,
                           ymax = hwy_mean + hwy_se), width = 0.2) 
...
#ANSWER: It succesfully made the histogram with the correct x axis. 
...
#ANSWER: The violin plot shows the distributions of the numeric data using density curves (similar to a box plot, but with density curves).
...
ggplot(manufacturerFreq, aes(x="", y=frequency, fill=manufacturer)) +
  # Creates a plot from manufacturerFreq with no x axis, a y axis for frequency, and fileld with manufacturer. 
  geom_bar(stat="identity", width=1) +
  # Creates a bar graph using numbers from the dataset with a width of 1
  coord_polar(theta="y", start=0) +
  # Turns the bar graph into a circle, creating a pie chart, theta = y ensures that each "slice" reaches to the end of the circle and start = 0 indicates that the first data point should be at the "12 o clock" position of the pie chart." 
  theme_void() +
  # Sets the theme to void, removing any background elements like grid lines.
  theme(legend.title = element_text(size = 12.5), 
        # Sets the side for the text of the title of the legend.
        legend.text  = element_text(size = 8.5),
        # Sets the size for the text of the legend
        legend.key.size = unit(0.75, "lines"))
        # Sets the size of the legend key as 75% of the size of the text
