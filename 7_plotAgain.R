...
ggplot(mpg, aes(x = cty, y = hwy)) + 
  geom_point(shape = 24, aes(fill = as.factor(class))) +
...
# I'm gonna say less than 100. Which is less than all the observations
...
# The data points moved around a bit.
...
# Points seem to be moving around their orginal point by a bit in a random direction each time it's run.
...
mpgSubset <- mpg %>% mutate(class = ifelse(class %in% c("compact", "midsize", "suv"), class, NA)) %>% drop_na(class)
...
# Seems to have chosen at regular intervals throughout the pallete?
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#7BAFD4', '#A61A32', '#0F2044'))
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#7BAFD400', '#A61A3280', '#0F2044FF'))


# QUESTION: What happened to the point that you set to 100% transparent?

# They became spectral :O
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) + 
  geom_jitter() + 
  scale_color_manual(values=c('#FCBA03', '#380754', '#496916'), #Change point colors
...
# Labels are not in the same order as break. So the points are representing different groups.
...
# This sets the Y min and max for visualizing data
...
# It was taught to me as sort of the "shape" of the data. It's basically a 
# boxplot but presents a geometric object that is wider where there are more
# data points and more narrow where there are less data points/
...
# 681 call ggplot, assign what values to present and how to color them.
# 682 tell R its a bar plot and organize by identity (manufact. in this case)
# 684 change to a specific pie chart form
# 684 $ 685 set the theme and move the lengend to desired position.
