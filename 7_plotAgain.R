...
#For Set1, it appeared to use the first three colors, but for RdBu, it used the
#middle color and what I think is the colors two away from the middle in either
#direction.
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c(20, 412, 299))
#My new figure doesn't look great as two of the colors are way too similar.
...
#putting the colors into a dataframe gives a numbered list of all of them.
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#940c0c', '#531875', '#2b6128'))
...
ggplot(data=mpgSubset, aes(x=cty, y=hwy, color=class)) +
  geom_jitter() +
  scale_color_manual(values=c('#940c0cFF', '#53187580', '#2b612800'))

# QUESTION: What happened to the point that you set to 100% transparent?

#It turned white, because this is the color of the background of our plot.
...
highwayMPG <- mpg %>% 
  group_by(class) %>% 
  summarise(hwy_mean = mean(hwy, na.rm = TRUE),
            hwy_sd = sd(hwy, na.rm = TRUE),
            hwy_se = 1.96*sd(hwy, na.rm = TRUE))
...
ggplot(data=highwayMPG, aes(y=hwy_mean, x=class, fill=class)) + 
...
cityMPG<-mpg %>% 
  group_by(class) %>% 
  summarise(city_mean = mean(cty, na.rm = TRUE),
...
ggplot(cityMPG, aes(x=reorder(class, -city_mean), y=city_mean, fill=class)) + 
  ...
  scale_y_continuous(breaks=seq(0,30,5), name='Mean City MPG')+
  scale_x_discrete(labels=c('subcompact', 'compact', 'midsize', 'minivan', 'sport', 'SUV', 'pickup'), name='Car Class')+
  scale_fill_manual(values=c('#B23539', '#FAB57C', '#F7E790', '#73652D', 
                             '#E79498', '#514289', '#14232A'))+
...
ggplot(mpg, aes(manufacturer)) + 
  geom_histogram()
#The error reads: "Error in `geom_histogram()`:
...
#It graphs how many cars of each manufacturer are in the dataset. 
...
#It shows the distribution of how common different y values were for each x
#value.
