...
##There are about 75 dots visible but there are actually 234 observations because many points overlap.
...
##The points shift around a little due to being randomly placed.
...
##Keeps the graph in visible range.
...
ggplot(data = highwayMPG, aes(x = class, y = hwy_mean, fill = class)) +
  ...
  scale_x_discrete(labels = c(
    ...
    "Subcompact",
    "SUV")) +
  scale_y_continuous(name = "Highway MPG", breaks = seq(0, 30, 5)) +
...
ggplot(cityMPG, aes(x = reorder(class, -city_mean), y = city_mean, fill = class)) +
  ...
  geom_errorbar(aes(ymin = city_mean - city_se, ymax = city_mean + city_se),
                width = 0.3) +
  scale_fill_brewer(palette = "Set2") +
  scale_y_continuous(name = "City MPG", breaks = seq(0, 30, 5)) +
  coord_cartesian(ylim = c(0, 30)) +
  xlab("Class of Car") +
  theme(legend.position = "none")
...
ggplot(manufacturerFreq, aes(x="", y=frequency, fill=manufacturer)) + ##sets up plot, uses frequency for size and manufacturer for color
  geom_bar(stat="identity", width=1) + ##creates bars using actual values and removes spacing
  coord_polar(theta="y", start=0) + ##converts bar chart to pie chart
  theme_void() + ##removes background, axes, and gridlines
  theme(legend.title = element_text(size = 12.5), ##changes legend title size
        legend.text  = element_text(size = 8.5), ##changes legend text size
        legend.key.size = unit(.75, "lines")) ##changes legend box size
